...
### `/loan-application-commissions` — сверка кнопок (2026-07-09)
Повод: аудит мокапа `mockups/loan-application-commission/commission.html` показал 12
кнопок без обработчика. Сверка со стендом (`scripts/inspect/commission-mockup-parity*.mjs`,
2026-07-09) показала, что **на живом почти всё работает** — сортировка колонок,
фильтр, лукапы `•••`/`✕`, «Просмотр заявки», датапикер «Дата заседания», селекты
«Решение»/«Риск», сохранение по OK. Это расхождения мокапа, а не дефекты приложения —
в `TODO.md` они не заводятся. Ниже — только то, что мертво/криво **на самом стенде**.

- **P3-19** — `/loan-application-commissions` список — 🟡 minor — _проверено 2026-07-09 (`commission-mockup-parity4.mjs`)_
  - **Issue:** Плитка **«Всего поручительств» = 0**, при этом соседние плитки дают
    «На рассмотрении» 1 + «Одобрено» 1 и грид содержит 2 строки.
  - **Expected:** плитка «Всего…» = сумма остальных четырёх (= числу строк без фильтра).
  - **Actual:** счётчик не наполняется — всегда 0.
  - **Notes:** плитки не кликабельны (`cursor:auto`, нет обработчика) — как фильтры не
    работают. Плитка «Одобрено» при значении 1 всё равно с серой рамкой
    `rgb(121,125,123)` вместо зелёной — цвет рамки не связан со значением. → P3-R32.

- **P3-20** — `/loan-application-commissions` фильтр + плитки — 🔵 cosmetic — _проверено 2026-07-09_
  - **Issue:** На экране комиссий **по заявкам** подписи говорят про поручительства:
    фильтр «**Поручительство** =», плитка «Всего **поручительств**».
  - **Expected:** «Заявка =» / «Всего заявок» (или «Всего комиссий»).
  - **Actual:** чужая доменная сущность в подписях — экран, видимо, склонирован с
    `/guarantee-commissions`. → P3-R32.

- **P3-21** — `/loan-application-commissions/{id}` диалоги — 🟡 minor — _проверено 2026-07-09 (`commission-mockup-parity1/3.mjs`)_
  - **Issue:** Модальные диалоги **«Ваш отзыв»** (Проголосовать) и **«Подтверждение»**
    (Одобрить / Отклонить / Запросить доп. информацию / Удалить) не закрываются ни по
    **Escape**, ни по клику по подложке — оверлей остаётся открытым.
  - **Expected:** Escape закрывает немодальные диалоги подтверждения (стандарт Vaadin
    `no-close-on-esc=false`); как минимум — единообразно по всем диалогам модуля.
  - **Actual:** единственный выход — «Отмена» или `✕`. Если пользователь ждёт Escape,
    экран воспринимается как зависший. → P3-R33.

- **P3-22** — `/loan-application-commissions` подтверждения — 🔵 cosmetic — _проверено 2026-07-09_
  - **Issue:** Кнопки подтверждения рассогласованы: удаление — «**Да / Нет**»
    («Вы действительно хотите удалить выбранные элементы?»), решение комиссии —
    «**Да / Отмена**» («Вы уверены?»).
  - **Expected:** одна пара подписей на модуль.
  - **Actual:** две разные пары в двух диалогах одного экрана. → P3-R33.

- **P3-23** — `/loan-application-commissions/{id}` подтверждение решения — 🟡 minor — _проверено 2026-07-09_
  - **Issue:** «Одобрить», «Отклонить» и «Запросить доп. информацию» открывают
    **один и тот же** диалог с текстом «**Вы уверены?**» — ни выбранное действие, ни
    номер записи не названы.
  - **Expected:** «Одобрить заявку «Проверка комиссии - 138»?» — действие + запись в тексте.
  - **Actual:** необратимое решение комиссии подтверждается безымянным вопросом;
    промах по соседней кнопке не ловится. → P3-R33.

- **P3-24** — `/loan-application-commissions` фильтр — 🟡 minor — _проверено 2026-07-09 (`commission-mockup-parity3.mjs`, `.auth/parity-filter-noenter.png` / `-enter.png`)_
  - **Issue:** Фильтр применяется **только по Enter**. Ввод «zzz» в «№ содержит» без
    Enter оставляет грид как есть («2 строки»); после Enter — «0 строк». Кнопки
    «Найти» / «Применить» на экране нет, состояние «фильтр введён, но не применён»
    ничем не обозначено.
  - **Expected:** либо кнопка применения, либо debounce-фильтрация по вводу, либо
    явная подсветка неприменённого условия.
  - **Actual:** молчаливое расхождение между тем, что в полях, и тем, что в гриде. → P3-R32.

- **P3-25** — `/loan-application-commissions/{id}?mode=readonly` → Члены комиссии — 🔵 cosmetic — _наблюдение 2026-07-09; требует подтверждения_
  - **Issue:** Колонка «Комментарий» содержит кнопку с иконкой `lumo:eye-disabled` в
    состоянии `disabled` для **обеих** строк записи 138.
  - **Наблюдение, не вывод:** у членов комиссии записи 138 комментариев нет, поэтому
    неясно, отключена ли кнопка **по причине пустого комментария** (корректно) или
    **всегда** (дефект). Нужна запись с заполненным комментарием члена комиссии.
  - **Next:** перепроверить после того, как в тест-данных появится проголосовавший член
    комиссии с текстом. → P3-R34.

### Комиссия — UX/UI-разбор экрана (2026-07-10)

_Источник: разбор as-is мокапа `mockups/loan-application-commission/commission.html`
(попиксельная clone-сверка живого стенда 2026-07-08, git `c50077b`). Дефекты P3-26…P3-33
относятся к тому же экрану `/loan-application-commissions`, что и P3-19…P3-25;
подтверждение на живом стенде — отдельной задачей (см. «Next» у P3-26)._

- **P3-26** — `/loan-application-commissions/{id}` → Финальное решение — 🔴 blocker — _разбор 2026-07-10_
  - **Issue:** Кнопки «Одобрить» / «Отклонить» / «Запросить доп. информацию» доступны
    **всегда**, без связи с голосованием. Прогресс голосования размещён **под** кнопками
    решения — председатель физически выносит решение раньше, чем видит голоса.
    Поля «Номер протокола» и «Дата заседания» ничего не блокируют и могут остаться пустыми.
  - **Expected:** решение недоступно, пока не набран кворум голосов и не заполнены
    номер протокола + дата заседания; причина блокировки — человекочитаемым текстом.
  - **Actual:** комиссию из 2 человек можно «одобрить» при **0 голосов** и пустом протоколе.
  - **Next:** подтвердить на живом стенде, что серверная сторона тоже не проверяет кворум
    (UI-гейт без серверного — не защита). → P3-R38.

- **P3-27** — `/loan-application-commissions/{id}` → Состав комиссии — 🟠 major — _разбор 2026-07-10_
  - **Issue:** Член комиссии не видит голосов и комментариев других членов: голосует
    он в редакторе, а «Отзывы» — вкладка экрана «Просмотр». Секция «Комментарии членов
    комиссии» в редакторе — **пустая карточка без содержимого** (дублирует вкладку «Отзывы»).
  - **Expected:** решение, риск и комментарий каждого члена — прямо в гриде «Состав комиссии».
  - **Actual:** коллегиальный орган работает вслепую; мёртвая секция занимает экран. → P3-R38.

- **P3-28** — `/loan-application-commissions/{id}` → «Проголосовать» — 🟠 major — _разбор 2026-07-10_
  - **Issue:** «Проголосовать» — одна ссылка-кнопка в шапке секции «Состав комиссии»,
    без привязки к строке текущего пользователя. Из UI не следует, **за кого** подаётся голос.
  - **Expected:** действие в строке своего члена комиссии; чужие строки — read-only;
    повторное голосование заблокировано.
  - **Actual:** непонятно, чей голос будет записан. → P3-R38.

- **P3-29** — `/loan-application-commissions/{id}` — 🟠 major — _разбор 2026-07-10_
  - **Issue:** У одной сущности **две разные информационные архитектуры**. «Просмотр»
    (`?mode=readonly`): шапка + прогресс + 5 вкладок (Обзор/Отзывы/Документы/История/Протокол).
    «Изменить» (`/{id}`): шапка + плоская стопка карточек + **второй** блок прогресса внизу,
    вкладок нет. Итог: «Отзывы» видны только в одном режиме, решение выносится только в другом.
  - **Expected:** один экран карточки; read-only/edit — состояние, а не другая вёрстка.
  - **Actual:** пользователь переучивается при каждом переходе. → P3-R38.

- **P3-30** — `/loan-application-commissions` список — 🟠 major — _разбор 2026-07-10_
  - **Issue:** Список — реестр, а не рабочий стол. Нет ответа на два главных вопроса члена
    комиссии: «где ждут **моего** голоса» и «что **просрочено**». Поле «Крайний срок» есть
    в гриде членов комиссии на деталке, но в списке его нет ни колонкой, ни фильтром.
  - **Expected:** плитка/фильтр «Ждут моего голоса» + колонка «Крайний срок» с подсветкой
    просрочки.
  - **Actual:** чтобы найти свою работу, надо открыть каждую запись. → P3-R39.

- **P3-31** — `/loan-application-commissions` грид — 🟡 minor — _разбор 2026-07-10_
  - **Issue:** Четыре дефекта одного грида:
    1. «Тип решения» выводится **простым текстом**, хотя статус-токены (`--status-success-bg`,
       `--status-warning-bg`, `--status-error-bg`) в стилях экрана **объявлены и не использованы**;
    2. колонка «Комиссия» показывает `0/2` — это голоса, но из заголовка это не следует;
    3. колонка «Тип документа» = `Заявки` во **всех** строках (константа, 0 бит информации),
       колонка «Комментарий» всегда пуста;
    4. заголовки и ФИО обрезаны (`nowrap` + `ellipsis`) **без `title`** — прочитать нельзя.
  - **Expected:** статус-чип; колонка «Голоса» с подписью «N из M»; константные и всегда-пустые
    колонки убрать; обрезанные ячейки — с `title`.
  - **Actual:** плотный грид, из которого не считывается ни статус, ни прогресс. → P3-R39.

- **P3-32** — `/loan-application-commissions/{id}` → Прогресс голосования — 🟡 minor — _разбор 2026-07-10_
  - **Issue:** 1. Полоса прогресса **одноцветная зелёная**, хотя легенда под ней даёт четыре
    категории (одобрили / отклонили / воздержались / не проголосовали) — раскол голосов
    визуально неотличим от единогласия.
    2. Заголовок «Прогресс голосования - 0.0%» — процент **от чего**, не сказано.
    3. Цвета легенды заданы мимо палитры: «Отклонили» = `rgb(255,123,123)`, контраст на белом
    **≈ 2.3:1** — провал WCAG AA (нужно ≥ 4.5:1); «Не проголосовали» = `rgb(34,39,46)`,
    чёрная плашка читается как «отключено».
  - **Expected:** стек-сегменты по 4 категориям; «Проголосовало N из M · кворум K»;
    цвета из палитры (`--asubk-green` / `--asubk-red` / `--asubk-amber` / серый).
  - **Actual:** ключевой индикатор экрана не отражает состав голосов и не проходит по контрасту.
    → P3-R40.

- **P3-33** — `/loan-application-commissions/{id}` → пустые состояния — 🟡 minor — _разбор 2026-07-10_
  - **Issue:** Тупики и мёртвые зоны:
    1. пустые гриды «Состав» / «Документы» рисуют **150px пустоты внутри рамки** (`.empty-rows`)
       без текста и без действия;
    2. в секции «Документы» редактора **нет кнопки загрузки вообще**;
    3. вкладка «Протокол» показывает «Протокол не сформирован» и **не предлагает действия**
       (при том что загрузка скана протокола требуется процессом — см. P3-R35);
    4. «История» — одна строка текста, а не таймлайн кто/что/когда;
    5. read-only поля нарисованы как Vaadin-селекты **с шевроном** — выглядят активными.
  - **Expected:** empty state с текстом и действием; «Загрузить» в документах; действия на
    вкладке «Протокол»; таймлайн; read-only значения без имитации контролов.
  - **Actual:** пользователь упирается в экраны, с которых некуда идти. → P3-R40.
