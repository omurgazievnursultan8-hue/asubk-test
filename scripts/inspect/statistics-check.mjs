...
(() => {
  ...
  let late269 = {}, add269 = {}, rm269 = {}, cl269 = {}, back269 = {}, once269 = {}, late42 = {};
  /* Открытый август: мера родилась с поручителем; заёмщика добавили — ночь 21.08, пересчитанная
     ...
     распоряжения рождает её на дату строки меры и снимает представительство с поручителя на
     месте; заёмщика сняли — его пара «сторнирована» без представительства, на месте. */
  ST.seed();
  ...
  try {
    ...
    /* Мера закрытого мая, родившаяся ночью 22.08 (срез её дня закрыт — строка на ночь), получает
       заёмщика: пара-заёмщик ложится на дату строки меры 22.08, а не на ночь 23.08, — иначе
       22.08 поручитель уже снят с представительства на месте, а заёмщика ещё нет, и у меры
       представителя нет вовсе. Мера МВ-2026/42 живёт только внутри сторожа. */
    Wm[i269] = JSON.parse(keep269);
  ...
  ok(269, pairs.size === 7 && dist.size === 5 && prim.size === 5 &&
        ...
        add269.rows === '08-13:11-1+ 08-13:11-2' && none269(add269.bad) && all269(add269.cnt, '6/6') &&
        rm269.rows === '08-13:11-1× 08-13:11-2+' && none269(rm269.bad) && all269(rm269.cnt, '6/6') &&
        ...
        late42.rows === '08-22:03-1+ 08-22:03-2' && none269(late42.bad) && all269(late42.cnt, '6/6'),
    `мера направлена на цели, и строка лежит у пары (ИС-55, схема §11.2): пар ${pairs.size}, мер ${dist.size}. Представитель у каждой меры один — цель-заёмщик (${[...prim].join(' · ')}); по нему считаются суммы, а число мер — различными мерами: срез отвечает ${cnt269.ok ? cnt269.total['a-count'].v : '—'}, а не ${pairs.size}. Дверь меры на дату отдаёт все пары (${door269.length}), срез — представителей (СС-175). Цели меняются — представитель один (правка ревью 1 З-16b; строки: «+» представитель, «×» сторнирована, «*» зафиксирована). Открытый август, МВ-2026/41: заёмщика добавили — пересчёт ночи 21.08 после ночи 22.08 пару не родил (не тронуто ${late269.kept}, рождено ${late269.created}), плановая ночь 23.08 без распоряжения родила её сама (${add269.created || '—'}) на дату строки меры — ${add269.rows || '—'}, решения ночи: ${add269.tally || '—'}; заёмщика сняли — ${rm269.rows || '—'}, его пара на 23.08 — «${rm269.off || '—'}». Закрытый май, МВ-2026/19: заёмщика сняли — ${cl269.rows || '—'}, маркеров ${cl269.mk}; заёмщик на 21.08 — «${cl269.ask || '—'}», на 22.08 — «${cl269.now || '—'}»; вернули — ${back269.rows || '—'}, маркеров ${back269.mk}. Дат с другим числом представителей, чем один, — ${[add269.bad, rm269.bad, cl269.bad, back269.bad].map(a => (a || ['—']).length).join(' · ')}; число мер на дату (срез/различных) — ${[].concat(add269.cnt || [], rm269.cnt || [], cl269.cnt || []).join(' · ')}. Прогон за закрытое 15.06 с новой целью — ${once269.tally || '—'}: мера в одном решении ночи, а не в «не тронуто» и «без изменений» сразу. Мера закрытого мая МВ-2026/42, родившаяся ночью 22.08, получила заёмщика ночью 23.08 — ${late42.rows || '—'}: пара легла на дату строки меры, дат без представителя ${(late42.bad || ['—']).length}, число мер ${(late42.cnt || []).join(' · ')}`);
  ...
  /* #270 — у меры поправка — строка ПОЛНОГО состояния пары на срез исправления, не приращение:
     ...
     «не тронуто» на меру, без строк и маркеров (ИС-8).
