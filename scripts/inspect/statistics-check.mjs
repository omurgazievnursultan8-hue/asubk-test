...
// отбитый набор оставляет её открытой.
...
(() => {
  ...
  /* Правка ревью 2 З-16b — работа из очереди снимается только после двери. Перепривязка
     закрытого июня задним числом (с 10.06), распоряжение поставлено, и первым его видит прогон
     за закрытое 15.06, позванный мимо дверей: сторно и перепривязка на 15.06 отбиты, и
     распоряжение остаётся открытым. Ближайшая открытая ночь 22.08 без второго распоряжения
     кладёт сторно и перепривязку на свою дату. Прежде работа снималась до двери, и изменение
     терялось: работы нет, ночь запись не обходит. */
  ST.seed();
...
(() => {
  ...
  /* #270 — у меры поправка — строка ПОЛНОГО состояния пары на срез исправления, не приращение:
     ...
     ближайшая открытая ночь без второго распоряжения (правка ревью 2 З-16b).
