...
(() => {
  ...
  ok(1, st.objects.length === 10 && st.indicators.length === 229 && own1.length === 155 &&
       ...
       st.dims.length === 79 && ST.registry().length === 308 && badSrc.length === 0 &&
...
(() => {
  ...
  /* Волна 23 (переписан на месте, З-15b): ИС-12 для пропуска внутри истории заменён догоном
     ...
     `substituted`/`skipped` здесь не читаются — их снимает З-15c. */
...
(() => {
  ...
  const track = st.rows.filter(r => r.ref === 'ПП-2026/0620')
    .slice().sort((a, b) => a.date < b.date ? -1 : a.date > b.date ? 1 : 0).map(r => r.dims['d-rmatch']);
  ...
  ok(116, fz.dims['d-rmatch'] === 'расхождение' && fz.dims['d-rfrz'] === 'заморожено' &&
       hist.length === 2 && seq.join(' → ') === 'подтверждено → отозвано → восстановлено',
  ...
  ok(119, P.dims.length === 8 && iPay === 7 && aPay === 8 && somPay === 14 &&
...
(() => {
  ...
  ok(126, PR.dims.length === 9 && iP === 0 && aP === 1 && prRows.rows.length === 5 && bare &&
        ...
        M.dims.length === 7 && iM === 1 && aM === 2 && ST.DIM('d-mcur').obj === 'obj-measure' &&
...
(() => {
  ...
  ok(130, ds.length === 118 && dsRun.length === 112 && dsLeg.length === 6 &&
...
(() => {
  ...
  ok(202, bare202.by['критическая дата'].length === 0 && bare202.moved.length === 0 &&
        ...
        run202.written === flat202.written + 4 && flat202.written === 42 &&
...
(() => {
  ...
  ok(211, flat211.ok && flat211.dense === 19 && stMade211 === 0 && flat211.refreshed === 0 &&
  ...
  ok(212, base212.ok && base212.dense === 19 && base212.refreshed === 0 &&
        r212.ok && r212.dense === 19 && r212.refreshed === 4 && r212.fixed === 1276 &&
...
(() => {
  ...
  ok(226, ds.length === 118 && dsLeg.length === 6 && dsRun.length === 112 && gMayOwn === null &&
