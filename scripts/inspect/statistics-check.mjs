...
// (from, to] равно приросту «погашено всего» по каждому кредиту, поправки — в месяце исправления,
// пока после конца интервала ни одно его событие не переписано на месте (открытый вопрос
// ADR-0239 §3) · правка ревью 1: представитель у меры один на каждую дату и после того, как цели
...
(() => {
  ...
  ok(116, fz.dims['d-rmatch'] === 'расхождение' && fz.dims['d-rfrz'] === 'заморожено' &&
       hist.length === 2 && rows0620.length === 1 && rows0620[0].part === 'original' &&
       rows0620[0].date === '2026-06-11' && rows0620[0].dims['d-rmatch'] === 'восстановлено' &&
       rw0620.join() === '2026-06-23,2026-07-05',
    `три оси ТЗ 14 §3.1 не схлопнуты в один «статус»: ПП-2026/0844 одновременно «${fz.dims['d-rmatch']}» по сопоставлению и «${fz.dims['d-rfrz']}» по заморозке — одним полем это не выразить. Обе оси читаются ИЗ ИСТОРИИ (их ${hist.length}), а не из поля: сопоставление ПП-2026/0620 двигалось дважды в открытом июне, и строка у него одна (${rows0620.length}, на ${rows0620.length ? rows0620[0].date : '—'}) с нынешним «${rows0620.length ? rows0620[0].dims['d-rmatch'] : '—'}» — в открытом месяце событие переписывается на месте, а движения названы журналом перезаписей ночей ${rw0620.join(', ')} (ИС-10, ИС-55, ADR-0239 §3)`);
...
(() => {
  ...
  ok(172, a172.bad.length === 0 && a172.seen === 16109 && a172.som === 5075 && dates172.length === 59 && objs172.length === 10 &&
        own172.length === 2195 && leg172.length === 34 && legSom172 === 0,
...
(() => {
  ...
  ok(212, base212.ok && base212.made === 0 && !('dense' in base212) && base212.refreshed === 0 &&
        r212.ok && r212.made === 0 && r212.refreshed === 2 && r212.fixed === 52 &&
...
(() => {
  ...
  /* #272 — поток событиями: «погашено за период» — строки со срезом в (from, to], поправки
     ...
     ПОКА после конца интервала ни одно его событие не переписано на месте. Событие открытого
     месяца переписывается на месте, строки кредита на прошедшие даты — нет, и сторно, пришедшее
     позже, их разводит: открытый вопрос владельцу ADR-0239 §3 (правка ревью 1 З-16b, дверь
     `ST.eventFlow`). Сторож держит тождество там, где оно обещано, и не называет его безусловным.
     ...
     Правка ревью 1 З-16b: конец потока — дата прогона (ИС-36), те же ворота, что у потока
     итогов; начало — не раньше запуска, пока легаси-строк событий нет (СС-Д25). */
  ST.seed();
  const mix272 = ST.eventFlow ? ST.eventFlow('obj-repay', 'm-ramount', '2026-06-01', '2026-07-01') : {ok: true};
  const cur272 = ST.eventFlow ? ST.eventFlow('obj-repay', 'm-ramount', '2026-06-01', '2026-07-01', 'd-pcur') : {ok: false, by: {}};
  const msr272 = ST.eventFlow ? ST.eventFlow('obj-measure', 'm-mclaim', '2026-05-01', TODAY) : {ok: true};
  const ev272 = (a, b, dim) => ST.eventFlow ? ST.eventFlow('obj-repay', 'm-ramount', a, b, dim) : {ok: true};
  ...
  ok(272, flowJun.ok && badJun.length === 0 && flowJun.by['КД-2024/117'] === 28000 &&
        ...
        flowAug.by['КД-2024/117'] === 68000 && flowAug.by['КД-2025/088'] === 42500 && guard272 && gate272,
    `«погашено за период» строками событий равно приросту «погашено всего» по каждому кредиту — для интервала, после конца которого ни одно его событие не переписано на месте: июнь (01.06 → 01.07] — расхождений ${badJun.length} (${Object.keys(flowJun.by || {}).map(k => k + ' ' + flowJun.by[k]).join(' · ')}); август после перепривязки ПГ-2026/1102 — расхождений ${badAug.length}: КД-2024/117 ${flowAug.by['КД-2024/117']} (96 000 − 28 000 сторно), КД-2025/088 ${flowAug.by['КД-2025/088']} (14 500 + 28 000 перепривязки). Поправка события закрытого месяца входит в поток месяца исправления, закрытый месяц не переписан (ИС-17, ADR-0239 §6); сторно события ОТКРЫТОГО месяца, пришедшее после конца интервала, переписывает событие на месте, а строки кредита нет — и тождество расходится: открытый вопрос владельцу ADR-0239 §3. Июнь без разреза — отказ: «${String(mix272.why || '').slice(0, 72)}…»; по валюте — ${Object.keys(cur272.by || {}).map(k => k + ' ' + cur272.by[k]).join(' · ')}, сом одним числом ${cur272.som} (ADR-0214 §1). У меры — «${String(msr272.why || '').slice(0, 60)}…». Конец потока — дата прогона (ИС-36): (01.08, 31.08] — «${String(tail272.why || 'считается').slice(0, 64)}…», как у потока итогов (${fb272.ok ? 'считается' : 'отказ'}); 2025 год — ${y2025.ok ? 'считается' : 'отказ'}; начало 15.04 — ${pre272.ok ? 'считается' : 'отказ (СС-Д25)'}, с 01.05 — ${launch272.ok ? 'считается' : 'отказ'}`);
})();
