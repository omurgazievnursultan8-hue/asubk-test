...
(() => {
  ...
  let usd267 = {v: null, bad: ['не собрано']};
  ...
  const w268 = doRun268(ST.state, '2026-06-15', 'внеплановый', 'проба', 'проба отказа записи', null,
                        {full: true, why: 'проба отказа записи', keepT: true});
  const j268 = ST.state.runs[ST.state.runs.length - 1];
