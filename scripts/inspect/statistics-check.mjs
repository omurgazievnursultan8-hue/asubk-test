...
(() => {
  ...
  /* Волна 17 ч.8 добавила восьмое поле `srcs`, и «форма закрыта» перестала быть счётом
     полей: считать надо носителей ЗНАЧЕНИЙ, а их по-прежнему два. Ряды названы порознь,
     пересекаться не могут и вместе обязаны покрыть форму без остатка — иначе поле
     завелось бы вне обоих рядов и запрет ИС-15 его бы не касался. Различие рядов
     проверяется механически, а не на слово: ключи поля ряда значений — имена записей
     реестра, ключи поля ряда судьбы — не запись реестра ни один (ИС-42, ADR-0208 §2). */
  const key = ST.ROW_KEY, vals = ST.ROW_VALUES, fate = ST.ROW_FATE;
  const rows3f = key.concat(vals, fate);
  ...
  ok(3, shape.length === 8 && shape.indexOf('som') < 0 && shape.indexOf('доля') < 0 &&
       shape.join(',') === 'obj,ref,date,dims,inds,srcs,fixed,by' &&
       vals.length === 2 && both.length === 0 && cover &&
    ...
    `форма строки закрыта: ${shape.join(' · ')} — долей и дельт нет (ИС-15). Полей восемь, но носителей ЗНАЧЕНИЙ по-прежнему два: рядов три и вместе они покрывают форму без остатка, не пересекаясь ни одним полем: АДРЕС (${key.join(' · ')}) · ЗНАЧЕНИЯ (${vals.join(' · ')}) · СУДЬБА (${fate.join(' · ')}). Ряды различаются механически: ключей в ряду значений ${valKeys.length}, и каждый — запись реестра; ключей в ряду судьбы ${fateKeys.length} (${fateKeys.join(', ')}), и не запись реестра ни один — это имена СОСЕДЕЙ (ИС-42, ADR-0208 §2). Сомовая величина вошла в строку колонкой внутри inds под именем записи реестра «${ST.IND(som3).name}» (${(u3.inds[som3] || {}).v} ${ST.IND(som3).unit}), а не полем формы: поле нельзя назвать в отчёте и прекратить датой, запись — можно (ИС-44, ADR-0214 §2)`);
...
(() => {
  ...
  const would184 = plan184.reduce((n, r) => n + r.parts.reduce(
    (k, p) => k + p.n + (p.same || 0) + (p.have || 0) + (p.kept || 0), 0), 0);
  ...
  ok(188, bare188.ok && bp188.n === 0 && bp188.same === 5 &&
...
(() => {
  ...
  ok(199, l0.length === 0 && l2.length === 0 && l3.length === 1 &&
        k3.nb === 'взыскание' && k3.key === 'сосед-молчит/взыскание' && k3.runs === 3 &&
