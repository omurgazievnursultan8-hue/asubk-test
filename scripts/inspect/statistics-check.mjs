// Headless smoke для mockups/statistics/statistics.html (ИС-1…ИС-58, ADR-0145…0152 + 0176…0245).
...
// закрытом месяце — на ночь, когда платёж стал известен · правка события в закрытом месяце —
// поправки в месяце исправления и маркер «исправлено позже» (с З-16c так же и в открытом
// чужом месяце: на месте — только исправление в месяце строки, СС-180) · молчание соседа
// известное событие не трогает (СС-179), а строка, рождённая в молчание, неполна и зовёт
// пересчёт сама (очередь дозаполнения снята в З-22b) · в закрытое не пишет никто — одной
// функцией записи.
...
// кредита нет, охват путь не режет; путь соединяется во всех дверях — срез, ряд, строки,
// выгрузка, поток, реестр, — и одно значение даёт один ответ; датировка пути — у строки соседа.
...
// мера видна, если видна ЛЮБАЯ её цель на дату вопроса, а не только представитель.
...
// соседа ждёт; retro пишет журнал перезаписи с причиной «retro»; пересчёт берёт задачу по
// датам (`from < at`), а не по виду — ответ соседа, слитый в ручную задачу, пересчитывается.
...
(() => {
  ...
  const rows6 = each.filter(x => x.ok), mute6 = each.filter(x => !x.ok);
/* Волна 23, З-20: объектов одиннадцать — членство в группе (ADR-0244 §2).
     Волна 28 (переписан на месте): объектов шестнадцать, и пятнадцать из них отвечают
     СТРОКАМИ одним движком. Шестнадцатый — «Эпизод просрочки» — отвечает ОТКАЗОМ, и это не
     поломка движка, а объявленное состояние: перечня эпизодов владелец не отдаёт, вывести
     его самой витрине запрещено (ИС-1), и отказ называет заявку поимённо (ИС-24). */
  ok(6, rows6.every(x => x.n > 0) && each.length === 16 && rows6.length === 15 &&
        mute6.length === 1 && mute6[0].name === 'Эпизод просрочки' &&
        has(mute6[0].why, 'ЗАЯВКА ВЛАДЕЛЬЦУ') && has(mute6[0].why, 'P20-R33') &&
        has(mute6[0].why, 'overdueSpells') && has(mute6[0].why, 'ИС-1'),
    `шестнадцать объектов одним движком, пятнадцать из них — строками: ${rows6.map(x => x.name + ' ' + x.n + '/' + x.g + ' групп').join(' · ')}. Шестнадцатый молчит НЕ пустотой: «${mute6[0].name}» отвечает отказом — «${mute6[0].why.slice(0, 120)}…» (ИС-1, ИС-24, ADR-0201 §3)`);
...
/* Волна 23, З-20 (переписан на месте): у членства рождение объявлено — дата вступления «since». */
  ok(89, declared.length === 16 && tbl89.ok && !noBorn.ok && has(noBorn.why, 'ИС-33') && !guess,
...
/* Волна 23, З-20 (переписан на месте): объектов 11 — членство в группе добавлено релизом; снятое
  ...
  ok(121, st.objects.length === 16 && !gone.ok && has(gone.why, 'нет в реестре объектов') &&
...
/* #133 — снят волной 23 (З-19): «заёмщик режется своим разрезом d-lcurator» стоял на форме
   `scope.dim`; правило own держит тот же случай — #289 (ADR-0243 §1, §2). Номер не
   переиспользуется. */
...
/* #134 — снят волной 23 (З-19): «охват читается тем же читателем, что разрез; срез и реестр
   ...
   объектов, а не у 6: #289 (ADR-0243 §2, §3). Номер не переиспользуется. */
...
/* #136 — снят волной 23 (З-19): «ворота охвата в общей проверке вопроса отбивают все двери»
   ...
   владельца сходятся) (ADR-0243 §1, §5). Номер не переиспользуется. */
...
(() => {
  ...
  ok(150, cR.id !== bR.id && cR.name !== bR.name && cR.obj !== bR.obj && sameDict &&
        /* Волна 28 (переписан на месте): 4 → 5 записей — «Территория заявителя» у заявки на
           кредит. Дорога третья, справочник тот же, и расщепления иерархии по-прежнему нет. */
        geo.length === 5 && twoHier.length === 0 && cKeys === bKeys && !sameRoad(cR, bR),
...
(() => {
  ...
  /* Волна 23, З-20: +261 строка членства без валюты — сомовых клеток в них нет.
     Волна 28 (переписан на месте): объектов со строками 11 → 15 (эпизод просрочки строк не
     имеет вовсе), своих строк 2457 → 3373, клеток 16109 → 17583, итогов в сомах 5075 → 5233,
     остатков 9338 → 10654; курс лежит у 1272 строк — прибавились заявки и расчёты. */
  ok(172, a172.bad.length === 0 && a172.seen === 17583 && a172.som === 5233 && a172.flow === 1696 &&
        a172.bal === 10654 && a172.fxOn === 1272 && a172.fxOff === 2135 &&
        dates172.length === 59 && objs172.length === 15 &&
        own172.length === 3407 && leg172.length === 34 && legSom172 === 0 && leg172.every(r => r.fx === null),
  ...
  /* Волна 28 (переписан на месте): близнецов 74 → 88 — семь новых валютных величин и их своды. */
  ok(176, twins.length === 88 && sameName.length === 0 && dup.length === 0 &&
  ...
  /* Волна 28 (переписан на месте): 37 → 44 валютных строчных (две суммы заявки, четыре числа
     расчёта, пик эпизода) и 17 → 18 итогов в сомах (база среза заявки на реструктуризацию). */
  ok(179, moneyRows.length === 44 && badRoll.length === 0 && badBy.length === 0 &&
        somRows.length === 18 && badSom.length === 0 &&
  ...
  ok(180, noTwinRow.length === 0 && noTwinAgg.length === 0 && overTwin.length === 0 &&
        moneyAgg.length === 44 && somOnly180.length === 18 && twinnedSom.length === 0 &&
...
/* Волна 23, З-20 (переписан на месте): записей 316, разрезов 87, строчных с колонками 198 — четыре
  ...
  /* Волна 28 (переписан на месте): 316 → 397 записей, 87 → 122 разреза, 198 → 254 строчных
  ...
  ok(183, F183.indexOf('round') >= 0 && RU183.round === 'правило округления' &&
        money183.length === 150 && noRule183.length === 0 &&
        rules183.length === 1 && rules183[0] === RULE &&
        /* Волна 28 (переписан на месте): 128 → 150 денежных записей — запрошенная и одобренная
           суммы заявки, четыре числа расчёта реструктуризации и пик эпизода просрочки, каждая
           со своей сомовой стороной; объявляют правило 106, наследуют 44. */
        own183.length === 106 && inh183.length === 44 &&
...
(() => {
  ...
  /* Волна 28 (переписан на месте): соседей 5 → 8 — «Заявки» (appClock), «Реструктуризация»
     (rsClock) и «Сотрудники» (hrClock); швов реестр называет 17. Шов эпизодов просрочки
     (overdueSpells) в этот счёт НЕ входит: его у ядра ещё нет, и запись на него не ссылается
     ни одна — величины эпизода объявлены полями владельца (P20-R33). */
  ok(193, nbs.length === 8 && orphanSeam.length === 0 && twice.length === 0 &&
        notReal.length === 0 && usedSeams.length === 17 &&
  ...
  /* Неполная строка родившегося события сама зовёт запись в ночь, полная — перестаёт звать —
     тем же правилом, что у состояния (ADR-0208 §5, СС-179): иначе строка ждала бы, пока
     событие попадёт в кандидаты по другому поводу. Волна 23, З-22b (переписан на месте):
     очередь неполных — сами строки, задачи «дозаполнение» нет (ADR-0245 §4, СС-206); выборка
     — по строкам, а не по очереди. */
  const q198 = () => ST.state.rows.filter(r => ['ПГ-2026/1166', 'ПП-2026/0739'].indexOf(r.ref) >= 0)
      .map(r => ({ref: r.ref, done: ST.isPartial(r) ? null : 'строка написана полной'}));
  const was1141 = k1141();
...
(() => {
  ...
  ok(201, c201.ok && c201.full === false && c201.n === 38 && r201.cand.n === 38 &&
        /* Волна 23, З-20: +5 строк членства в ночь — соседи их не называют, и они копируются
           (ИС-54). Волна 28 (переписан на месте): +18 живых записей — 6 заявок, 3 обращения,
           4 расчёта и 5 сотрудников; эпизодов просрочки 0, их владелец перечня не отдаёт
           (P20-R33). Кандидатов 38, написано 65, скопировано 27, живых 99. */
        r201.written === 65 && r201.copied === 27 && r201.same === 0 && r201.skip === 34 &&
        r201.written + r201.skip === 99 &&
  ...
  ok(202, bare202.by['критическая дата'].length === 0 && bare202.moved.length === 0 &&
        c202.moved.join() === 'USD' && kd202.length === 3 && only202.length === 3 &&
        kd202.indexOf('obj-credit|КД-2025/043') >= 0 &&
        kd202.indexOf('obj-borrower|10510198203112') >= 0 &&
        /* Волна 28 (переписан на месте): третьей приходит заявка в долларах — З-2026-000140,
           поток B. Курс её запрашиваемой суммы сдвинулся, а владелец не назвал её ничем. */
        kd202.indexOf('obj-app|З-2026-000140') >= 0 &&
        ...
        /* Волна 23, З-20: 42 → 47 — пять строк членства (довод у #201).
           Волна 28 (переписан на месте): 47 → 65, объектов в множестве 2 → 3. */
        run202.written === flat202.written && flat202.written === 65 &&
        new Set(only202.map(k => k.split('|')[0])).size === 3,
  ...
  ok(206, bad206.every(r => !r.ok) && has(bad206[0].why, 'поводов очереди два') &&
        ...
        add206.ok && twice206.ok && ST.queueWhy().length === 2 && c206.n === 39 &&
        c206.by['очередь'].length === 1 && has(c206.by['очередь'][0].why, 'распоряжение') &&
        /* Волна 23, З-20: 42 → 47, копий 11 → 16 — довод у #201.
           Волна 28 (переписан на месте): 47 → 65, копий 16 → 26. */
        run206.written === 65 && run206.copied === 26 && run206.same === 1 && run206.skip === 34 &&
        open206 === 0 && all206 === 1 &&
...
(() => {
  ...
  ok(211, flat211.ok && !('dense' in flat211) && flat211.made === 0 && stMade211 === 0 &&
        ...
        /* Волна 28 (переписан на месте): переписанных 2 → 3 — к кредиту и его заёмщику
           прибавилась валютная заявка З-2026-000140 (USD): её сомовый близнец собран по
           тому же курсу. Проверяемое правило от этого не зависит — ни одна строка не
           переписана ПРАВКОЙ ОБЪЯВЛЕНИЯ, и только настоящая причина двигает строки. */
        rew211.ok && rew211.refreshed === 3 &&
  ...
  ok(212, base212.ok && base212.made === 0 && !('dense' in base212) && base212.refreshed === 0 &&
        ...
        /* Волна 28 (переписан на месте): переписано 2 → 3 (кредит, его заёмщик и валютная
           заявка З-2026-000140), зафиксировано 58 → 76 — в июльский итог вошли строки
           заявок, обращений, расчётов и сотрудников. */
        r212.ok && r212.made === 0 && r212.refreshed === 3 && r212.fixed === 76 &&
        j212.kind === 'защёлка' && j212.written === 3 &&
        j212.written === j212.repoll.again &&
        j212.repoll.made === 0 && j212.repoll.again === 3 && j212.repoll.done === true &&
        chg212.some(r => r.obj === 'obj-borrower') &&
        j212.cand.scan === 'полный' &&
        chg212.length === 3 && kd212 && chg212.some(r => r.obj === 'obj-app') &&
        cell212('m-total-som', was212) === 12920251.35 && cell212('m-total-som', kdw212.inds) === 13390613.18 &&
        j212.parts.filter(p => p.rewrote.length).length === 3,
  ...
  /* Волна 28 (переписан на месте): соседей 5 → 8, и слоя не имеют ДВОЕ — кураторство и
     кадры: кадровый факт учётным периодом не мерится, увольнение задним числом слоем не
     удостоверяется. Окончательных после простановки колонок 4 → 6. */
  ok(215, pre215.ok && pre215.final === 0 && pre215.layerless === 2 &&
        pre215.nbs.every(n => n.final === false) &&
        has(core215.why, 'не проставлена: величины предварительные') &&
        post215.final === 6 && post215.layerless === 2 &&
...
(() => {
  ...
  ok(228, ghosts.length === 0 && cand.ok &&
        /* Волна 23 (переписан на месте): прогон за 21.08 пишет 30 строк, а не 34 — довод
           ...
           (ИС-54, довод у #201). */
        /* Волна 23, З-20: 42 → 47 — пять строк членства (довод у #201).
           Волна 28 (переписан на месте): 47 → 65 — заявки, обращения, расчёты и сотрудники. */
        run228.ok && run228.written === 65 && run228.date === '2026-08-22' &&
...
(() => {
  ...
  /* Волна 28 (переписан на месте): таблиц строк 11 → 16, колонок 602 → 720 (§16–§20). */
  ok(235, names.length === 16 && total === 720 && !off.length && !dups.length && !noStore.length,
...
/* Волна 23, З-20 (переписан на месте): объектов-состояний 8, копий 17, написано 47 — членство
  ...
  /* Волна 28 (переписан на месте): объектов-состояний 8 → 13 (заявка, обращение, расчёт,
     сотрудник и эпизод просрочки — у последнего строк нет, и дыр он не даёт: живых записей
     у владельца ноль). Написано 65, скопировано 27. */
  ok(256, states256.length === 13 && days256.length >= 50 && holes256.length === 0 &&
        r256.ok && !c256.full && r256.copied === nonCand && r256.copied === 27 &&
        r256.written === 65 && r256.skip === 34 &&
...
/* Волна 23, З-20 (переписан на месте): 42 → 47 — пять строк членства (довод у #201). */
  ok(258, re258.ok && stW258 === 65 && re258.copied === 0 && j258.cand.scan === 'полный' &&
...
(() => {
  ...
  /* Волна 23, З-22b (переписан на месте): задач «дозаполнение» нет — очередь неполных — сами
     строки (ADR-0245 §4, СС-206), и молчание «кураторства» задач не ставит вовсе. Задания дней
     ставятся руками: выполненное — досчёт 20.07, который ночь 20.07 снимает; невыполненное —
     распоряжение, поставленное 31.07 после последней ночи июля. Открытая работа неполных
     считается по строкам. */
  const ref260 = ST.registryList('obj-borrower', '2026-07-20')[0];
  ST.state.today = '2026-07-20';
  ST.enqueue('obj-borrower', ref260, 'досчёт', 'задание дня 20.07');
  ...
  ST.state.today = '2026-07-31';
  ST.enqueue('obj-program', 'БК-2021', 'распоряжение', 'задание дня 31.07');
  ST.state.today = TODAY;
  ...
  const open260 = st260.queue.filter(q => !q.done).length;
  const partJul260 = () => st260.rows.filter(r => isState(r) && (inJul(r) || r.date === S260) && ST.isPartial(r)).length;
  const partBefore260 = partJul260();
  ...
  const openAfter = st260.queue.filter(q => !q.done);
  const partAfter260 = partJul260();
  const cal260 = ST.calendar().find(p => p.month === '2026-07') || {};
  ok(260, doneJul > 0 && open260 > 0 && inc260 > 0 && incS260 > 0 &&
        partBefore260 > 0 && partAfter260 === 0 &&
...
/* ===== Волна 23 · З-16a — событие дельтой (ИС-55, ADR-0239). Способ хранения — реквизит
   релиза; строка события ложится на срез дня, когда событие стало известно; правка на месте —
   только если исправление ложится в месяц строки (с З-16c, СС-180), иначе — поправки в месяце
   исправления и маркер «исправлено позже». В закрытое не пишет никто — у всех таблиц одной
   функцией. ===== */
(() => {
  const stor = id => ST.storageOf ? ST.storageOf(id) : null;
  const W = vm.runInContext('WORLD', sandbox);
...
/* Волна 23, З-20 (переписан на месте): таблиц 11 — stat_row_group_member хранит состояние. */
  /* Волна 28 (переписан на месте): таблиц 11 → 16; событий среди них по-прежнему три. */
  ok(264, ids264.length === 16 && off264.length === 0 && own264.length === 0 &&
...
/* Волна 23, З-20 (переписан на месте): таблиц 11 — запись в закрытое отбита и у членства. */
  /* Волна 28 (переписан на месте): дверь одна на 16 таблиц, а не на 11. */
  ok(268, !!wr && objs268.length === 16 && denied.length === 16 && pass268.ok && leg268.ok &&
...
(() => {
  ...
  /* #270 — у меры поправка — строка ПОЛНОГО состояния пары на срез исправления, не приращение:
     ...
     нет): исправление в месяце строки переписывает полную строку пары на месте (СС-180), и журнал называет
...
(() => {
  ...
  /* Волна 28 (переписан на месте): колонку курса релиз вывел ещё у трёх таблиц — заявка на
     кредит, расчёт реструктуризации и эпизод просрочки; у заявки на реструктуризацию её нет,
     и это не пропуск: её итог возникает СУММОЙ разновалютных баз и живёт только в сомах
     (ADR-0240 §4). */
  ok(280, fxObjs280.join() === 'obj-app,obj-claim,obj-credit,obj-measure,obj-odue,obj-receipt,obj-repay,obj-rscalc' &&
...
/* Волна 23, З-20 (переписан на месте): списков 20 — состояние группы d_group_state. */
  /* Волна 28 (переписан на месте): колонок вида «code» со списком 20 → 33 — статус и стадия
     заявки, поток, стадия и исход обращения, ступень и состояние ДС расчёта, три оси и вид
     назначения сотрудника, слой и состояние эпизода. */
  ok(282, shape282.length === 0 && nCk282 === 33 && bad282.length === 0 && legacy282 > 0 && corr282.length === 0 &&
...
(() => {
  ...
  /* Правка ревью этапа 3 (переписан на месте): путь — во ВСЕХ дверях, одно значение — один ответ
     (ИС-14, ИС-22, ADR-0241 §8). Фильтр «юридическое лицо» даёт тот же набор кредитов у среза,
     списка, выгрузки и реестра владельца; поток по нему не ноль, и потоки трёх значений пути
     складываются в поток без фильтра. Датировка пути — датировка строки заёмщика (ADR-0242 §4,
     ADR-0205 §2): «тип лица» у заёмщика собран текущим — у кредита он тоже текущий; сними имя
     из `now` строк заёмщика — и путь «на дату». */
  const ul287 = cmp287('юридическое лицо');
  const ulS287 = ST.statSlice(Object.assign(q287(ASK287), {dims:[], filter: ul287}));
  const ulSet287 = ulS287.ok ? ulS287.groups[0].refs.slice().sort().join() : '—';
  const ulRows287 = ST.statRows({obj:'obj-credit', date: ASK287, filter: ul287});
  const ulRowSet287 = ulRows287.ok ? ulRows287.rows.map(r => r.ref).sort().join() : '—';
  const ulJob287 = ST.exportJob({obj:'obj-credit', date: ASK287, filter: ul287});
  const ulReg287 = ST.registryList('obj-credit', ASK287, ul287).slice().sort().join();
  const flow287 = v => { const f = ST.flowBetween({obj:'obj-credit', inds:'m-repaid', from:'2026-08-01', to: ASK287,
    filter: v ? cmp287(v) : null}); return f.ok ? f.value : NaN; };
  const flUl287 = flow287('юридическое лицо'), flAll287 = flow287(null);
  const flSum287 = ['индивидуальный предприниматель', 'физическое лицо', 'юридическое лицо'].reduce((a, v) => a + flow287(v), 0);
  const dat0 = s0.ok ? s0.passport.dating : {now: []};
  const bNow287 = ST.state.rows.filter(r => r.obj === 'obj-borrower' && r.date === ASK287);
  const keepNow287 = bNow287.map(r => r.now);
  bNow287.forEach(r => { r.now = (r.now || []).filter(x => x !== 'd-ptype'); });
  const sAsof287 = ST.statSlice(q287(ASK287));
  bNow287.forEach((r, i) => { r.now = keepNow287[i]; });
  ...
  ok(287, s0.ok && byKey287(s0) === j0 && j0 === 'индивидуальный предприниматель:1, физическое лицо:3, юридическое лицо:4' &&
        ...
        ulSet287.split(',').length === 4 && ulRowSet287 === ulSet287 && ulJob287.ok && ulJob287.job.n === 4 &&
        ulReg287 === ulSet287 && flUl287 > 0 && Math.abs(flSum287 - flAll287) < 0.005 &&
        dat0.word === 'текущее' && dat0.now.indexOf('d-cbptype') >= 0 && bNow287.length > 0 &&
        sAsof287.ok && sAsof287.passport.dating.word === 'на дату' &&
...
(() => {
  ...
  ok(288, errs288.length === 0 && st288.objects.every(o => Array.isArray(o.scope) && o.scope.length >= 1) &&
        /* Волна 28 (переписан на месте): заявка режется своим специалистом, обращение — своим
           куратором, расчёт — через заявку, эпизод — через кредит, а реестр кадров общий:
           список сотрудников есть рабочий экран поиска (ТЗ 11 §4.2). */
        (kinds288.own || []).join() === 'obj-credit,obj-borrower,obj-collateral,obj-claim,obj-repay,obj-measure,obj-app,obj-rsapp' &&
        /* Волна 23, З-20: членство режется через строку заёмщика. */
        (kinds288.via || []).join() === 'obj-gmember,obj-case,obj-measure,obj-rscalc,obj-odue' &&
        (kinds288.open || []).join() === 'obj-zdeal,obj-program,obj-receipt,obj-staff' &&
  ...
  ok(289, bAll.length === 8 && bRows.ok && bRows.rows.length === 3 && bReg.length === 3 &&
        ...
        /* Волна 23, З-20: пар 8 — членство через заёмщика.
           Волна 28 (переписан на месте): 8 → 12 — заявка, обращение, расчёт и эпизод
           просрочки; у эпизода реестр владельца пуст, и срез пуст вместе с ним. */
        pairs289.length === 12 && drift289.length === 0,
  ...
  /* Правка ревью этапа 3 (#291 переписан на месте): охват меры — по ВСЕМ её парам на дату
     вопроса, а не по цели представителя (ADR-0243 §3, схема §11.1, ИС-14). Осмонов Т. на 21.08
     ведёт ТВ-2026/03-2 и ТВ-2025/11-2 — цели МВ-2026/19 и МВ-2026/31, но не представителей:
     срез мер и реестр владельца видят обе меры. */
  ST.seed();
  ST.state.scope['Аналитик'] = 'Осмонов Т.';
  ST.setRole('Аналитик');
  const os291 = ST.statSlice({obj:'obj-measure', dims:[], inds:['a-count'], date: ASK});
  const osReg291 = ST.registryList('obj-measure', ASK).slice().sort().join();
  const osRep291 = ST.measureAt(ASK).filter(r => ['МВ-2026/19', 'МВ-2026/31'].indexOf(r.ref) >= 0 &&
    r.dims['d-mprimary'] === true).map(r => r.part).sort().join();
  ST.setRole('Администратор статистики');
  ST.seed();
  ...
  ok(291, m290a.ok && m290a.n === 2 && grp(m290a) === 'исковое заявление:1, претензия:1' &&
        ...
        os291.ok && os291.n === 2 && refs(os291) === 'МВ-2026/19+МВ-2026/31' &&
        osReg291 === 'МВ-2026/19,МВ-2026/31' && osRep291 === 'ТВ-2025/11-1,ТВ-2026/03-1',
    `мера — через требование-цель ИЛИ автору (ADR-0243 §3): до передач аналитику видно ${m290a.ok ? m290a.n : '—'} (${grp(m290a)}), после — ${m291 && m291.ok ? m291.n : '—'} (${grp(m291)}): меры по ТВ-2025/11-1 пришли через цель, меры Бековой по ТВ-2026/03-1 остались за ней как за автором, а чужая претензия МВ-2026/27 не видна. Группы складываются в число видимых строк — охват резал до группировки. Паспорт: «${m291 && m291.ok ? m291.passport.scope : '—'}»; реестр владельца — ${mw291 && mw291.ok ? mw291.n : '—'}. Путь признаков охватом не режется — #287. Охват меры — по всем её парам: Осмонову на ${ASK} видно ${os291 && os291.ok ? os291.n : '—'} (${refs(os291)}) через цели, которые не представители (представители — ${osRep291}), реестр владельца — ${osReg291} (ИС-58, ADR-0241 §8, СС-199)`);
})();
...
(() => {
  ...
  ok(294, noArr294 === 0 && ST.ROW_ORIGIN.join() === 'now' && ST.ROW_SHAPE.indexOf('when') < 0 &&
        ...
        /* Волна 28 (переписан на месте): своих строк 2457 → 3407, с «текущими» 2331 → 3281. */
        full294 === 3281 && own294.length === 3407 &&
        ...
        run294a.ok && copied294 === 27 && copies294.length === 15 && carried294 === 15 &&
        run294b.ok && blank294.length === 30 &&
        JSON.stringify(blankBy294) === JSON.stringify({'obj-borrower': 2, 'obj-gmember': 5, 'obj-zdeal': 5, 'obj-case': 3, 'obj-program': 5, 'obj-app': 4, 'obj-rsapp': 1, 'obj-rscalc': 4, 'obj-staff': 1}) &&
        fresh294 &&
...
(() => {
  ...
  try {
    ...
    /* «БК-2021» к 22.08 уже «закрыта» (#206): смена — обратно в «действует». */
    p.h.pstate = p.h.pstate.concat([['2026-08-21', 'действует']]);
...
(() => {
  ...
  /* Правка ревью этапа 3: ручная задача и ответ соседа на одной записи. Задача одна (ADR-0245
     §4), вид — у первой, «manual», а `from` — ранняя, из ответа соседа; пересчёт берёт задачу
     по датам (`from < at`), а не по виду, и строки с 11.08 переписаны. Бери он только задачи
     «retro» — ночь закрыла бы слитую задачу «обойдён прогоном», не пересчитав ни одной её
     даты. Ручная задача одна (`from` = `at`) по-прежнему отвечается ночью (#270, #276, #278). */
  ST.seed();
  let m299 = {};
  try {
    W299['obj-credit'][iC299].h.curator = W299['obj-credit'][iC299].h.curator.concat([['2026-08-10', 'Касымов Т.']]);
    const e = ST.enqueue('obj-credit', 'КД-2024/117', 'распоряжение', 'пересчитать по акту');
    const f = ST.nbChanged('кураторство', 'obj-credit', 'КД-2024/117', '2026-08-10');
    const run = ST.run(TODAY);
    const q = ST.queue(true).filter(x => x.obj === 'obj-credit' && x.ref === 'КД-2024/117');
    m299 = {ok: !!(e.ok && f.ok && run.ok), retro: run.retro,
      task: q.map(x => x.kind + ':' + x.from + ':' + x.whys.join('+') + ':' + (x.done ? 'done' : 'open')).join(),
      d11: cur299('2026-08-11'), d21: cur299('2026-08-21'),
      log: ST.rewriteLog().filter(x => x.reason === 'retro' && x.obj === 'obj-credit' && x.ref === 'КД-2024/117').length};
  } finally { W299['obj-credit'][iC299] = JSON.parse(keep299); }
  ST.seed();
...
(() => {
  ...
  ST.state.today = TODAY;
  /* Дату к выполнению удаляет закрытие месяца (ADR-0245 §11): задание на 15.07 оформлено, июль
     закрыт — день удалён. Прямое удаление строк дату из журнала прогонов не убирает, и
     `dateGate` его не видит: выгрузка ответила бы пустым файлом (находка З-23b). */
  const k302 = ST.exportJob({obj: 'obj-program', date: '2026-07-15'}).job;
  ST.closeLayer('2026-07', 'учёт', 'Осмонова Г., главный бухгалтер', '2026-08-05');
  ST.closeLayer('2026-07', 'классификация', 'Турдубаева А., администратор классификации', '2026-08-07');
  const shut302 = ST.closePeriod('2026-07', 'Мамбетов Э., администратор статистики');
...
const stamp = `SMOKE 2026-09-22 · ${pass}/${results.length} PASS\n` + lines.join('\n');
console.log(stamp);

const body = lines.map(l => '  ' + l).join('\n');
const injected = `  SMOKE 2026-09-22 · ${pass}/${results.length} PASS\n` + body;
