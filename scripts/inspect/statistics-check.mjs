...
(() => {
  ...
  ok(172, a172.bad.length === 0 && a172.seen === 16204 && a172.som === 5075 && dates172.length === 60 && objs172.length === 10 &&
        own172.length === 2214 && leg172.length === 34 && legSom172 === 0,
...
(() => {
  ...
  ST.seed();
  const ev198run = ST.run('2026-07-20', {silent:{'погашения':'недоступен'}});
  ST.closeLayer('2026-07', 'учёт', 'Осмонова Г., главный бухгалтер', '2026-08-05');
  ST.closeLayer('2026-07', 'классификация', 'Турдубаева А., администратор классификации', '2026-08-07');
  const ev198 = ST.closePeriod('2026-07', 'Осмонова Г., главный бухгалтер');
  const evB198 = (ev198.blockers || []).filter(b => b.part === 'события');
  const evFix198 = ST.run('2026-07-20', {});
  const evYes198 = ST.closePeriod('2026-07', 'Осмонова Г., главный бухгалтер');
...
(() => {
  ...
  ok(212, base212.ok && base212.made === 0 && !('dense' in base212) && base212.refreshed === 0 &&
        r212.ok && r212.made === 0 && r212.refreshed === 2 && r212.fixed === 57 &&
