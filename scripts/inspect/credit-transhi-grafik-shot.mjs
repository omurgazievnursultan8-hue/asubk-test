...
// производные транши с происхождением, остаток тела по ИР-3 (теперь на «Расчётах»),
// статьи в графике, сверка ИР-2′, кнопка «Применить ДС».
...
/* КВ-36: тело переехало на «Расчёты» — сторожим второй берег переезда прямо здесь, на
   кредите с переносами. Четыре слагаемых ИР-3 обязаны стоять колонками, а не тултипом. */
await openTab('K-7', 'Расчёты');
t = await text();
say('K-7 расчёты · таблица тела по траншам', /Тело по траншам/.test(t) && /Остаток тела/.test(t));
say('K-7 расчёты · слагаемые ИР-3 колонками', /Перенесено/.test(t) && /Принято/.test(t));
say('K-7 расчёты · свод называет свою область', /Свод по кредиту на/.test(t));
await shot('k7-raschety-telo');
