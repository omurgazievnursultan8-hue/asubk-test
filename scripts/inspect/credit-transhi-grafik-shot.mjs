// «Транши» и «График» под реструктуризацию (КВ-26, волна 12.08.2026).
// K-1 — обычный кредит: колонок статей нет, секции «Движение по траншу» нет — признак того,
// что пустая колонка не рисуется. K-7 — два применённых ДС и третье в хвосте «Условий»:
// производные траншы с происхождением, остаток тела по ИР-3, статьи в графике, сверка ИР-2′,
// кнопка «Применить ДС».
...
say('K-7 транши · «Доступно к распределению» не в минусе', !/−\s*\d|-\d[\d\s]*,\d\d\s*$/m.test((t.match(/Доступно к распределению[^\n]*\n[^\n]*/)||[''])[0]));
