...
// Блок АА (#117…#132) закрывает волну 18 — разбор 21.09.2026 (ADR-0248…ADR-0254): подпись
// обзора перестала стоять на живых строках (ИА-30) и требует покрытия охвата (ИА-31),
// подтверждённой неполноты (ИА-32) и НАЗВАННОГО адресата (ИА-33); числа под подписью
// приходят от ВЫПУСКА швом repIssueValues и стоят на экране рядом с живыми (ИА-35, ADR-0248);
// замена обзора НАЗЫВАЕТСЯ человеком и ставит пару ссылок (ИА-34); суждение правит автор, а
// при неактивном авторе — заведующий отделом с отметкой «правка не автором» (ADR-0254);
// реестр поручений уехал к соседу «задания» (ADR-0250), и наружу открылась ЧЕТВЁРТАЯ дверь —
// analysisLeads: два вида повода, полное множество, устойчивый ключ и названная база отсчёта
// срока (ADR-0251, ADR-0229).
...
/* Волна 18: подпись обзора стоит ТОЛЬКО на закрытом учётном периоде (ИА-30), и обзоры,
   которые прогон подписывает, переехали на закрытый I квартал. Открытое полугодие
   осталось там, где оно и нужно, — под черновиками и под отказом подписи. */
const CLOSED = '2026-03-31';
...
(() => {
  ...
  ok(28, seams.length === 4 &&
    ...
    `отдаёт не суждение и не факт, а СОСТОЯНИЕ — «вот это сейчас требует задания» (ADR-0251)`);
...
(() => {
  ...
  ok(58, rw.ok && rw.rows.length === 7 && rw.rows.every(r => r.fixation) &&
        ...
        /* Волна 18: суженной роли срез СЧИТАЕТСЯ по видимым ей строкам, а не отказывается.
           Роль применена ДО группировки (ADR-0152 §3), итог — по видимому множеству, и
           сказано это и в паспорте (режим «срез в вашем охвате»), и словами. */
    ...
    `«${mine.passport.scope.join(', ')}». Срез ему теперь СЧИТАЕТСЯ, а не отказывается: ` +
    `${rub(slCur.total)} по ${slCur.nodes.length} узлам его охвата, паспорт «${slCur.passport.short}», ` +
    `и правило названо словами — «${slCur.why.slice(0, 72)}…». Прежнее «агрегат ролью не ` +
    `сужается» снято волной 18: правило владельца строк (ADR-0152 §3) старше нашего (ADR-0249)`);
})();
...
(() => {
  ...
  ok(63, nSeams === 3 && seams.every(s => s.owner && s.q) && a1 - a0 === nSeams && a2 - a1 === nSeams &&
        ...
        /* Волна 18: срез суженной роли СЧИТАЕТСЯ (ADR-0152 §3), а отказывает РЯД — и
           отказывает не правилом, а тем, чего у двери нет: точками ряда лежат готовые
           снимки по всему портфелю, строк за прошлые даты в ней не хранится. */
        ni.slice.ok && ni.slice.data.total !== null && ni.slice.passport.narrowed === true &&
        !ni.series.ok && ni.series.kind === 'отказ соседа' &&
        has(ni.series.why, 'готовые снимки по всему портфелю') &&
        ni.rows.ok && ni.rows.data.n === 4 && ni.rows.passport.narrowed === true &&
        narrow.refused.length === 1 && narrow.answered === 2 && narrow.asks === 3 &&
        !wide.ok && wide.kind === 'отказ по форме',
    `счётчик обращений у статистики: ${a0} → ${a1} → ${a2} (${nSeams} шва на показатель) — второй раз ` +
    `сосед спрошен ЗАНОВО, ответ не переиспользован. Под ролью «${CURATOR}» срез ПОСЧИТАН в её ` +
    `охвате и несёт это в паспорте («${ni.slice.passport.short}»), строки сузились до ` +
    `${ni.rows.data.n} из 14, а ряд отказал — и отказал СВОИМ словом, не правилом: ` +
    `«${ni.series.why.slice(0, 64)}…». Отказавших швов ${narrow.refused.length} из ${narrow.asks}, ` +
...
(() => {
  ...
  AN.seed();
  AN.setRole(ANALYST);
  const mk = AN.newReview({ tpl: 'ШО-09', asOf: CLOSED, period: Q1, by: 'подразделение',
    cuts: { 'отрасль': 'переработка' }, inds: ['s-over'] });
  ...
  const ap = AN.approveReview(mk.no, { to: 'Администрирование кредитов' });
  ...
  /* Обзор без единой ответившей величины. Он остаётся на ОТКРЫТОМ полугодии намеренно:
     показатель s-over объявлен редакцией 2 шаблона ШО-12, а она действует с 01.04.2026 —
     на закрытом квартале этого обзора не существует вовсе. Проверяется здесь порядок
     отказов: «утверждать нечего» стоит РАНЬШЕ «период не закрыт», потому что документ,
     под которым не встала ни одна величина, не спасает и закрытый период. */
  const em = AN.newReview({ tpl: 'ШО-12', asOf: '2026-06-30', period: H1, by: 'область',
    inds: ['s-over'] });
  ...
  const emptyReal = AN.approveReview(em.no, { to: 'Отраслевой департамент' });
  ...
  const pt = AN.newReview({ tpl: 'ШО-04', asOf: CLOSED, period: Q1, by: 'область',
    inds: ['s-port', 's-over'] });
  ...
  const part = AN.approveReview(pt.no, { to: 'Отраслевой департамент', ack: true });
...
(() => {
  ...
  ok(69,
      sr.ok && t.ok && t.task.by === 'Тентимишев К.' && t.task.basis.ref === 'ОБ-1' &&
      t.task.no.indexOf('ЗД-') === 0 && lst.seam === 'zdByBasis' && lst.owner === 'задания' &&
  ...
  AN.setRole(HEAD);
  /* Считается число заданий ВОПРОСОМ у соседа: своей таблицы поручений в состоянии
     анализа больше нет вовсе — заглушка снята волной 18 (ADR-0250), и проверяется это
     здесь же, перечнем полей состояния. */
  const nBefore = AN.tasksOfReview('ОБ-1').n;
  const mass = AN.tryMassTasks('ОБ-1');
  const nAfter = AN.tasksOfReview('ОБ-1').n;
  ...
  ok(70,
    ...
    `руководителю, у которого право поручать есть: «${mass.why.slice(0, 210)}…». Заданий по ` +
    `ОБ-1 было ${nBefore}, после попытки ${nAfter} (ответ пришёл швом ${still.seam} у соседа ` +
    `«${still.owner}») — рабочий список из 14 объектов ни одного поручения не породил. Своего ` +
    `реестра поручений в состоянии анализа НЕТ ни полем: ${noOwnRegistry ? 'поля tasks не ' +
    'существует' : 'поле tasks осталось'} (ADR-0250 §1)`);
})();
...
(() => {
  ...
  ok(71, errZero === null && errFull === null &&
        has(full, 'ОБ-1 · Обзор портфеля') && has(full, '«ШО-04», редакция 1') &&
        has(full, '01.01.2026 — 31.03.2026') && has(full, 'Дата среза') &&
        has(full, '<span class="pill info">область</span>') &&
        has(full, 'Портфель за I квартал вырос') &&
    ...
    `Пометка источника и ноль поручений напечатаны СЛОВАМИ, а не пустым местом. Рядом с подписью ` +
    `стоят её реквизиты: адресат «Отраслевой департамент» и отметка «подписано при … неответивших» ` +
    `(ИА-32, ИА-33). На пустом журнале экран тоже рисуется (${zero.length} знаков) и говорит: ` +
    `«Обзоров в журнале нет — ни одного, и сказано это словами… журнал обзоров пуст, потому что ` +
    `обзоров ещё не заводили» (ИА-14)`);
  ...
  ok(72, d1.asks === 6 && d1.answered === 2 && overLimit.length === 2 && seamFail.length === 2 &&
        ...
        has(full, 'снимка «s-over» в разрезе «область» на 31.03.2026 у статистики нет') &&
        has(full, 'статистика · на 31.03.2026 · снимок на дату · охват: весь портфель') &&
        has(full, 'статистика · на 31.03.2026 · ряд снимков · охват: весь портфель'),
    ...
    `«s-over» в разрезе «область» на 31.03.2026 у статистики нет: срез считается её прогоном…»). ` +
    `Рядом с ответившими числами стоит краткий паспорт производителя: «статистика · на 31.03.2026 ` +
  ...
  AN.render();
  const after = JSON.stringify(AN.state);
  const leaked = ['47,5', '+0,01', '+7,6', 'п.п.'].filter(x => before.includes(x));
  ...
  ok(73, before === after && leaked.length === 0 && mk.ok &&
        has(full, '47,5 %') && has(full, 'Доля, % целого') &&
        has(full, 'Изменение доли, п.п.') && has(full, '+0,01 п.п.') &&
    ...
    `«47,5», ни «+0,01», ни «+7,6», ни «п.п.» в нём не встречается ни разу. Сумма узлов ` +
    `1 186 600 000,00 показана рядом с нераспределённым остатком, а не вместо итога. На обзоре ` +
...
(() => {
  ...
  AN.seedReviews();
  /* Открытый период с волны 18 живёт под ЧЕРНОВИКОМ: подписанный обзор на незакрытых
     строках стоять не может (ИА-30), и витрина это разводит — утверждённые на закрытом
     квартале, черновик ОБ-3 на открытом полугодии. Баннер при этом тот же самый.     */
  const draft78 = AN.state.reviews.filter(r => r.state === 'черновик')[0];
  /* Черновик виден тому, кто его ведёт (ИА-24), — роль ставится ЯВНО, а не достаётся от
     предыдущего блока: иначе проверка баннера упёрлась бы в границу доступа. */
  AN.setRole(ANALYST);
  AN.state.review = draft78.no; AN.go('review');   /* ШО-12, asOf 30.06.2026 — период открыт */
  ...
  ok(78, has(openReview, 'banner warn') && has(openReview, 'предварительные данные') &&
    ...
    `обзор ${draft78.no} (срез 30.06.2026, период открыт) несёт тот же баннер, что и финанализ: ` +
    `«предварительные данные»/«период не закрыт» — закрытость периода спрашивает один общий ` +
    `справочник (ADR-0204), а не свой признак у каждой формы (ИА-21). Черновик он не случайно: ` +
    `на открытом периоде подпись волной 18 запрещена, а живые числа показываются и правлению к ` +
    `заседанию уходят — с плашкой, а не с подписью под бегущим числом (ИА-30)`);
...
(() => {
  ...
  ok(109, has(b5, 'Запросы финансового пакета') && has(b5, '<th>Установленная дата</th>') &&
    ...
    `площадке швов их четыре, и рядом с ними стоит несуществующий «docsEvading»: спросив его, ` +
...
/* ============ БЛОК АА. ВОЛНА 18 — ПОДПИСЬ, ЗАДАНИЯ И ПОВОДЫ =================
   Семь решений разбора 21.09.2026 (ADR-0248…ADR-0254). Проверяется здесь не «появились
   поля», а поведение: подпись перестала стоять на живых строках, числа под подписью
   пришли от ВЫПУСКА и стоят рядом с живыми, у документа появился НАЗВАННЫЙ адресат,
   неполнота подтверждается вслух, замена НАЗЫВАЕТСЯ человеком, реестр поручений уехал
   к соседу, а наружу открылась четвёртая дверь — ПОВОДЫ для заданий.              */
(() => {
  const ANALYST = 'Сотрудник отдела анализа';
  const SECOND  = 'Сотрудник отдела анализа (Сатыбалдиев Н.)';
  const HEAD    = 'Заведующий отделом анализа';
  const CHIEF   = 'Руководитель подразделения';
  const CURATOR = 'Ведущий куратор (Асанов А.)';
  const REP = win.REP;
  const rub = n => Number(n).toLocaleString('ru-RU');
  const mkQ1 = (tpl, by, cuts, inds) => AN.newReview({ tpl, asOf: CLOSED, period: Q1, by,
    cuts: cuts || {}, inds });

  /* --- #117. Подпись стоит только на ЗАКРЫТОМ периоде --------------------------- */
  AN.seed();
  AN.setRole(ANALYST);
  const open = AN.newReview({ tpl: 'ШО-09', asOf: '2026-06-30', period: H1,
    by: 'подразделение', cuts: { 'отрасль': 'переработка' }, inds: ['s-over'] });
  AN.setReviewText(open.no, 'Просрочка по переработке выросла за полугодие.');
  AN.setReviewVerdict(open.no, 'требует внимания');
  const dOpen = AN.reviewData(open.no);
  const noClose = AN.approveReview(open.no, { to: 'Администрирование кредитов' });
  const closed = mkQ1('ШО-09', 'подразделение', { 'отрасль': 'переработка' }, ['s-over']);
  AN.setReviewText(closed.no, 'Просрочка по переработке сосредоточена в двух подразделениях.');
  AN.setReviewVerdict(closed.no, 'требует внимания');
  ...
  ok(117,
    ...
    `подпись на живых строках больше не ставится: обзор ${open.no} (срез 30.06.2026, период не ` +
    `закрыт) подписать нельзя — «${noClose.why.slice(0, 140)}…», документ остался черновиком и ` +
    `выпуска не случилось. Работать он при этом работает: ответило ${dOpen.answered} из ` +
    `${dOpen.asks} обращений, живые числа показываются с плашкой «период не закрыт» — правлению ` +
    `к заседанию уходит черновик, а не подпись под бегущим числом. Тот же обзор на закрытом ` +
    `квартале (${closed.no}, 31.03.2026) подписан: выпуск ${okClose.issue} (ИА-30, ADR-0249 §2)`);

  /* --- #118. Числа под подписью лежат У ВЫПУСКА, а не в обзоре ------------------ */
  AN.seed();
  AN.setRole(ANALYST);
  const fr = mkQ1('ШО-04', 'область', {}, ['s-port', 's-over']);
  AN.setReviewText(fr.no, 'Портфель за I квартал вырос по всем четырём областям.');
  AN.setReviewVerdict(fr.no, 'требует внимания');
  const dLive = AN.reviewData(fr.no);
  const apFr = AN.approveReview(fr.no, { to: 'Отраслевой департамент', ack: true });
  const iv = REP.issueValues(apFr.issue);
  const dFr = AN.reviewData(fr.no);
  const port = dFr.ok ? dFr.inds.find(x => x.id === 's-port') : {};
  const over = dFr.ok ? dFr.inds.find(x => x.id === 's-over') : {};
  const recJson = JSON.stringify(AN.state.reviews);
  const frozenHidden = recJson.indexOf(String(iv.values[0].total)) < 0;
  ok(118,
      apFr.ok && apFr.frozen === 1 && iv.ok && iv.values.length === 1 &&
      iv.values[0].ind === 's-port' && iv.values[0].total === 1203500000 &&
      iv.issue === apFr.issue && iv.asOf === CLOSED && !!iv.passport &&
      port.frozen && port.frozen.ok === true && port.frozen.total === 1203500000 &&
      port.slice.ok && port.slice.data.total === 1203500000 &&
      over.frozen && over.frozen.ok === false && has(over.frozen.why, 'не заморозил') &&
      frozenHidden && dLive.ok && dLive.inds.every(x => x.frozen === null),
    `числа под подписью лежат У ВЫПУСКА: окончательный выпуск ${apFr.issue} заморозил ` +
    `${apFr.frozen} величину (s-port, ${rub(iv.values[0].total)}) и отдаёт её швом ` +
    `repIssueValues с паспортом «${iv.passport.short}». В записи обзора этого числа нет ни ` +
    `строкой (искали в ${recJson.length} знаках журнала). Показатель, по которому шов в день ` +
    `подписи не ответил, остался БЕЗ числа под подписью, и сказано это словами: ` +
    `«${over.frozen.why.slice(0, 72)}…» — пустое место честнее подставленного нуля. У черновика ` +
    `замороженных чисел нет вовсе (ADR-0248, ИА-20)`);

  /* --- #119. Экран: два столбца — под подписью и живое рядом -------------------- */
  const el119 = () => ({ innerHTML: '', textContent: '', dataset: {}, value: '',
    classList: { toggle() {}, add() {}, remove() {} }, appendChild() {}, remove() {} });
  const n119 = { '#panel': el119(), '#title': el119(), '#foot': el119(), '#role': el119(),
    '#subj': el119() };
  sandbox.document = { querySelector: k => n119[k] || el119(), querySelectorAll: () => [],
    getElementById: () => null, createElement: () => el119() };
  AN.seed();
  AN.seedReviews();
  AN.setRole(ANALYST);
  AN.state.review = 'ОБ-1'; AN.go('review');
  const scr = n119['#panel'].innerHTML;
  AN.state.review = 'ОБ-3'; AN.go('review');
  ...
  ok(119,
    ...
    `на лице подписанного документа стоят ДВА числа: «Под подписью» (от выпуска) и «Живое ` +
    `сейчас» (ответ статистики на сегодняшний вопрос), и между ними напечатано расхождение — ` +
    `сегодня «числа совпали». Показатель, который выпуск не заморозил, честно говорит «числа ` +
    `под подписью нет». У ЧЕРНОВИКА первого столбца нет вовсе: морозить было нечему, и пустой ` +
    `колонки на его месте тоже нет (ADR-0248, ИА-35)`);

  /* --- #120. Адресат обязателен и берётся из контура шаблона -------------------- */
  AN.seed();
  AN.setRole(ANALYST);
  const ad = mkQ1('ШО-09', 'подразделение', { 'отрасль': 'переработка' }, ['s-over']);
  AN.setReviewText(ad.no, 'Просрочка по переработке сосредоточена в двух подразделениях.');
  AN.setReviewVerdict(ad.no, 'требует внимания');
  const noTo = AN.approveReview(ad.no, {});
  const alienTo = AN.approveReview(ad.no, { to: 'Представительство в г. Ош' });
  const withTo = AN.approveReview(ad.no, { to: 'Администрирование кредитов' });
  ...
  ok(120,
      ...
      withTo.ok && withTo.to === 'Администрирование кредитов' &&
      AN.REVIEW(ad.no).to === 'Администрирование кредитов',
    `окончательный выпуск без получателя не бывает: подпись без адресата отказана — ` +
    `«${noTo.why.slice(0, 116)}…», и отказ сам называет контур шаблона. Адресат ВНЕ контура ` +
    `отказан ДРУГИМИ словами: «${alienTo.why.slice(0, 96)}…» — направить суждение туда, где ` +
    `его не открыть, нельзя. С названным адресатом подпись прошла, и «${withTo.to}» лёг в ` +
    `запись (ИА-33, ИА-24, ADR-0252)`);

  /* --- #121. Неполнота подтверждается вслух и числом ---------------------------- */
  AN.seed();
  AN.setRole(ANALYST);
  const pa = mkQ1('ШО-04', 'область', {}, ['s-port', 's-over']);
  AN.setReviewText(pa.no, 'Портфель за I квартал вырос по всем четырём областям.');
  AN.setReviewVerdict(pa.no, 'требует внимания');
  const dPa = AN.reviewData(pa.no);
  const noAck = AN.approveReview(pa.no, { to: 'Отраслевой департамент' });
  const withAck = AN.approveReview(pa.no, { to: 'Отраслевой департамент', ack: true });
  const recPa = AN.REVIEW(pa.no);
  /* Полный ответ подтверждать нечего: отметки не появляется, и её отсутствие — не пробел. */
  const fu = mkQ1('ШО-09', 'подразделение', { 'отрасль': 'переработка' }, ['s-over']);
  AN.setReviewText(fu.no, 'Просрочка по переработке сосредоточена в двух подразделениях.');
  AN.setReviewVerdict(fu.no, 'требует внимания');
  const dFu = AN.reviewData(fu.no);
  const apFu = AN.approveReview(fu.no, { to: 'Администрирование кредитов' });
  ok(121,
    ...
    `неполнота больше не проходит молча: обзор с ${dPa.answered} ответившими из ${dPa.asks} без ` +
    `подтверждения не подписывается — «${noAck.why.slice(0, 118)}…». Это НЕ запрет: назвали ack — ` +
    `подпись прошла, и рядом с ней навсегда встала строка «подписано при ${recPa.ack.missing} ` +
    `неответивших из ${recPa.ack.of}» (${recPa.ack.by}, ${recPa.ack.at}). Жёсткого порога полноты ` +
    `нет и не будет: сосед законно отказывает на ряд, которого у него нет. Полностью отвеченный ` +
    `обзор (${dFu.answered} из ${dFu.asks}) подписан без всякой отметки — подтверждать нечего, и ` +
    `пустое поле здесь не пробел (ИА-32, ADR-0249 §4)`);

  /* --- #122. Покрытие охвата: подпись только тому, кто видит охват целиком ------ */
  AN.seed();
  AN.setRole(ANALYST);
  const cv = mkQ1('ШО-09', 'подразделение', { 'отрасль': 'переработка' }, ['s-over']);
  AN.setReviewText(cv.no, 'Просрочка по переработке сосредоточена в двух подразделениях.');
  AN.setReviewVerdict(cv.no, 'требует внимания');
  const covWide = AN.reviewCoverage(cv.no, ANALYST);
  const covNarrow = AN.reviewCoverage(cv.no, CURATOR);
  const narrowSign = AN.approveReview(cv.no, { to: 'Администрирование кредитов' }, CURATOR);
  const wideSign = AN.approveReview(cv.no, { to: 'Администрирование кредитов' });
  const region = m[1].slice(m[1].indexOf('/* ==ОБЗОР== */'), m[1].indexOf('/* ==/ОБЗОР== */'));
  ok(122,
    ...
    `«${covWide.cut}» виден целиком (${covWide.n} из ${covWide.of}) — подпись прошла. Названной ` +
    `узкой роли видно ${covNarrow.n} из ${covNarrow.of}, и подпись отказана: ` +
    `«${covNarrow.why.slice(0, 112)}…» — смотреть числа и вести черновик это не мешает. Считает ` +
    `покрытие ВЛАДЕЛЕЦ СТРОК (шов statCoverage), а не анализ: ни PORTFOLIO, ни visible( в ` +
    `участке ==ОБЗОР== нет ни разу — второе правило видимости рядом с первым разошлось бы с ним ` +
    `молча (ИА-31, ИА-9, ADR-0152 §3)`);

  /* --- #123. Замена НАЗЫВАЕТСЯ человеком, а не выводится по датам --------------- */
  AN.seed();
  AN.setRole(ANALYST);
  const first = mkQ1('ШО-09', 'подразделение', { 'отрасль': 'переработка' }, ['s-over']);
  AN.setReviewText(first.no, 'Просрочка по переработке сосредоточена в двух подразделениях.');
  AN.setReviewVerdict(first.no, 'требует внимания');
  AN.approveReview(first.no, { to: 'Администрирование кредитов' });
  const second = mkQ1('ШО-09', 'подразделение', { 'отрасль': 'переработка' }, ['s-over']);
  AN.setReviewText(second.no, 'Уточнённое суждение: просрочка выросла и в третьем подразделении.');
  AN.setReviewVerdict(second.no, 'требует решения руководства');
  /* Без слова «взамен» второй обзор — просто второй документ: первый остаётся действующим. */
  const alone = AN.approveReview(second.no, { to: 'Администрирование кредитов' });
  /* Снимок берётся КОПИЕЙ: запись обзора — живой объект, и через пять строк она уже
     изменится заменой. Сравнение с живой ссылкой доказывало бы состояние «потом». */
  const stillLive = JSON.parse(JSON.stringify(AN.REVIEW(first.no)));
  const third = mkQ1('ШО-09', 'подразделение', { 'отрасль': 'переработка' }, ['s-over']);
  AN.setReviewText(third.no, 'Третье суждение — оно и заменяет первое.');
  AN.setReviewVerdict(third.no, 'требует внимания');
  const otherTpl = mkQ1('ШО-04', 'область', {}, ['s-port']);
  const selfRep = AN.approveReview(third.no, { to: 'Администрирование кредитов', replaces: third.no });
  const draftRep = AN.approveReview(third.no, { to: 'Администрирование кредитов', replaces: otherTpl.no });
  const alienRep = AN.approveReview(third.no, { to: 'Администрирование кредитов', replaces: 'ОБ-99' });
  const repOk = AN.approveReview(third.no, { to: 'Администрирование кредитов', replaces: first.no });
  const oldRec = AN.REVIEW(first.no), newRec = AN.REVIEW(third.no);
  const twice = mkQ1('ШО-09', 'подразделение', { 'отрасль': 'переработка' }, ['s-over']);
  AN.setReviewText(twice.no, 'Четвёртое суждение.');
  AN.setReviewVerdict(twice.no, 'требует внимания');
  const chain = AN.approveReview(twice.no, { to: 'Администрирование кредитов', replaces: first.no });
  ok(123,
      alone.ok && stillLive.state === 'утверждено' && !stillLive.replacedBy &&
      !selfRep.ok && has(selfRep.why, 'не заменяет сам себя') &&
      !draftRep.ok && has(draftRep.why, 'черновик') &&
      !alienRep.ok && has(alienRep.why, 'замена называется номером существующего') &&
      repOk.ok && newRec.replaces === first.no && oldRec.replacedBy === third.no &&
      oldRec.state === 'заменён' && !!AN.REVIEW(first.no).issue &&
      !chain.ok && has(chain.why, 'уже заменён'),
    `замена — РЕШЕНИЕ ЧЕЛОВЕКА, а не вывод по датам: второй обзор за тот же период, ` +
    `подписанный БЕЗ слова «взамен» (${second.no}), первого не отменил — ${first.no} остался ` +
    `«${stillLive.state}». Названная замена ставит ПАРУ ссылок одним действием: ${third.no} ` +
    `«взамен ${newRec.replaces}», ${first.no} — «заменён ${oldRec.replacedBy}», состояние ` +
    `«${oldRec.state}»; документ при этом остаётся виден и выпуск свой держит — по нему уже ` +
    `поручали. Четыре промаха названы четырьмя разными отказами: сам себя, черновик, ` +
    `несуществующий номер и вторая замена одного и того же («${chain.why.slice(0, 64)}…») ` +
    `(ИА-34, ADR-0253)`);

  /* --- #124. Правит автор; при неактивном авторе — заведующий, с отметкой -------- */
  AN.seed();
  AN.setRole(SECOND);
  const mine = mkQ1('ШО-09', 'подразделение', { 'отрасль': 'переработка' }, ['s-over']);
  AN.setReviewText(mine.no, 'Просрочка по переработке сосредоточена в двух подразделениях.');
  AN.setReviewVerdict(mine.no, 'требует внимания');
  AN.approveReview(mine.no, { to: 'Администрирование кредитов' });
  const author = AN.REVIEW(mine.no).author;
  const byOther = AN.correctReview(mine.no, { text: 'Правка чужой рукой.', why: 'так короче' }, ANALYST);
  const headEarly = AN.correctReview(mine.no, { text: 'Правка заведующего.', why: 'до увольнения' }, HEAD);
  const own = AN.correctReview(mine.no, { text: 'Автор уточнил сам: подразделений три.',
    why: 'при подготовке комитета пересчитаны строки' }, SECOND);
  AN.state.today = '2026-09-20';                      /* после 15.09.2026 автор неактивен */
  const headLate = AN.correctReview(mine.no, { verdict: 'требует решения руководства',
    why: 'автор уволен, вывод исправлен по решению отдела' }, HEAD);
  const byOtherLate = AN.correctReview(mine.no, { text: 'И снова чужой рукой.', why: 'почему бы нет' },
    ANALYST);
  const marks = AN.REVIEW(mine.no).corrections;
  AN.state.today = TODAY;
  ok(124,
      ...
      has(headLate.notByAuthor.why, 'правка не автором') &&
      marks.length === 2 && !marks[0].notByAuthor && !!marks[1].notByAuthor &&
      marks[1].notByAuthor.since === '2026-09-15' && marks[1].by === 'Алымкулов Б.' &&
      !byOtherLate.ok && has(byOtherLate.why, 'неактивен') && has(byOtherLate.why, 'ЗАВЕДУЮЩИЙ'),
    `суждение правит АВТОР: ${author} поправил свой обзор сам, и запись «было → стало» ` +
    `авторская. Другому сотруднику отказ — «${byOther.why.slice(0, 84)}…», и заведующему, пока ` +
    `автор активен, тот же: «${headEarly.why.slice(0, 76)}…». После 15.09.2026 автор неактивен — ` +
    `заведующий правит, и правка ПОМЕЧЕНА: в записи ${marks.length} корректировки, вторая несёт ` +
    `«правка не автором» и имя ${marks[1].by}. Другому сотруднику и тогда отказано: право ` +
    `исключения — у заведующего, а не у отдела (ИА-35, ADR-0254)`);

  /* --- #125. Поручение уходит К СОСЕДУ; вход не меняет предмета ----------------- */
  AN.seed();
  AN.seedReviews();
  AN.setRole(CHIEF);
  const fromRow = AN.taskFromReview('ОБ-1', { obj: 'p-11', what: 'выйти на заёмщика',
    due: '2026-09-30', from: 'показатель' });
  const outside = AN.taskFromReview('ОБ-2', { obj: 'p-01', what: 'выйти на заёмщика',
    due: '2026-09-30', from: 'показатель' });
  const lst125 = AN.tasksOfReview('ОБ-1');
  /* Смотрим в ЗАПИСИ ОБЗОРОВ, а не во всё состояние: в ленте действий человека номер
     задания стоит законно — её пишет тот, кто нажал, а вот в документе его быть не
     должно ни полем, ни строкой (ИА-11).                                          */
  ...
  ok(125,
    ...
    `показателя», объект взят из списка выпуска. Объект, который статистика в ответе ПОКАЗЫВАЕТ, ` +
    `но в замороженный состав не вошёл, отказан отдельными словами: ` +
    `«${outside.why.slice(0, 118)}…» — расхождение не прячется, оно и есть полезный сигнал. ` +
    `Самой записи в состоянии анализа нет: номер ${fromRow.no} в нём не встречается ни разу, ` +
    `поля tasks не существует (ИА-36, ADR-0250 §1, §3, §4)`);

  /* --- #126. Четвёртая дверь: вид повода обязателен, повод отпадает ОПРОСОМ ----- */
  AN.seed();
  AN.seedReviews();
  const noKind = AN.analysisLeads('');
  const badKind = AN.analysisLeads('обзор просрочен');
  const before126 = AN.analysisLeads('review-decision');
  AN.setRole(CHIEF);
  AN.taskFromReview('ОБ-2', { obj: 'p-06', what: 'вынести на кредитный комитет',
    due: '2026-09-30' });
  ...
  ok(126,
      ...
      has(after126.note, 'ПОЛНОЕ') && has(after126.note, 'опросом, а не отметкой'),
    `повод первый — «обзор требует решения, поручений нет»: в множестве ${before126.n} — ` +
    `${lead.obj} с выводом «${lead.verdict}», адресат «${lead.to}», число поручений спрошено ` +
    `швом ${lead.seamAsked} у соседа. Завели по нему задание — и повод ОТПАЛ САМ, следующим ` +
    `опросом (стало ${after126.n}): отметки «закрыто» никто не ставил и ставить некому. Шов ` +
    `отвечает ПО ВИДУ повода: без вида — отказ («${noKind.why.slice(0, 72)}…»), с выдуманным ` +
    `видом — свой, другой отказ (ADR-0210, ADR-0211, ADR-0251 §2)`);

  /* --- #127. Повод второй: закрытый период, объявленная периодичность ----------- */
  AN.seed();
  AN.seedReviews();
  const miss = AN.analysisLeads('review-missing');
  const q4 = miss.ok ? miss.leads.find(x => x.obj === 'ШО-04') : null;
  const skipNoCycle = miss.ok ? miss.skipped.find(x => x.tpl === 'ШО-09') : null;
  const skipOpen = miss.ok ? miss.skipped.find(x => x.period && x.period.to === '2026-06-30') : null;
  const tpl04 = REP.reportTemplate('ШО-04', TODAY);
  const tpl09 = REP.reportTemplate('ШО-09', TODAY);
  /* Своего поля периодичности ОБЗОРА у анализа нет: в состоянии модуля есть только
     периодичность строк расписания финанализа (§7), а у обзора её вообще нет поля —
     она приходит от шаблона отчётности с ссылкой на приказ.                       */
  const noOwnCycle = AN.state.reviews.every(r => !('cycle' in r) && !('periodicity' in r)) &&
    !JSON.stringify(AN.state.reviews).includes('приказ');
  ok(127,
      miss.ok && miss.n === 1 && !!q4 && q4.period.to === '2025-12-31' &&
      q4.to === 'отдел анализа' && has(q4.what, 'приказ № 118 от 12.01.2026') &&
      !!skipNoCycle && has(skipNoCycle.why, 'периодичность обзора по этой форме приказом не объявлена') &&
      !!skipOpen && has(skipOpen.why, 'не закрыт') &&
      tpl04.ok && tpl04.cycle.every === 'квартал' && tpl04.cycle.basis.indexOf('приказ') === 0 &&
      tpl09.ok && tpl09.cycle === null && !!tpl09.cycleWhy && noOwnCycle,
    `повод второй — «обзора за период нет»: квартал ${q4.period.from}—${q4.period.to} закрыт, ` +
    `периодичность у ШО-04 объявлена («${tpl04.cycle.every}», ${tpl04.cycle.basis}), ` +
    `утверждённого обзора за него нет — повод один, и адресован он отделу анализа. Квартал, за ` +
    `который обзор подписан, повода не даёт; НЕ ЗАКРЫТЫЙ период назван отдельно ` +
    `(«${skipOpen.why.slice(0, 64)}…») — это ответ, а не пропуск. Шаблон без объявленной ` +
    `периодичности тоже назван словами: «${skipNoCycle.why}» — пусто здесь ОТВЕТ, а не пробел. ` +
    `Своего календаря обзоров анализ не держит: поля периодичности в записи обзора нет, и ` +
    `ссылки на приказ там тоже нет ни одной — периодичность живёт полем ШАБЛОНА у отчётности, ` +
    `потому что число, движущееся вместе с распоряжением, обязано быть записью (ADR-0251 §3, §4)`);

  /* --- #128. Ключ устойчив, база отсчёта срока названа -------------------------- */
  AN.seed();
  AN.seedReviews();
  const l1 = AN.analysisLeads('review-decision');
  const l2 = AN.analysisLeads('review-decision');
  const lm = AN.analysisLeads('review-missing');
  const keysSame = JSON.stringify(l1.leads.map(x => x.key)) ===
                   JSON.stringify(l2.leads.map(x => x.key));
  const kDec = l1.leads[0], kMiss = lm.leads[0];
  ok(128,
      keysSame && kDec.key === 'review-decision·обзор:ОБ-2·2026-01-01..2026-03-31' &&
    ...
    `max(дата состояния, дата ввода вида повода в действие): у первого ${kDec.base} (состояние), ` +
    `у второго состояние ${kMiss.at}, а база ${kMiss.base} — «${kMiss.baseWhy.slice(0, 92)}…». ` +
    `Без этого первый же опрос объявил бы просроченным всё накопленное за годы (ADR-0229 §9)`);

  /* --- #129. Шов открыт заданиям, журнал вызовов помнит вид повода -------------- */
  AN.seed();
  AN.seedReviews();
  const call = AN.callSeam('задания', 'analysisLeads', 'review-decision');
  const alien = AN.callSeam('классификация', 'analysisLeads', 'review-decision');
  const done = AN.callSeam('сопровождение', 'analysisDone', 'b-1', '1П 2026', 'plan');
  const log = AN.seamCalls();
  const leadRec = log.find(x => x.seam === 'analysisLeads' && x.ok);
  const refRec = log.find(x => x.seam === 'analysisLeads' && !x.ok);
  const doneRec = log.find(x => x.seam === 'analysisDone');
  ok(129,
      call.ok && call.answer.kind === 'review-decision' &&
      !alien.ok && has(alien.why, 'ФАКТ, а не суждение') &&
      done.ok && log.length === 3 &&
      !!leadRec && leadRec.occasion === 'обзор требует решения, поручений нет' &&
      leadRec.module === 'задания' && leadRec.n === 1 &&
      !!refRec && refRec.module === 'классификация' && refRec.ok === false &&
      !!doneRec && doneRec.occasion === 'плановый' && doneRec.arg === 'b-1',
    `четвёртая дверь открыта ЗАДАНИЯМ и только им: классификации отказ теми же словами, что и ` +
    `на суждение («${alien.why.slice(0, 64)}…»). Журнал вызовов помнит ПОВОД: у ` +
    `analysisLeads записан вид повода («${leadRec.occasion}»), у analysisDone — повод ` +
    `заключения («${doneRec.occasion}»), а без этого вызовы по разным поводам выглядели бы ` +
    `одинаково. Отказ записан НАРАВНЕ с ответом (${log.length} записи на 3 вызова): «не ` +
    `спросили» и «спросили и получили отказ» — разные события (§12, ИА-19)`);

  /* --- #130. Экран «Расписание и швы»: поводы и журнал, без кнопки «поручить» --- */
  const el130 = () => ({ innerHTML: '', textContent: '', dataset: {}, value: '',
    classList: { toggle() {}, add() {}, remove() {} }, appendChild() {}, remove() {} });
  const n130 = { '#panel': el130(), '#title': el130(), '#foot': el130(), '#role': el130(),
    '#subj': el130() };
  sandbox.document = { querySelector: k => n130[k] || el130(), querySelectorAll: () => [],
    getElementById: () => null, createElement: () => el130() };
  AN.seed();
  AN.seedReviews();
  AN.setRole(ANALYST);
  AN.go('schedule');
  const sch130 = n130['#panel'].innerHTML;
  const leadCard = sch130.slice(sch130.indexOf('Что у анализа сейчас требует задания'));
  ok(130,
      has(sch130, 'Что у анализа сейчас требует задания') &&
      has(sch130, 'обзор требует решения, поручений нет') && has(sch130, 'обзора за период нет') &&
    ...
    `множество НЕ вошло и почему. Рядом — журнал вызовов, который на пустом месте говорит ` +
    `словами «Швы ещё не звали». Кнопки «поставить задание» в карточке нет ни одной: анализ ` +
    `отдаёт СОСТОЯНИЕ, задание собирает сосед (ИА-11, ADR-0251 §5)`);

  /* --- #131. Реестр: адресат и замена стоят колонками --------------------------- */
  const el131 = () => ({ innerHTML: '', textContent: '', dataset: {}, value: '',
    classList: { toggle() {}, add() {}, remove() {} }, appendChild() {}, remove() {} });
  const n131 = { '#panel': el131(), '#title': el131(), '#foot': el131(), '#role': el131(),
    '#subj': el131() };
  sandbox.document = { querySelector: k => n131[k] || el131(), querySelectorAll: () => [],
    getElementById: () => null, createElement: () => el131() };
  AN.seed();
  AN.seedReviews();
  AN.setRole(ANALYST);
  const rep = mkQ1('ШО-09', 'подразделение', { 'отрасль': 'переработка' }, ['s-over']);
  AN.setReviewText(rep.no, 'Уточнённое суждение по переработке.');
  AN.setReviewVerdict(rep.no, 'требует внимания');
  const repAp = AN.approveReview(rep.no, { to: 'Администрирование кредитов', replaces: 'ОБ-2' });
  AN.go('reviews');
  const reg131 = n131['#panel'].innerHTML;
  ok(131,
      repAp.ok && has(reg131, '<th>Кому направлен</th>') && has(reg131, '<th>Замена</th>') &&
      has(reg131, 'Отраслевой департамент') && has(reg131, 'заменён <b>' + rep.no + '</b>') &&
      has(reg131, 'взамен <b>ОБ-2</b>') && has(reg131, 'адресата у черновика нет') &&
      has(reg131, '<span class="pill warn">заменён ' + rep.no + '</span>'),
    `в реестре обзоров стоят две новые колонки: «Кому направлен» и «Замена». У черновика ` +
    `адресата нет, и сказано это словами («адресата у черновика нет: его называют при подписи»), ` +
    `а не прочерком. Замена видна С ОБЕИХ сторон: ${rep.no} — «взамен ОБ-2», ОБ-2 — «заменён ` +
    `${rep.no}» и пилюлей в состоянии; заменённый документ из реестра не исчезает (ИА-33, ИА-34, ` +
    `ADR-0252, ADR-0253)`);

  /* --- #132. Пять швов внутрь и четыре наружу — все названы вопросом ------------ */
  AN.seed();
  const inAll = REP.seamsIn().list.concat(win.STAT.seamsIn().list, win.ZD.seamsIn().list);
  const out = AN.seams();
  const newIn = ['repIssueValues', 'zdByBasis', 'zdNewTask', 'statCoverage']
    .filter(id => inAll.some(x => x.id === id));
  const allAsked = inAll.every(x => x.q && x.owner);
  const cons = AN.consumers().find(c => c.module === 'задания');
  ok(132,
    ...
    `таблицы. Модуль «задания» спрашивает у анализа ровно одно — поводы: «${cons.why.slice(0, 96)}…»`);
})();

/* ---- отчёт ---- */
const pass = results.filter(r => r.pass).length;
const lines = results.map(r => `   ${r.pass ? 'PASS' : 'FAIL'}  #${r.n}  ${r.note}`);
console.log(`SMOKE 2026-09-21 · ${pass}/${results.length} PASS\n` + lines.join('\n'));
