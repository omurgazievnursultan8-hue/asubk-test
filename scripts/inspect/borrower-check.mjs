...
/* Имя проверки — про карточку целиком, а не про строку-зеркало: ключ, тип лица и
   наименование печатает и id-bar, поэтому упасть от снятого зеркала эта проверка не может
   (перебивание P-A ревью 29.07.2026). Утверждение верное и оставлено как есть; зеркало
   держит СБ-14.3а ниже. */
ok('СБ-14.2 карточка заёмщика показывает ключ, тип лица и наименование',
