...
/* M-2 (финальное ревью 29.07.2026): id выдавался как 'S-' + (SUBJECTS.length + 1). После любого
   удаления длина падала, и следующее создание выдавало id, уже занятый, — а subject(ref) вернул
   бы по нему первую из двух записей. Формат при этом расходился с засевом (S-001 против S-9).
   Проверяем монотонность через удаление: номер удалённой записи не должен вернуться другой.
   Свой DOM: тест удаляет запись, а общий набор нужен последующим проверкам целым. */
ok('3.9 внутренний id выдаётся счётчиком, а не длиной набора, и после удаления не повторяется (M-2)',
  (() => { const f = mk();
    const a = f.ev("(()=>{createSubject({key:'99999999999999',source:'individuals',name:'Первый'});return subject('99999999999999').id;})()");
    const del = f.ev("(()=>{try{deleteSubject('99999999999999');return true;}catch(e){return false;}})()");
    const b = f.ev("(()=>{createSubject({key:'88888888888888',source:'individuals',name:'Второй'});return subject('88888888888888').id;})()");
    return /^S-\d{3}$/.test(a) && /^S-\d{3}$/.test(b) && del && a !== b &&
      f.ev("(()=>{const ids=SUBJECTS.map(s=>s.id);return new Set(ids).size===ids.length;})()"); })());
...
/* M-3 (финальное ревью 29.07.2026): addEvent клал в ленту СЫРОЙ ключ, тогда как addDoc,
   addBankReq и addIpReg все идут через resolveKey. Событие, заведённое по присоединённому
   ключу, оставалось лежать на псевдониме — читается оно и оттуда, но любой расчёт,
   псевдонимов не знающий, его теряет. Свой DOM: слияние необратимо и мутирует набор. */
ok('6.10 событие, заведённое по присоединённому ключу, ложится на ГЛАВНЫЙ ключ (M-3, инвариант 8)',
  (() => { const f = mk();
    f.ev("doMerge('07701199970071','S-DUP',{confirm:'07701199970071'})");
    f.ev("addEvent({key:'S-DUP',kind:'перевод долга',date:'01.06.2026',basis:'Соглашение о переводе долга',doc:'СГ-5',successorKey:'01204199910016'})");
    return f.ev("SUBJECT_EVENTS.some(e=>e.doc==='СГ-5')") &&
           f.ev("SUBJECT_EVENTS.some(e=>e.doc==='СГ-5'&&e.key==='07701199970071')") &&
           f.ev("SUBJECT_EVENTS.every(e=>e.key!=='S-DUP')"); })());
...
/* I-2 (финальное ревью 29.07.2026): прежняя редакция сравнивала два набора ролей на
   равенство — и только. В демо-наборе они совпадали и БЕЗ учёта псевдонимов (у обеих
   записей ровно «Залогодатель»), поэтому равенство выполнялось и у слепого расчёта:
   перебивание `subjectRoles: const ks=[ref]` оставляло прогон 100/100. Половина инварианта 8
   была недоказуема.
   Теперь ответ по псевдониму обязан не просто СОВПАСТЬ с ответом по главному ключу, а
   ОТЛИЧАТЬСЯ от того, что каждый ключ даёт в одиночку: пока aliasOf не проставлен, keysOf
   возвращает ровно переданный ключ — то есть soloDup/soloMain и есть ответы расчёта,
   псевдонимов не знающего. Они разные (у S-DUP свой договор поручительства, ДП-78), и
   объединение обязано быть шире каждого из них. */
ok('7.3 роли считаются по ключу и его псевдонимам (инвариант 8)',
  (() => { const soloDup  = g.ev("subjectRoles('S-DUP').map(r=>r.role).join(',')");
    const soloMain = g.ev("subjectRoles('07701199970071').map(r=>r.role).join(',')");
    g.ev("subject('S-DUP').aliasOf='07701199970071'");
    const viaAlias = g.ev("subjectRoles('S-DUP').map(r=>r.role).join(',')");
    const viaMain  = g.ev("subjectRoles('07701199970071').map(r=>r.role).join(',')");
    g.ev("delete subject('S-DUP').aliasOf");
    return soloDup !== soloMain            /* фикстура сторожит саму возможность падения */
      && viaAlias === viaMain && viaAlias.length > 0
      && viaMain !== soloMain; })());      /* главный ключ реально ПРИОБРЁЛ роль по псевдониму */
...
ok('8.5 своё перенесено: документы, связи, события, реквизиты — на главном ключе',
  ...
  m.ev("IP_REG.every(r=>r.key!=='S-DUP')") &&
  m.ev("IP_REG.some(r=>r.key==='07701199970071'&&r.no==='ИП-0770117')"));
/* I-2: одного равенства мало — оно выполняется и у расчёта, псевдонимов не знающего, если
   наборы совпадают случайно. Здесь слияние уже произошло, «в одиночку» ключ не спросить,
   поэтому требуем ИМЕННО ту роль, которая держится ссылкой, оставшейся на присоединённом
   ключе: договор поручительства ДП-78 зарегистрирован на S-DUP, слияние его не переписало
   (инвариант 9), своего поручительства у главной записи нет ни одного. Значит роль
   «Поручитель» у главного ключа существует только через псевдоним — расчёт, псевдонимы
   игнорирующий, её не даст. */
ok('8.6 роли считаются по обоим ключам и не переносятся (они производные, СБ-6)',
  ...
  && m.ev("SURETIES.some(x=>x.guarantorKey==='S-DUP'&&x.active)")           /* чужая ссылка на месте */
  && m.ev("SURETIES.every(x=>x.guarantorKey!=='07701199970071')")           /* своего договора нет */
  && m.ev("subjectRoles('07701199970071').some(r=>r.role==='Поручитель')"));
...
ok('8.15 инвариант 10: при нуле ссылок удаление действительно удаляет',
  (() => { const before = m.ev("SUBJECTS.some(s=>s.id==='S-DUP2')");
    ...
    /* Вызов идёт под try: без него любая регрессия выше роняет прогон исключением, и
       вместо строки FAIL печатается стек, а тесты после этого места не отчитываются вовсе. */
    const done = m.ev("(()=>{try{deleteSubject('S-DUP2');return true;}catch(e){return false;}})()");
    return before && can && done && m.ev("SUBJECTS.every(s=>s.id!=='S-DUP2')"); })());
/* I-3 (финальное ревью 29.07.2026). canDelete опознаёт запись по ключу ИЛИ по внутреннему id
   (через subject), а deleteSubject искал её по subjectRef, то есть «ключ ‖ id». Для записи
   С ключом, переданной по id, эти два никогда не совпадали: findIndex давал -1, и
   splice(-1, 1) сносил ПОСЛЕДНЮЮ запись набора. Целевая выживала, посторонняя гибла — и гибла
   в обход инварианта 10, потому что ссылки проверялись не у неё.
   8.15 мимо этого проходит: единственная удаляемая запись набора (S-DUP2) ключа не имеет, и
   там subjectRef(s) === s.id совпадает случайно. Проверка требует обеих половин — названная
   запись удалена И соседняя жива; на возврат splice без проверки индекса краснеют обе.
   Свой DOM: тест создаёт и удаляет записи, набор m нужен проверкам ниже целым. */
ok('8.15а инвариант 10: удаление по внутреннему id сносит названную запись, а не последнюю в наборе',
  (() => { const d = mk();
    const id = d.ev("(()=>{createSubject({key:'99999999999999',source:'individuals',name:'Цель'});return subject('99999999999999').id;})()");
    d.ev("createSubject({key:'88888888888888',source:'individuals',name:'Посторонний'})");
    /* смысл проверки держится только пока жертва действительно последняя в массиве */
    const last = d.ev("SUBJECTS[SUBJECTS.length-1].key")==='88888888888888';
    const can  = d.ev("canDelete('" + id + "')")===true;
    const done = d.ev("(()=>{try{deleteSubject('" + id + "');return true;}catch(e){return false;}})()");
    return last && can && done
      && d.ev("!subject('99999999999999')")
      && d.ev("!!subject('88888888888888')"); })());
...
ok('8.17 «взять B» по полю, которого у присоединяемой записи нет, не предлагается и поля не портит (M-4)',
  (() => { const f = mk();
    f.ev("location.hash='#/merge/07701199970071/S-DUP'"); f.ev("route()");
