...
check('metricsAt: вакантных ставок — 3,5 (2 руководителя + 1 у гл. спец. ГО + 0,5 у совместителя)', m.vac.free === 3.5);
...
check('три блока под ГО, руководитель каждого — Заместитель председателя',
  await page.evaluate(d => UNITS.filter(u => u.type === 'блок')
    .every(u => parentAt(u.id, d) === 'go' && TITLES[headPosOf(u.id, d).titleId].name === 'Заместитель председателя')
...
check('служба — уровень департамента: у неё Начальник службы и свои штатные единицы',
  await page.evaluate(d => UNITS.filter(u => u.type === 'служба')
    .every(u => TITLES[headPosOf(u.id, d).titleId].name === 'Начальник службы'), D));
...
await page.fill('#maFrom', '2026-01-01');   // раньше «сегодня»: иначе освободить (см. ниже) можно только задним числом
await page.click('#maSubmit');
...
// ставка одна и наполовину занята совместителем — основное на 1,0 не помещается (переполнение штата запрещено)
await rowVac.locator('.asgbtn').click();
await page.selectOption('#maEmp', { label: 'Сыдыков Э. Ж.' });   // основного назначения не имеет
await page.selectOption('#maKind', 'main');
await page.click('#maSubmit');
check('назначение сверх числа ставок отклонено',
  (await page.locator('#maErr').innerText()).includes('не помещается'));
await page.click('#maCancel');
// освобождаем совместителя (кадровый оборот в обратную сторону) — ставка снова свободна
await rowVac.locator('[data-rel]').click();
await page.fill('#gf_date', '2026-07-10');
await page.click('#gfSubmit');
check('совместитель освобождён — ставка свободна',
  await page.evaluate(() => ratesOn(headPosOf('sector', state.date).id, state.date)) === 0);
// основное назначение — закрывает вакансию руководителя
await rowVac.locator('.asgbtn').click();
await page.selectOption('#maEmp', { label: 'Сыдыков Э. Ж.' });
await page.selectOption('#maKind', 'main');
await page.click('#maSubmit');
