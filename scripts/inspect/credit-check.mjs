...
(() => {
     ...
     К-7 (с ДС) и К-C4 (закрытый неосвоенный остаток). */
  {
    ...
    /* 155. ГРАНИЦА «СОСТАВА» И «РАСЧЁТОВ» (КВ-36). Тело — величина долга, а не состава:
       ИР-3, зеркальные строки погашений и слово «остаток тела» ушли из «Состава» на
       «Расчёты». Тест сторожит ОБА берега: на «Составе» их нет и стоит «Не освоено», на
       «Расчётах» есть таблица тела с четырьмя слагаемыми развёрнутыми в колонки. Одна
       половина без другой — потеря величины, а не переезд, поэтому условия в одном кейсе.
       К-7 (перенос по ДС) проверяет то, ради чего ИР-3 и заведена: «Перенесено»/«Принято»
       непусты, а у производного транша «Освоено» — прочерк.
       ПРАВКА КВ-40: колонки переносов стали УСЛОВНЫМИ (на сиде перенос ровно у одного
       кредита из 60, у остальных обе колонки давали по два прочерка в строке). Сторожим
       теперь обе стороны гейта: у К-7 колонки есть и непусты, у К-1 их нет, а формула ИР-3
       в подписи остаётся — она определение остатка, а не пересказ колонок. */
    ...
    ok(155, gone(trh1) && gone(trh7) && /Не освоено/.test(trh1)
            && /Тело по траншам/.test(r1) && /Остаток тела/.test(r1) && /ИР-3/.test(r1)
            && !/>Перенесено</.test(r1) && !/>Принято</.test(r1)
            && />Перенесено</.test(r7) && />Принято</.test(r7)
            && /Свод по кредиту на/.test(r1)
            && /− [\d\s ]+,\d\d/.test(r7) && /\+ [\d\s ]+,\d\d/.test(r7)
            && /Производный транш не осваивается/.test(r7),
       `«Состав» чист ${gone(trh1) && gone(trh7)} · «Расчёты» держат тело`
       + ` ${/Тело по траншам/.test(r1)} · К-7 переносы в колонках ${/− [\d\s ]+,\d\d/.test(r7)}`
       + ` · К-1 без колонок переносов ${!/>Перенесено</.test(r1)}`);
