...
(() => {
    ...
    for (const c of CR2.db.credits){
      CR2.openDetail(c.id);
      {
        ...
        /* ДЕТАЛИ — ПО КНОПКЕ, СВОЕЙ НА БЛОК (разбор 16.08.2026; параметры отрезка добавлены
           КВ-67). По умолчанию свёрнуты, поэтому в шапке их ноль; развёрнутые обязаны
           вернуться все разом: два нарастающих по телу (потребовано графиком · погашено) +
           три по процентам (начислено · потребовано графиком · погашено, КВ-65), а освоение,
           перенос, списание и накопленные добавляют свои. Кнопок ТРИ — у каждого блока своя:
           у пени нарастающих по-прежнему нет, но «Ставка» и «За день» прячутся её кнопкой.
           ПАРАМЕТРЫ ОТРЕЗКА стерегутся обоими концами: в свёрнутом их нет ни одного, в
           развёрнутом «Ставка» стоит дважды — у процентов и у пени, — и односторонняя проба
           («есть при развороте») пропустила бы блок, потерявший свою. */
        if ((tbl.match(/>Нарастающим</g) || []).length !== 0)
          bad.push(`${c.id}: нарастающие показаны по умолчанию — они сворачиваются кнопкой блока`);
        const sw = (html.match(/CR\.toggleCalcRun\('(od|int|pen)'\)/g) || []);
        if (new Set(sw).size !== 3)
          bad.push(`${c.id}: переключателей деталей ${new Set(sw).size} вместо трёх (КВ-67)`);
        const both = () => { CR2.toggleCalcRun('od'); CR2.toggleCalcRun('int'); CR2.toggleCalcRun('pen'); };
        both();
        ...
        for (const [re, want] of [[/>Ставка</g, 2], [/>Дней</g, 1], [/>За день</g, 1]]){
          ...
          if (got !== want)
            bad.push(`${c.id}: по кнопке «детали» ${re.source} вернулась ${got} раз(а) вместо ${want}`);
        }
  ...
  /* 170. «НАКОПЛЕННЫЕ» — ПО СОСТАВУ, НЕ ПО КНОПКЕ (КВ-47, идиома ADR-0109). Пара колонок
     ...
     Значит ширина листа зависит уже от двух вещей, и стеречь надо обе:
       ...
       · СОСТОЯНИЕ — те же срезы с развёрнутыми деталями: 28 и 30.
     РАЗВЁРНУТЫЕ ЧИСЛА НЕ ДВИНУЛИСЬ НА КВ-67, свёрнутые упали на четыре (20→16, 21→17):
     волна не сняла ни одной колонки, она увела под кнопку четыре — «Ставка» и «Дней» у
     процентов, «Ставка» и «За день» у пени. Поэтому разворот теперь трогает и третий блок:
     `both()` жмёт все три кнопки, иначе параметры пени остались бы спрятанными и полная
     ширина не собралась бы. Разность 16→28 есть восемь нарастающих блока ОД и процентов
     плюс четыре параметра; 17→30 — девять и четыре, к нарастающим добавляется накопленный.
     Числа выросли на волне КВ-65: блок процентов получил «По графику» (видна всегда) и два
     нарастающих к ней и к «Погашено», — свёрнутое состояние +1, развёрнутое +3. «Списано»
     у K-7 нет НИ В ОДНОМ состоянии: у него нет списаний, и по новому правилу колонки быть
     не должно (обратную сторону — что у списанного кредита она есть — стережёт №173).
     У K-1 умолчание 14.
     Срез возвращается на TODAY: следующие проверки читают карточку. */
  {
    ...
    ok(170, all.length > 0 && all.every(x => x.bad === 0)
         && narrow.every(x => x.w === 16) && narrowOpen.every(x => x.w === 28)
         && wide.every(x => x.w === 17)   && wideOpen.every(x => x.w === 30)
         && plain.every(x => x.w === 14)  && accHead && !offHead,
