...
(() => {
  ...
  /* 180. ПЕНЯ РАЗЛОЖЕНА ПО БАЗАМ (КВ-73, запрос владельца 17.08.2026: «начислено за
     ...
     Проба стоит СЕМЬЮ сторонами, и каждая ловит свой класс поломки:
       ...
       (7) РАСХОЖДЕНИЕ ПРИМЕНЁННОЙ И ОБЪЯВЛЕННОЙ СТАВКИ НАЗВАНО В ЯЧЕЙКЕ, обеими сторонами:
           где модель начислила не по срезу условий (позиция несёт снимок, по которому
           построен график, — Д-5), там янтарная ячейка с двумя числами; где начислила по
           срезу — там её нет. В сиде расходятся ОБЕ стороны: K-3/19.04.2026 по телу (условия
           называют 0,1 %, начисление шло по 0) и семь отрезков K-4 по процентам (ДС-РС-1004
           снизило пеню по процентам вдвое с 01.05.2026, а позиции старее несут 0,05 против
           объявленных 0,1). До КВ-73 всё это было невидимо — ставки стояли склеенными, а
           начисление одним числом. Один из семи (10.12.2025) лежит в свёрнутом году, ячейки
           для него нет — поэтому ожидание для экрана считается по сверенным строкам.
     ОБРАТНАЯ СТОРОНА проверена двумя подменами в `credit.html` (обе откатаны): остаток
     копейки в МЕНЬШУЮ часть (`penSplit`: `<=` вместо `>=`) даёт 5 нарушений стороны (4) —
     на K-2b копейка садится на пустую базу тела и печатается пенёй −0,01 там, где просрочки
     тела нет; показ ОБЪЯВЛЕННОЙ ставки вместо применённой (`penRateCell` берёт
     `penRateMain`/`penRateInt`) даёт 14 нарушений сторон (3), (6) и (7) — на всех шести
     сверяемых расходящихся отрезках экранное произведение перестаёт сходиться с начислением
     (K-3/19.04.2026: 8 157,53 × 0,1 × 12 дн. = 97,89 против прочерка; K-4/01.01.2026:
     4 438,36 × 0,1 × 9 дн. = 39,95 против 19,97), ни одна ячейка расхождения не называет
     (0 из 6), и обе стороны разом остаются непроверенными. Значит проба ловит правило
  ...
  (() => {
    ...
    const bad = [];
    let rows = 0, only = 0, both = 0, dv = 0, cells = 0;
    for (const c of CR2.db.credits){
      const led = CR2.buildLedger(c, CR2.TODAY);
      for (const sh of (led.dateSheet || [])){
        ...
        for (const r of sh.rows){
          ...
          if ((r.penBaseP > 0.005 && Math.abs((r.penRateP || 0) - (r.penRateMain || 0)) > 0.00005)
           || (r.penBaseI > 0.005 && Math.abs((r.penRateI || 0) - (r.penRateInt  || 0)) > 0.00005)) dv++;
        }
      }
    }
    ...
    if (!dv)    bad.push('в сиде нет отрезка, где применённая ставка расходится с объявленной — сторона (7) не проверена');
    ...
    for (const c0 of CR2.db.credits){
      ...
      for (let k = 0; k < Math.min(tbls.length, sheets.length); k++){
        ...
        for (const tr of (body.match(/<tr[^>]*>[\s\S]*?<\/tr>/g) || [])){
          ...
          /* РАСХОЖДЕНИЕ СТЕРЕЖЁТСЯ С ОБЕИХ СТОРОН. Ставок теперь две, и расходиться может
             любая: у K-3 разошлась ставка по телу, у K-4 после ДС-РС-1004 — ставка по
             процентам (позиция несёт снимок графика, срез условий называет другое, Д-5).
             Одна проверенная сторона молчала бы о второй — модель насчитала бы расхождений
             больше, чем ячеек их называет, и счётчик `dvCells !== dv` только сообщил бы
             разницу, не указав колонку. */
    ...
    /* СЧИТАЕМ ОЖИДАНИЕ ПО СВЕРЕННЫМ СТРОКАМ, А НЕ ПО ВСЕЙ МОДЕЛИ: седьмое расхождение сида
       (K-4/10.12.2025) лежит в свёрнутом 2025-м, и ячейки для него на листе нет вовсе —
       требовать её значило бы требовать от показа нарушить правило свёртки. Зато обе стороны
       обязаны быть названы хотя бы раз: у K-3 расходится ставка по телу, у K-4 — по процентам,
       так что односторонний `penRateCell` попадётся. */
    ...
    if (!dvCells['ОД'] || !dvCells['%'])
      bad.push(`расхождение названо только одной стороной (тело ${dvCells['ОД']} · проценты ${dvCells['%']})`);
    ok(180, bad.length === 0 && rows > 100 && cells === want && cells >= 10 && only > 0 && both > 0 && dv > 0,
       `отрезков с пенёй ${rows} (только проценты ${only} · обе базы ${both})`
       + ` · сверено с экраном ${cells} из ${want} (свёрнутых лет ${folded})`
       + ` · названных расхождений ставки ${dvBoth} из ${dvWant} на экране (в модели ${dv})`
       + ` · нарушений ${bad.length}${bad.length ? ' — ' + bad.slice(0, 3).join(' | ') : ''}`);
  })();
