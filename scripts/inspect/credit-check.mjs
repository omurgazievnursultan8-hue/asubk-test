...
(() => {
  ...
  /* список дублирует DTABS макета вручную и уже разошёлся с ним один раз: девятую
     ...
     Сверка идёт join('|'), то есть стережёт и ПОРЯДОК — с КВ-74 он несёт смысл (три
     блока: что за кредит · деньги · сопровождение), а не историю появления вкладок. */
  const TABS = ['Договор','Состав','Условия','График','Прогноз','Платежи','Расчёты','План','Обеспечение','Проблемные','Досье'];
    ...
    for (const c of CR2.db.credits){
      CR2.openDetail(c.id);
      {
        ...
        /* ДЕТАЛИ — ПО КНОПКЕ, СВОЕЙ НА БЛОК (разбор 16.08.2026; параметры отрезка добавлены
           ...
           в шапке их ноль. Кнопок ТРИ — у каждого блока своя; после КВ-73 под кнопкой пени
           лежит весь её разрез: две базы, две ставки, дни, цена дня и нарастающее.
        ...
        let opened = '', openedPen = '', nRun = 0;
        for (const block of ['od','int','pen']){
          const { h, t } = openBlock(block);
          if (block === 'int') opened = t;
          if (block === 'pen') openedPen = t;
          ...
          for (const need of (block === 'od' ? ['Нарастающим'] : block === 'int' ? ['Ставка','Дней','Нарастающим']
                              : ['Просрочено ОД','Ставка ОД','Просрочено %','Ставка %','Дней','Нарастающим']))
        ...
        /* Семь нарастающих собираются ТРЕМЯ разборами, а не одним: разом их больше не
           показать. Два по телу (потребовано графиком · погашено) + три по процентам
           (начислено · потребовано графиком · погашено, КВ-65) + одно по пене (всего пени,
           КВ-73: остаток пени в модели единый, поэтому нарастающее у неё одно, а не по
           базам); освоение, перенос, списание и накопленные добавляют свои. */
        if (nRun < 7) bad.push(`${c.id}: нарастающих по всем разборам ${nRun}, а обязано быть не меньше семи (КВ-51, КВ-65, КВ-73)`);
        for (const [re, want] of [[/>Ставка</g, 1], [/>Дней</g, 1]]){
        ...
        /* РАЗРЕЗ ПЕНИ ПО БАЗАМ (КВ-73) — ОБЕИМИ СТОРОНАМИ, потому что каждая ловит своё.
           Пеня — единственная статья с ДВУМЯ формулами (просроч. тело × своя ставка и
           просроч. проценты × своя), и прежде лист печатал их сумму да склеенную ячейку
           ставок «0,1 / 0,05» — проверить руками нельзя было ни одну сторону.
             (1) в умолчании стоят ОБА начисления и «Всего»: это ответ, а не деталь, и
                 спрятать их под кнопку значило бы отменить сам запрос;
             (2) в умолчании нет ни одного множителя — правило КВ-67 не делает пене
                 исключения, хотя множителей у неё стало шесть;
             (3) при разборе пени каждый множитель приходит РОВНО ОДИН раз, а «Ставка»
                 процентов остаётся свёрнутой (КВ-68) — иначе на листе снова стоят две
                 «Ставки» разом, и читатель не знает, какая из них чья. */
        for (const need of ['Начислено ОД','Начислено %','Всего'])
          if (!tbl.includes('>' + need + '<'))
            bad.push(`${c.id}: «${need}» не стоит в умолчании — начисления пени врозь и общей цифрой это ответ (КВ-73)`);
        if (/>За день</.test(html))
          bad.push(`${c.id}: вернулась «За день» — цена дня читается из баз и ставок той же строки (КВ-73)`);
        for (const [re, want] of [[/>Просрочено ОД</g, 1], [/>Ставка ОД</g, 1], [/>Просрочено %</g, 1],
                                  [/>Ставка %</g, 1], [/>Дней</g, 1], [/>Ставка</g, 0]]){
          if ((tbl.match(re) || []).length !== 0)
            bad.push(`${c.id}: множитель пени ${re.source} показан по умолчанию — он уходит под «детали» (КВ-73/КВ-67)`);
          const got = (openedPen.match(re) || []).length;
          if (got !== want)
            bad.push(`${c.id}: при разборе пени ${re.source} встретился ${got} раз(а) вместо ${want} (КВ-73)`);
        }
  ...
  /* 170. «НАКОПЛЕННЫЕ» — ПО СОСТАВУ, НЕ ПО КНОПКЕ (КВ-47, идиома ADR-0109). Пара колонок
     ...
     Значит ширина листа зависит уже от двух вещей, и стеречь надо обе:
       · СОСТАВ — срез 23.07.2026 (позиции ДС-РС-2002 15.08–15.10.2026 ещё не наступили)
         против среза 20.11.2026, где «Накопленные» появляются: 18 → 19 колонок;
     ...
     КВ-73 РАЗДВИНУЛА УМОЛЧАНИЕ НА ДВЕ КОЛОНКИ (16 → 18, 17 → 19, K-1 14 → 16): блок пени
     печатает начисления по базам врозь и общей цифрой. Разбор пени встал ровно в ширину
     умолчания K-1 (16) — по этому упору из блока и убрана «За день»: тринадцатая колонка
     разбора нарушала обещание волны, а цену дня видно из баз и ставок той же строки.
     ...
     Срез возвращается на TODAY: следующие проверки читают карточку. */
  {
    ...
    ok(170, all.length > 0 && all.every(x => x.bad === 0) && fits
         && narrow.every(x => x.w === 18) && wide.every(x => x.w === 19)
         && plain.every(x => x.w === 16)
         && narrowD[0].every(x => x.w === 16) && narrowD[1].every(x => x.w === 14)
         && narrowD[2].every(x => x.w === 16)
     ...
     статьях не путается. Подпись кнопки деталей проверяется здесь же. */
  {
    ...
    for (const c of CR2.db.credits){
      ...
      for (let k = 0; k < Math.min(yrows.length, years.length); k++){
        ...
        /* ПЕНЯ В ИТОГЕ ГОДА — ТРИ ПОТОКА (КВ-73): обе стороны врозь и общая цифра. Складывать
           их полагается порознь, а не выводить третью из двух: сумма пары обязана совпасть с
           «Всего» и в итоге года, а не только построчно (округление копейки живёт в строке). */
        flow(idx(pen, 'Начислено ОД'), r2(rs.reduce((a, r) => a + (r.penaltyP || 0), 0)), '«Пеня ОД»');
        flow(idx(pen, 'Начислено %'),  r2(rs.reduce((a, r) => a + (r.penaltyI || 0), 0)), '«Пеня %»');
        flow(idx(pen, 'Всего'), r2(rs.reduce((a, r) => a + (r.penalty || 0), 0)), '«Пеня всего»');
        ...
        /* ПАРАМЕТР ОТРЕЗКА НА ИТОГОВОЙ СТРОКЕ МОЛЧИТ (КВ-58): у ставок и баз пени правила
           итога нет — ни поток, ни состояние, — и пустая ячейка тут содержательна. После
           КВ-73 таких колонок в блоке пени четыре, и «Дней» рядом с ними складывается: у
           него правило потока есть, поэтому в список не входит. */
        for (const [blk, name] of [[int, 'Ставка'], [pen, 'Ставка ОД'], [pen, 'Ставка %'],
                                   [pen, 'Просрочено ОД'], [pen, 'Просрочено %']]){
          const i = idx(blk, name);
          if (i >= 0 && cs[i].replace(/<[^>]*>/g, '').trim() !== '')
            bad.push(`${c.id}/${years[k]}: «${name}» печатает итог, а это параметр отрезка`);
        }
      }
      ...
      if (!trow) bad.push(`${c.id}: строки «Итого по траншу» нет`);
        ...
        const gi = numOf(tc[idx(int, 'Начислено')]) || 0, gp = numOf(tc[idx(pen, 'Всего')]) || 0;
     ...
     ловится именно гейт, а не совпадение разметки. */
  {
    ...
    const SCOPED = ['Договор','Состав','Условия','График','Прогноз','Платежи','Расчёты','План','Обеспечение','Проблемные'];
  ...
  /* 178. «ПЛАТЕЖИ»: КОПИЯ СВОДА СНЯТА, ХВОСТ ОЧЕРЕДИ СВЁРНУТ (КВ-71, владелец 17.08.2026).
     Две правки одной вкладки, и стережём их вместе, потому что вторая опирается на первую:
     итог очереди сверяют со сводом, а свод с этой правки живёт только на «Расчётах».
     (1) «Задолженность по статьям» с «Платежей» снята — секция печатала тот же
         `debtArticlesOf(d.debt)`, что свод «Расчётов», но под снятым 14.08.2026 именем
         «Начислено», в обратном порядке колонок, без строк-исключений и с заголовком на
         `cardAsOf` там, где числа посчитаны к `d.calcUntil` (9 кредитов сида с остановленным
         начислением). Положительный контроль обязателен: величина не потеряна, свод на
         «Расчётах» стоит у всех 60 — иначе снятие копии прошло бы вместе со снятием оригинала.
     (2) Хвост очереди свёрнут по умолчанию: ненаступившие строки в разнесение не идут
         (ADR-0057 §1), а на сиде их 1 000 из 1 254, и у 35 кредитов из 60 наступивших ноль.
         Проверяется по РАЗМЕТКЕ и в обе стороны: свёрнуто — ни одной пилюли «не наступило»
         при целом числе «просрочено»; раскрыто — ровно `tail` серых строк, блок назван
         строкой-группой, наступившие не тронуты. Односторонняя проверка пропустила бы
         фильтр, который прячет всё подряд.
     (3) Кнопка есть ровно там, где есть что прятать, и несёт число хвоста; итог таблицы
         («Итого просроченного») от кнопки не зависит — окно решает, что видно, а не что
         посчитано.
     (4) Состояние — на карточку: у другого кредита другой хвост, и открытие соседа обязано
         вернуть умолчание. */
  {
    const NOTDUE = />не наступило</g, OVER = />просрочено</g;
    let bad = [], withTail = 0, opened = 0, emptyDue = 0, svod = 0;
    for (const c of CR2.db.credits){
      CR2.openDetail(c.id);
      const q = CR2.derive(c, CR2.TODAY).queue;
      const due = q.rows.filter(r => !r.future).length, tail = q.rows.length - due;
      let h; try { h = CR2.renderTab('Платежи', c); }
      catch(e){ bad.push(`${c.id}: «Платежи» не отрисовались — ${e.message}`); continue; }
      if (/Задолженность по статьям/.test(h)) bad.push(`${c.id}: копия свода вернулась на «Платежи»`);
      if (!/Очередь погашения/.test(h))       bad.push(`${c.id}: секции очереди нет`);
      const nNot = (h.match(NOTDUE) || []).length, nOver = (h.match(OVER) || []).length;
      if (nNot)          bad.push(`${c.id}: ${nNot} ненаступивших строк при свёрнутом хвосте`);
      if (nOver !== due) bad.push(`${c.id}: наступивших в разметке ${nOver} против ${due} в модели`);
      if (due && !/Итого просроченного/.test(h)) bad.push(`${c.id}: итога просроченного нет`);
      if (!due){ emptyDue++;
        if (tail && !/Просроченного нет/.test(h))
          bad.push(`${c.id}: пустая таблица подписана не «Просроченного нет»`); }
      if (tail){ withTail++;
        if (!h.includes(`Ненаступившее (${tail} ·`)) bad.push(`${c.id}: кнопки хвоста на ${tail} строк нет`);
        CR2.toggleQueueTail(); opened++;
        const h2 = CR2.renderTab('Платежи', c);
        const n2 = (h2.match(NOTDUE) || []).length;
        if (n2 !== tail) bad.push(`${c.id}: раскрыто ${n2} строк хвоста из ${tail}`);
        if ((h2.match(OVER) || []).length !== due) bad.push(`${c.id}: раскрытие тронуло наступившие`);
        if (!/class="gyear static"[\s\S]{0,240}Ненаступившее/.test(h2)) bad.push(`${c.id}: блок хвоста не назван`);
        /* Приглушение — на КЛАССЕ строки, а не инлайновым `color` на <tr>: `table.cgrid
           tbody td` назначает ячейке свой цвет, и до неё наследование не доходило —
           хвост печатался наравне с просроченным (найдено при КВ-71). */
        if ((h2.match(/<tr class="qfut">/g) || []).length !== tail)
          bad.push(`${c.id}: хвост без класса приглушения`);
        if (due && !/Итого просроченного/.test(h2)) bad.push(`${c.id}: итог пропал при раскрытии`);
        CR2.toggleQueueTail();
      } else if (/CR\.toggleQueueTail/.test(h)) bad.push(`${c.id}: кнопка хвоста при пустом хвосте`);
      if (/Свод по кредиту на /.test(CR2.renderTab('Расчёты', c))) svod++;
    }
    /* Умолчание возвращается на каждую карточку: разворачиваем хвост, уходим к соседу,
       возвращаемся — серых строк быть не должно. */
    const tc = CR2.db.credits.find(c => CR2.derive(c, CR2.TODAY).queue.rows.some(r => r.future));
    let reset = false;
    if (tc){ CR2.openDetail(tc.id); CR2.toggleQueueTail();
      const other = CR2.db.credits.find(c => c.id !== tc.id);
      CR2.openDetail(other.id); CR2.openDetail(tc.id);
      reset = !NOTDUE.test(CR2.renderTab('Платежи', tc)); NOTDUE.lastIndex = 0; }
    /* Класс без правила — украшение: проверяем и стиль в исходнике. */
    const css = /table\.cgrid tbody tr\.qfut td\{[^}]*color:var\(--text-muted\)/.test(readFileSync(HTML, 'utf8'));
    if (!css) bad.push('правила `tr.qfut td` в стилях нет — класс ничего не приглушает');
    const n = CR2.db.credits.length;
    ok(178, bad.length === 0 && svod === n && withTail >= 20 && opened === withTail
            && emptyDue >= 20 && reset && css,
       `кредитов ${n} · свод на «Расчётах» у ${svod} · с хвостом ${withTail} (раскрыто и свёрнуто ${opened})`
       + ` · без наступивших ${emptyDue} · умолчание вернулось ${reset} · приглушение хвоста ${css}`
       + ` · нарушений ${bad.length}${bad.length ? ' — ' + bad.slice(0, 3).join(' | ') : ''}`);
  }
  ...
  /* 180. ПЕНЯ РАЗЛОЖЕНА ПО БАЗАМ (КВ-73, запрос владельца 17.08.2026: «начислено за
     просрочку основной суммы и процентов отдельно и одной общей цифрой, просрочка,
     нарастающие»). Пеня — единственная статья, которую считают ДВЕ формулы разом: просроч.
     тело × своя ставка и просроч. проценты × своя (ADR-0129 §2). Лист печатал их сумму и
     склеенную ячейку ставок «0,1 / 0,05» — проверить руками нельзя было ни одну сторону.
     Проба стоит СЕМЬЮ сторонами, и каждая ловит свой класс поломки:
       (1) пара сходится в «Всего» ТОЧНО, без допуска: слагаемые копятся неокруглёнными и
           округляются один раз (`penSplit`), поэтому «≈» здесь было бы признанием, что
           экран печатает три числа, из которых два не дают третьего;
       (2) «Всего» осталось ТЕМ ЖЕ числом — сверяется с суммой пени позиций транша
           (`penaltyAccrued`), то есть со сводом статей и очередью: раздел по базам обязан
           быть показом, а не пересчётом долга;
       (3) арифметика КАЖДОЙ стороны: база × применённая ставка × дней = начислено этой
           стороны. Множители взяты из тех же полей, что печатает лист, — иначе проба
           проверяла бы модель, а читатель видел бы другие числа;
       (4) НУЛЕВАЯ БАЗА ДАЁТ НУЛЬ — сторона проверки на копейку остатка: `penSplit` двигает
           её в БОЛЬШУЮ часть, и обратное правило (в меньшую) напечатало бы копейку пени
           там, где просрочки нет вовсе (K-3 — пеня только с процентов);
       (5) ОБА СЛУЧАЯ ЕСТЬ В СИДЕ: односторонняя пеня (K-3, ставка по телу обнулена) и
           двусторонняя (K-1). Без этой стороны проба прошла бы и на данных, где вторая
           формула не работает вовсе, — а именно её раздел и вводит;
       (6) ЭКРАН ПЕЧАТАЕТ ТЕ ЖЕ ТРИ ЧИСЛА — ячейки разобранного листа сверяются с моделью
           построчно, вместе с базами; базы к тому же обязаны совпасть с «Просрочено»
           соседних блоков, потому что владелец сознательно оставил их на листе дважды
           («сколько должен» в своём блоке и «с чего считали пеню» — в пене);
       (7) ПРИМЕНЁННАЯ СТАВКА РАВНА ОБЪЯВЛЕННОЙ — ВЕЗДЕ, и ни одна ячейка не янтарная
           (КР-64, перевёрнуто 17.08.2026). Прежде сторона требовала ОБРАТНОГО — «пусть
           в сиде найдётся отрезок, где они расходятся»: расхождение считали свойством
           модели (позиция несла снимок условий, по которому построен график, Д-5), а
           ячейка его называла. Это было закреплением дефекта: пеня — санкция за КАЖДЫЙ
           день просрочки, ставка её берётся на ЭТОТ день (Р-21), и позиционный снимок на
           дату среза протекал в прошлое — K-4 недосчитал 908,98 (ДС-РС-1004 снизило пеню
           по процентам вдвое с 01.05.2026, а старые дни считались уже по сниженной),
           K-3 — 97,89. С исправлением начисление и колонка читают условия на начало
           ОДНОГО отрезка, поэтому равенство стало инвариантом, а янтарь — сторожем;
       (8) СТАВКА ВСЁ-ТАКИ МЕНЯЕТСЯ ВО ВРЕМЕНИ — иначе (7) проходила бы и на модели,
           замороженной одним снимком: там применённая тоже равна… самой себе. В сиде
           у K-4 пеня по процентам стоит 0,1 % до 01.05.2026 и 0,05 % после, у K-3 пеня
           по телу обнуляется судебным решением — значит хотя бы у одного транша набор
           ставок по отрезкам двузначен. Возврат к снимку на срез обнулил бы этот счётчик.
     ОБРАТНАЯ СТОРОНА проверена подменами в `credit.html` (все откатаны): остаток копейки
     в МЕНЬШУЮ часть (`penSplit`: `<=` вместо `>=`) даёт 5 нарушений стороны (4) — на K-2b
     копейка садится на пустую базу тела и печатается пенёй −0,01 там, где просрочки тела
     нет; возврат ставок к позиционному снимку (`r.penaltyMain/penaltyInt` вместо
     `conditionsAt(t, fd(a))`) роняет стороны (7) и (8) разом — 7 расходящихся отрезков
     на K-3/K-4 и ни одного транша с меняющейся ставкой; показ ОБЪЯВЛЕННОЙ ставки вместо
     применённой ломает стороны (3) и (6) на любом расхождении. Значит проба ловит правило
     раздела и правило показа, а не совпадение разметки. */
  (() => {
    const r2v = v => Math.round(v * 100) / 100;
    const numOf = s => { const t = s.replace(/<[^>]*>/g, '').replace(/[\s  ]/g, '').replace(',', '.').replace(/−/g, '-');
      return /^-?\d+(\.\d+)?$/.test(t) ? +t : null; };
    const bad = [];
    let rows = 0, only = 0, both = 0, dv = 0, cells = 0, varying = 0;
    for (const c of CR2.db.credits){
      const led = CR2.buildLedger(c, CR2.TODAY);
      for (const sh of (led.dateSheet || [])){
        /* сторона (8): набор ставок транша ПО ОТРЕЗКАМ. Двузначен там, где условия за
           жизнь транша менялись, — и обязан быть двузначен хотя бы у одного. */
        const seenP = new Set(), seenI = new Set();
        const sheetPen = r2v(sh.rows.reduce((a, r) => a + (r.penalty || 0), 0));
        const splitPen = r2v(sh.rows.reduce((a, r) => a + (r.penaltyP || 0) + (r.penaltyI || 0), 0));
        const ledPen = r2v(led.rows.filter(r => r.trancheNo === sh.trancheNo)
          .reduce((a, r) => a + (r.penaltyAccrued || 0), 0));
        if (Math.abs(splitPen - sheetPen) > 0.02)
          bad.push(`${c.id}/т${sh.trancheNo}: сумма сторон ${splitPen} ≠ «Всего» ${sheetPen}`);
        if (Math.abs(sheetPen - ledPen) > 0.05)
          bad.push(`${c.id}/т${sh.trancheNo}: «Всего» ${sheetPen} ≠ пеня позиций ${ledPen} — раздел пересчитал долг`);
        for (const r of sh.rows){
          if (!(r.penalty > 0.005)) continue; rows++;
          if (r2v(r.penaltyP + r.penaltyI) !== r.penalty)
            bad.push(`${c.id}/${r.date}: ${r.penaltyP} + ${r.penaltyI} ≠ ${r.penalty} — копейка раздела потерялась`);
          const wp = r.penBaseP * ((r.penRateP || 0) / 100) * r.days;
          const wi = r.penBaseI * ((r.penRateI || 0) / 100) * r.days;
          if (Math.abs(wp - r.penaltyP) > 0.05)
            bad.push(`${c.id}/${r.date}: ОД ${r.penBaseP} × ${r.penRateP} × ${r.days} дн. даёт ${r2v(wp)}, а начислено ${r.penaltyP}`);
          if (Math.abs(wi - r.penaltyI) > 0.05)
            bad.push(`${c.id}/${r.date}: % ${r.penBaseI} × ${r.penRateI} × ${r.days} дн. даёт ${r2v(wi)}, а начислено ${r.penaltyI}`);
          if (r.penBaseP <= 0.005 && r.penaltyP !== 0)
            bad.push(`${c.id}/${r.date}: пеня ${r.penaltyP} по телу без просроченного тела`);
          if (r.penBaseI <= 0.005 && r.penaltyI !== 0)
            bad.push(`${c.id}/${r.date}: пеня ${r.penaltyI} по процентам без просроченных процентов`);
          if (r.penaltyP <= 0.005 && r.penaltyI > 0.005) only++;
          if (r.penaltyP > 0.005 && r.penaltyI > 0.005) both++;
          if (r.penBaseP > 0.005) seenP.add(r.penRateP || 0);
          if (r.penBaseI > 0.005) seenI.add(r.penRateI || 0);
          /* (7) РАВЕНСТВО, А НЕ РАСХОЖДЕНИЕ: применённая ставка (взвешенная базой, из
             слагаемых начисления) обязана совпасть с условиями транша на начало отрезка.
             Каждое расхождение называется поимённо — это уже не свойство модели, а счёт
             недобранной или лишней пени. */
          if (r.penBaseP > 0.005 && Math.abs((r.penRateP || 0) - (r.penRateMain || 0)) > 0.00005){
            dv++; bad.push(`${c.id}/${r.date}: пеня по телу начислена по ${r.penRateP} %, а условия на начало отрезка называют ${r.penRateMain} %`);
          }
          if (r.penBaseI > 0.005 && Math.abs((r.penRateI || 0) - (r.penRateInt || 0)) > 0.00005){
            dv++; bad.push(`${c.id}/${r.date}: пеня по процентам начислена по ${r.penRateI} %, а условия на начало отрезка называют ${r.penRateInt} %`);
          }
        }
        if (seenP.size > 1) varying++;
        if (seenI.size > 1) varying++;
      }
    }
    if (!only)  bad.push('в сиде нет отрезка с пенёй ТОЛЬКО по процентам — раздел не проверен односторонним случаем');
    if (!both)  bad.push('в сиде нет отрезка с пенёй по обеим базам — раздел не проверен двусторонним случаем');
    if (!varying) bad.push('ни у одного транша ставка пени не меняется по отрезкам — сторона (7) прошла бы и на модели, замороженной снимком на срез');
    /* ЭКРАН — ПО ВСЕМУ СИДУ, а не на паре кредитов: у K-1 пеня двусторонняя, у K-3
       односторонняя и там же единственное расхождение ставок, но выбирать кредиты руками
       значит проверять то, о чём уже знаешь. Число сверенных строк считается из модели и
       сверяется с числом найденных на экране — иначе проба тихо проходит, отобрав ноль строк
       (ровно этой болезнью она болела на первом прогоне: порог «> 20» при десяти строках).
       Разбор пени открыт, иначе множителей на листе нет вовсе (КВ-67). */
    const dvCells = { 'ОД':0, '%':0 }, dvSeen = { 'ОД':0, '%':0 };
    let want = 0, folded = 0;
    for (const c0 of CR2.db.credits){
      const id = c0.id, c = c0;
      CR2.openDetail(id);
      CR2.toggleCalcDetail('pen');
      const html = CR2.renderTab('Расчёты', c);
      CR2.toggleCalcDetail('pen');
      const tbls = html.match(/<table class="cgrid tiered">[\s\S]*?<\/table>/g) || [];
      const sheets = CR2.buildLedger(c, CR2.TODAY).dateSheet || [];
      for (let k = 0; k < Math.min(tbls.length, sheets.length); k++){
        /* ЖДЁМ СТРОКИ ТОЛЬКО ОТКРЫТЫХ ЛЕТ. Лист складывает прошлые годы в одну строку
           (`gyear` без `open`), и у K-6 из 26 отрезков с пенёй на экране стоят 2: остальные
           лежат в свёрнутых 2023–2025. Ожидание берётся из САМОГО листа — из года, который
           он объявил открытым, — а не из модели целиком: иначе проба требует строк, которых
           экран и не обещал. Свёрнутые годы при этом тоже считаются: обнулись их число, и
           «сверено столько же, сколько ждали» стало бы правдой при развёрнутом наизнанку
           правиле показа. */
        const openYears = new Set([...tbls[k].matchAll(/<tr class="gyear open"[^>]*CR\.toggleCalcYear\((\d{4})\)/g)].map(x => +x[1]));
        folded += ((tbls[k].match(/<tr class="gyear"/g) || []).length);
        want += sheets[k].rows.filter(r => r.penalty > 0.005 && r.days > 0
                                       && openYears.has(+r.date.slice(6, 10))).length;
        const hrows = (tbls[k].match(/<thead>[\s\S]*?<\/thead>/) || [''])[0].match(/<tr[\s\S]*?<\/tr>/g) || [];
        const lab = ((hrows[1] || '').match(/<th[^>]*>[\s\S]*?<\/th>/g) || []).map(t => t.replace(/<[^>]*>/g, '').trim());
        /* первая ячейка строки — «Дата и событие» (rowspan=2, во втором этаже её нет),
           поэтому индекс подписи сдвигается на единицу */
        const col = (cs, name, last) => { const i = last ? lab.lastIndexOf(name) : lab.indexOf(name);
          return i < 0 ? undefined : cs[i + 1]; };
        const body = (tbls[k].match(/<tbody>[\s\S]*?<\/tbody>/) || [''])[0];
        for (const tr of (body.match(/<tr[^>]*>[\s\S]*?<\/tr>/g) || [])){
          if (/<tr class="g/.test(tr)) continue;                 // строки года и итога — не отрезки
          const cs = (tr.match(/<td[^>]*>[\s\S]*?<\/td>/g) || []);
          const date = (cs[0] || '').replace(/<[^>]*>/g, ' ').trim().slice(0, 10);
          const r = sheets[k].rows.find(x => x.date === date && x.days > 0);
          if (!r || !(r.penalty > 0.005)) continue;
          cells++;
          for (const [name, want] of [['Начислено ОД', r.penaltyP], ['Начислено %', r.penaltyI],
                                      ['Всего', r.penalty], ['Просрочено ОД', r.penBaseP],
                                      ['Просрочено %', r.penBaseI]]){
            const cell = col(cs, name);
            if (cell === undefined){ bad.push(`${id}: колонки «${name}» нет на разобранном листе`); continue; }
            const got = numOf(cell);
            if (want > 0.005 ? Math.abs((got || 0) - want) > 0.02 : got !== null)
              bad.push(`${id}/${date}: «${name}» на экране ${got} против ${want} в модели`);
          }
          /* БАЗЫ СТОЯТ ДВАЖДЫ — намеренно (решение владельца), и оба экземпляра обязаны
             называть одно число: «Просрочено» соседнего блока отвечает «сколько должен»,
             база пени — «с чего считали». Сравниваются только положительные: у соседей
             колонка двузнаковая (переплата минусом, КВ-50), а базы пени такой не бывает. */
          for (const [name, base, last] of [['Просрочено', r.penBaseP, false], ['Просрочено', r.penBaseI, true]]){
            const got = numOf(col(cs, name, last) || '');
            if (got !== null && got > 0.005 && base > 0.005 && Math.abs(got - base) > 0.02)
              bad.push(`${id}/${date}: «Просрочено» блока ${last ? 'процентов' : 'тела'} ${got} против базы пени ${base}`);
          }
          /* ПРОИЗВЕДЕНИЕ СЧИТАЕТСЯ ИЗ ПЕЧАТНЫХ ЧИСЕЛ, А НЕ ИЗ ПОЛЕЙ МОДЕЛИ. Обещание листа —
             «проверить руками»: читатель берёт базу, ставку и дни ИЗ СТРОКИ и получает
             напечатанное начисление. Сверка по полям модели этого не стережёт: показ мог бы
             печатать объявленную ставку вместо применённой, и все модельные тождества
             остались бы верны, а три числа на экране — нет. Янтарная ячейка расхождения
             числом не читается (там два числа) — такие строки пропускаются, их стережёт
             сторона (7). */
          const rateOf = txt => { const t = String(txt).replace(/<[^>]*>/g, '').replace(/[\s  ]/g, '')
            .replace('%', '').replace(',', '.');
            return /^-?\d+(\.\d+)?$/.test(t) ? +t : null; };
          const dn = numOf(col(cs, 'Дней') || '');
          for (const [rn, bn, an] of [['Ставка ОД', 'Просрочено ОД', 'Начислено ОД'],
                                      ['Ставка %',  'Просрочено %',  'Начислено %']]){
            const rate = rateOf(col(cs, rn) || ''), base = numOf(col(cs, bn) || ''), acc = numOf(col(cs, an) || '');
            if (rate === null || base === null || dn === null) continue;
            const wantAcc = base * rate / 100 * dn;
            if (Math.abs(wantAcc - (acc || 0)) > 0.05)
              bad.push(`${id}/${date}: ${base} × ${rate} × ${dn} дн. на экране даёт ${r2v(wantAcc)}, а в «${an}» стоит ${acc}`);
          }
          /* ЯНТАРЬ СТЕРЕЖЁТСЯ С ОБЕИХ СТОРОН — и обеих ждут ПУСТЫМИ. После КР-64 ставка
             начисления и ставка условий читаются на одну дату, поэтому `penRateCell`
             печатает одно число; появившийся янтарь означает, что раздел снова начислил
             не по тем условиям. Проверяется ЭКРАН, а не только модель: показ мог бы
             печатать объявленную ставку рядом с начислением по другой и молчать. */
          for (const [side, base, applied, declared, name] of
               [['ОД', r.penBaseP, r.penRateP, r.penRateMain, 'Ставка ОД'],
                ['%',  r.penBaseI, r.penRateI, r.penRateInt,  'Ставка %']]){
            const wantDv = (base > 0.005 && Math.abs((applied || 0) - (declared || 0)) > 0.00005);
            const gotDv  = /warn-badge/.test(col(cs, name) || '');
            if (wantDv !== gotDv)
              bad.push(`${id}/${date}: расхождение ставки ${side} ${wantDv ? 'есть в модели, а ячейка молчит' : 'названо ячейкой, а модели его нет'}`);
            if (wantDv) dvSeen[side]++;
            if (gotDv) dvCells[side]++;
          }
          /* СТАВКА НА ЭКРАНЕ — ТА, ЧТО В УСЛОВИЯХ НА НАЧАЛО ОТРЕЗКА. Сверка модельных полей
             этого не стережёт: `penRateP` и `penRateMain` могли бы разъехаться вместе с
             условиями и остаться равными друг другу. Здесь ставка берётся ИЗ СТРОКИ и
             сравнивается с `conditionsAt` — независимым чтением условий транша. */
          {
            const tr = (c.tranches || []).find(x => x.no === sheets[k].trancheNo);
            const cnd = tr ? CR2.conditionsAt(tr, date) : null;
            if (cnd) for (const [side, base, name, key] of
                 [['ОД', r.penBaseP, 'Ставка ОД', 'penaltyMain'], ['%', r.penBaseI, 'Ставка %', 'penaltyInt']]){
              if (!(base > 0.005)) continue;
              const shown = rateOf(col(cs, name) || '');
              if (shown !== null && Math.abs(shown - (cnd[key] || 0)) > 0.00005)
                bad.push(`${id}/${date}: ставка ${side} на экране ${shown} %, а условия транша на эту дату — ${cnd[key] || 0} %`);
            }
          }
        }
      }
    ...
    if (cells !== want)
      bad.push(`на экране найдено ${cells} строк с пенёй из ${want} ожидаемых — сверка отобрала не те строки`);
    if (!folded)
      bad.push('ни одного свёрнутого года на сиде — ожидание «строки только открытых лет» проверено с одного конца');
    /* ЯНТАРЯ БЫТЬ НЕ ДОЛЖНО НИ ОДНОГО (КР-64). Счётчики оставлены обоими: `dvSeen` —
       сколько расхождений насчитала модель на сверенных строках, `dvCells` — сколько
       названо ячейками. Раньше они сходились на семёрке и это считалось нормой; теперь
       норма — ноль, а сходиться они обязаны по-прежнему: показ, молчащий о расхождении,
       и показ, выдумавший его, — разные поломки, и различает их только пара счётчиков. */
    const dvBoth = dvCells['ОД'] + dvCells['%'], dvWant = dvSeen['ОД'] + dvSeen['%'];
    if (dvBoth !== dvWant)
      bad.push(`расхождений ставки на открытых годах ${dvWant}, а названо ячейками ${dvBoth}`);
    if (dvBoth)
      bad.push(`янтарных ячеек ставки ${dvBoth} (тело ${dvCells['ОД']} · проценты ${dvCells['%']}) — после КР-64 применённая ставка обязана равняться объявленной`);
    ok(180, bad.length === 0 && rows > 100 && cells === want && cells >= 10 && only > 0 && both > 0
            && dv === 0 && dvBoth === 0 && varying > 0,
       `отрезков с пенёй ${rows} (только проценты ${only} · обе базы ${both})`
       + ` · сверено с экраном ${cells} из ${want} (свёрнутых лет ${folded})`
       + ` · расхождений применённой и объявленной ставки ${dv} в модели / ${dvBoth} на экране (ждём 0)`
       + ` · траншей с меняющейся ставкой ${varying}`
       + ` · нарушений ${bad.length}${bad.length ? ' — ' + bad.slice(0, 3).join(' | ') : ''}`);
  })();
