...
const goTab=async()=>{await page.evaluate(()=>{const t=[...document.querySelectorAll('vaadin-tab')].find(t=>/Предпросмотр|Предпросмотр/i.test(t.innerText));t&&t.click();});await page.waitForTimeout(1500);};
await goTab();
...
log('===TAB9 DEFAULT===');log(JSON.stringify(await dump(),null,1));
await page.screenshot({path:'.auth/t9-default.png',fullPage:true});
...
log('===TAB9 ALL CHECKED===');log(JSON.stringify(await dump(),null,1));
await page.screenshot({path:'.auth/t9-checked.png',fullPage:true});
await ctx.close();
