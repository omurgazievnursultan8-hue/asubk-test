...
log('===TAB6 DEFAULT===');log(JSON.stringify(await dump(),null,1));
// reveal select options
log('===SELECT OPTIONS===');
log(JSON.stringify(await page.evaluate(()=>[...document.querySelectorAll('vaadin-select')].filter(s=>s.getBoundingClientRect().width>0).map(s=>{const lbl=s.querySelector('label,[slot=label]')?.innerText?.trim();const opts=[...(s.querySelector('vaadin-list-box, template')?.querySelectorAll?.('vaadin-item')||[])].map(o=>o.innerText.trim());return {lbl,opts};})),null,1));
...
log('===TAB6 ALL CHECKED===');log(JSON.stringify(await dump(),null,1));
await page.screenshot({path:'.auth/t6-checked.png',fullPage:true});
await ctx.close();
