...
// Walks the 11-tab loan-credits detail for a given record id.
// Usage: node scripts/inspect/tz/dump-loan-credit-tabs.mjs [id]
// Defaults to record 18 (stable/working). Records 20, 22 crash — skip them.
...
// Navigate directly to the loan detail page
...
const extractGrid = async () => page.evaluate(() => {
  ...
  document.querySelectorAll('vaadin-grid').forEach(grid => {
    ...
    // Cell content (first few rows only)
...
for (let i = 0; i < tabs.length; i++) {
  ...
  const screenshot = `.auth/tz-loan-credit-${recordId}-tab${i + 1}.png`;
  await page.screenshot({ path: screenshot, fullPage: true });

  results.push({ tabIndex: i + 1, tabName: tabs[i], fields, buttons, grids });
  console.log(`\n=== TAB ${i + 1}: ${tabs[i]} ===`);
  ...
  console.log('BUTTONS:', JSON.stringify(buttons.filter(b => !/^(Приложение|Мониторинг|Справочники|Система|Поручительства|Корпоративное|Документы|Отчёты|Регистры|СУГС|Взыскание|Организация|Оформление|Условия|Заемщик|Кредит|Залог|Орг|Кураторство|Сервисы|Администрирование|Безопасность|Инструменты|АСУБК)/.test(b))));
