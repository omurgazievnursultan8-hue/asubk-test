// Verify «Вид счета погашения» picker on loan-credits/25 (tab «Общая информация»).
import { chromium } from 'playwright-core';

const BASE = 'https://fkftest.okmot.kg/';
const USER = process.env.OK_USER || 'admin';
const PASS = process.env.OK_PASS || 'admin';
const TARGET = 'Вид счета погашения';
...
// find the jmix-value-picker whose label is «Вид счета погашения»
...
const dump = await page.evaluate(() => {
  ...
  const fullText = clean(ov.textContent).slice(0,800);
...
await page.screenshot({ path: '.auth/field-vidschet.png', fullPage: true });
log('\nScreenshot -> .auth/field-vidschet.png');
await ctx.close();
