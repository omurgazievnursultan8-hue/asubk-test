...
//   V    готовность к передаче (#100–#104): пакет объявлен в шапке, карта «экран → карточки»
...
/* ---------- V. Готовность к передаче разработчику (#100–#104) ---------- */
(() => {
  ...
  /* Расхождение «ролей четыре, а в модели пять» названо и в шапке, и на экране: молча
     недостающая роль читалась бы как решение. */
  ...
  ok(103, said && KU.dict.ROLES.length === 4,
    `пятая роль названа как пауза, а не как отсутствие: плашка на экране политики ссылается на ` +
    `P18-R2, где список назначаемых ролей закрыт пятью`);

  /* Адрес происхождения домена показывается именем модуля, а не путём в чужом репозитории:
     путь остаётся подсказкой. */
  const addr = /function srcAddr/.test(m[1]) && /вне пакета/.test(m[1]) &&
               /в пакет передачи не входит/.test(m[1]);
  ok(104, addr,
    `отметка сверенности показывает источник домена именем модуля («прототип модуля «Заёмщик»»), ` +
    `путь в чужом репозитории — подсказкой с пометкой «вне пакета»`);
})();
