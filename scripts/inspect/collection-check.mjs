...
ok('сроки заёмщика в сроки поручителя не текут',
   D.ev(`deadlinesOf(REQ_INDEX['307/307/з']).length`) === 2
...
/* Числа этого раздела пересчитаны с нуля по затравке ЗС (91 дело · 100 требований), снято
   с живой страницы: `dlAll()` даёт 76 записей, из них горизонт 7 дней пропускает 70
   (просроченных 54, истекает сегодня 1). Прежние 37/50/18 остались от старой затравки. */
...
ok('глубина просрочки производна, а не взята из хранимого поля (СК-Д2)', () => {
  ...
  return of('304').join('|') === '−47 · просрочен|−23 · просрочен'
      && of('391').join('|') === '−150 · просрочен|−126 · просрочен'; });
...
ok('отрисованные строки идут по возрастанию остатка', (()=>{
  ...
  return a.length === 70 && a.every((v,i) => i === 0 || a[i-1] <= v) && a[0] === -150 && a[a.length-1] === 6; })());
...
ok('рамка считает очередь и называет дату отсчёта',   // счётчики сняты с #dlFrame живой страницы
   /показано 70 из 76 · просрочено 54 · истекает сегодня 1 · отсчёт от 21\.07\.2026/.test(sFrame()));
