...
/* Дело 361 (K6-ИНИЦ) стоит на исполнительном производстве, но постановления о ВОЗВРАТЕ ИЛ
   в его цепочке нет — затравка банкротной ветви новым предусловиям не удовлетворяет, и
   это не дефект гейта: правило действует вперёд, уже зарегистрированные меры не отменяет
   (пересев затравки — шаг 6 наряда). */
ok('на живом требовании гейт называет недостающий факт пунктом',
   S.ev(`/п\\. 5[567]/.test(preconditionReason(${R('361/361/з')}, 'Заявление на банкротство')||'')`));
