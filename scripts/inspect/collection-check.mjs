...
/* Затравка ЗС состоявшихся реализаций залога не содержит (p.realization нет ни у одного
   дела — торги в K4-ТОРГИ доведены только до акта несостоявшихся торгов), поэтому
   выручка задаётся в тесте на залоговом деле 326 (два предмета, охват «обращением на
   предмет залога»). Проверяется то же правило: п. 49 — строка возврата залогодателю
   обязательна даже при нуле, непокрытый остаток назван пунктом 33. */
{ const m = mk();
  m.ev(`PROCESSES.find(p=>p.id==='326').realization =
          { proceeds: 50000, note:'Реализация предмета залога с торгов (тест)' }`);
  m.ev(`openDetail('326/326/з')`); m.ev(`switchTab(${TAB.dolg})`);
...
ok('соглашение ложится на требование и солидарных соседей', mk().ev(`(() => {
  ...
  return REQ_INDEX['307/307/з'].states.some(s=>s.agreement===a)
      && REQ_INDEX['307/307/п'].states.some(s=>s.agreement===a);
})()`));
...
ok('МС-5 · мировое допустимо на судебной стадии требования, не раньше',
   g.ev(`msStageEligible(${R('142/142/з')},'mirovoe').ok`) === true
   && g.ev(`msStageEligible(${R('201/201/з')},'mirovoe').ok`) === false);
ok('МС-5 · добровольное требует судебного акта по требованию',
   g.ev(`msStageEligible(${R('142/142/з')},'dobrovolnoe').ok`) === true
   && g.ev(`msStageEligible(${R('201/201/з')},'dobrovolnoe').ok`) === false);
...
/* Затравка ЗС: мировое-ПРОЕКТ (график ДАК + аттестация «не льготнее») — дело 344,
   утверждённое судом мировое — 120, добровольное исполнение — 142 и 345. */
{ const m = mk(); m.ev(`openDetail('344/344/з')`); m.ev(`msApprove('МС-344')`);
  ok('МС-3 · утверждение под гейтом (344: график + аттестация есть)',
     m.ev(`_msFind('МС-344').status`) === 'утверждено судом');
  ok('МС-3 · без графика отраслевого утвердить нельзя', m.ev(`(()=>{
    const a=_msFind('МС-344'); a.status='проект'; a.scheduleBy=''; msApprove('МС-344'); return a.status;
  })()`) === 'проект'); }
{ const m = mk(); m.ev(`openDetail('344/344/з')`); m.ev(`msReject('МС-344')`);
  ok('МС-3 · отказ в утверждении', m.ev(`_msFind('МС-344').status`) === 'отказано в утверждении'); }
{ const m = mk(); m.ev(`openDetail('120/120/з')`); m.ev(`msBreach('МС-120')`);
  ok('МС-7 · нарушение утверждённого МС ставит флаг breached', m.ev(`_msFind('МС-120').breached`) === true);
  ok('МС-7 · нарушение гасит состояние обязательства',
     m.ev(`REQ_INDEX['120/120/з'].states.some(s=>s.agreement && s.agreement.num==='МС-120' && s.active===false)`)); }
{ const m = mk(); m.ev(`openDetail('344/344/з')`);
  ok('МС-7 · msBreach не трогает проект (не утверждён судом)', m.ev(`(()=>{
    const a=_msFind('МС-344'); const b=a.breached; msBreach('МС-344'); return a.breached===b && !a.breached;
  })()`)); }
ok('МС-3 · баннер зависит от статуса (утверждено / проект / нарушено)',
   /утверждено судом/.test(g.ev(`msBanner(${P('120')}.agreements[0])`))
   && /проект, не утверждён/.test(g.ev(`msBanner(${P('344')}.agreements[0])`))
   && /нарушено/.test(g.ev(`msBanner({type:'mirovoe',num:'X',breached:true})`)));
/* Мировое требует судебной стадии требования (МС-5), а солидарная пара ЗС стоит в К1 —
   поэтому регистрация показывается на судебном 142. Растекание соглашения на солидарных
   соседей проверено отдельно, в М-11 («соглашение ложится на требование и соседей»). */
ok('регистрация мирового привязана к требованию', mk().ev(`(() => {
  ...
  return !!a && a.req === '142/142/з'
      && REQ_INDEX['142/142/з'].states.some(s=>s.agreement===a);
})()`));
{ const m = mk(); m.ev(`openDetail('120/120/з')`); m.ev('openAgreementModal()');
...
ok('после восстановления соглашение снова связано с состоянием', mk().ev(`(() => {
  ...
  return r.states.some(s => s.agreement && s.agreement.num === 'МС-120');
})()`));
...
/* Сроки заёмщика в требование поручителя не текут: у 307 все сроки адресованы
   заёмщику, и очередь поручителя пуста — цели срока разделяют их так же, как меры.
   Волна ЗС-2 (вывод сроков по журналу): очередь заёмщика 2 → 3. Снято со страницы —
   `deadlinesOf(REQ_INDEX['307/307/з']).map(d=>d.tpl+':'+d.due)` даёт
   ['6:30.06.2026','8:24.07.2026','41:07.08.2026']: к двум срокам исполнения претензий
   добавился тпл 41 «Служебная записка в ДПО о судебном взыскании», выведенный от
   истечения срока повторной претензии (ПР-307/2). Второй конъюнкт — само правило —
   не тронут: у поручителя по-прежнему ноль. */
ok('сроки заёмщика в сроки поручителя не текут',
   D.ev(`deadlinesOf(REQ_INDEX['307/307/з']).length`) === 3
   && D.ev(`deadlinesOf(REQ_INDEX['307/307/п']).length`) === 0);
...
ok('предстоящие сроки показаны, а не только просроченные',
   sRows().length === 80 && sRows().filter(r=>/просрочен/.test(r.textContent)).length === 59);
...
ok('отрисованные строки идут по возрастанию остатка', (()=>{
  ...
  /* Края очереди сняты со страницы: самый глубокий просроченный — −150 (дело 391,
     ...
     первичной претензии 391 и не дальше +6. */
  return a.length === 80 && a.every((v,i) => i === 0 || a[i-1] <= v) && a[0] === -150 && a[a.length-1] === 6; })());
...
/* Ни у одного живого срока затравки ЗС подразделение шаблона и подразделение требования
   не расходятся (все шесть выведенных — ОПК), поэтому на статике «из шаблона» неотличимо
   от «из требования» и проверка была бы пустой. Разводим их в отдельном DOM: требованию
   дела 336 ставим ДАК, шаблон №27 остаётся за ОПК — ответственный обязан остаться ОПК. */
...
ok('сумма документа не пересчитана, расхождение подписано «сейчас …»',   // 110 из 178 строк, снято со страницы
   rRows(C,'claims').filter(r=>/сейчас /.test(r.textContent)).length === 110
...
ok('рамка считает вручение, сторно и расхождение',   // счётчики сняты с #claimsFrame
   /без подтверждения вручения <b>0<\/b> \(п\. 20\.2\) · сторнировано <b>2<\/b> · сумма документа разошлась с требованием у <b>110<\/b>/
...
ok('умолчание — ждут решения',               Q.ev(`regState.committee.seg`) === 'pending'
   && rRows(Q,'committee').length === 6 && Q.ev(`REGS.committee.all().length`) === 72);   // 6 нерешённых из 72, снято со страницы
...
ok('отказов органа в затравке нет — все решённые вопросы положительные',
   ...
   && Q.ev(`REGS.committee.all().filter(x=>x.st.k === 'positive').length === 66`));
...
/* Дело 374 из затравки ушло. Сценарий «иск подан, ответа суда нет» несёт ситуация
   K3-ИСК-ЖДЁМ (дела 330 · 331), и ровно эти два иска дают дыру; третью даёт иск ИСК-344
   дела 344 (K3-МС — мировое ещё проект, определения по иску нет). Просрочена из трёх
   только дыра ИСК-344: 10 к.д. шаблона №45 от даты подачи там уже истекли, у 330/331
   иски свежие. Поэтому «дыра есть» показывается на 331, «дыра просрочена» — на 344. */
...
ok('дыра просрочена, когда 10 к.д. шаблона №45 истекли (дело 344, ИСК-344)',
   g.ev(`(() => { const d = dlOf(${P('344')}).find(x=>x._childGapOf && x._childGapOf.num==='ИСК-344');
     return !!d && d.tpl===45 && isOverdue(d); })()`));
...
/* Дыр в затравке ровно три (ИСК-330 · ИСК-331 · ИСК-344) — значит все ОСТАЛЬНЫЕ 47 исков
   свой ответ суда уже получили и дыр не показывают; проверяем это списком целиком, а не
   выборкой номеров, чтобы новый иск без ответа не проскочил мимо. */
ok('иски с уже зарегистрированным живым ответом суда (basedOn) дыр не показывают',
   g.ev(`(() => {
     ...
     return [...gaps].sort().join(',') === 'ИСК-330,ИСК-331,ИСК-344'
         && all.length === 50 && all.filter(n=>!gaps.has(n)).length === 47;
   })()`));
...
/* Видов-вех 21: 17 прежних + К7 переписан волной ЗС (заключение комиссии → «Признана
   ...
   правопреемника». Пересчитано по живому справочнику: MEASURE_KINDS с непустым kindPhase. */
ok('видов-вех 21 (свёртка kindPhase, MILESTONE_KINDS как отдельный справочник снят — ADR-0033/0038)',
   g.ev(`MEASURE_KINDS.filter(k=>kindPhase(k.name)).length`) === 21);
