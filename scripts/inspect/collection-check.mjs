...
{ const m = mk(); m.setRole('Отдел проблемных кредитов (ОПК)');
  ok('36. гейт пересечения срабатывает на судебной стадии (142 по кредиту 56 ↔ 151)',
    m.ev("measureGate(PROCESSES.find(p=>p.id==='142'),'Исковое заявление').kind") === 'crossing');
...
ok('50. 15 процессов в списке по умолчанию (203 скрыт)', g.$$('#listBody tr').length === 15);
