...
/* Дыр в затравке ровно три (ИСК-330 · ИСК-331 · ИСК-344) — значит все ОСТАЛЬНЫЕ 48 исков
   (было 47 — наряд п.7в добавил кейс 701, чья SPINE-цепочка доходит до «решение», то есть
   несёт живой ответ суда) свой ответ суда уже получили и дыр не показывают; проверяем это
   списком целиком, а не выборкой номеров, чтобы новый иск без ответа не проскочил мимо. */
ok('иски с уже зарегистрированным живым ответом суда (basedOn) дыр не показывают',
   g.ev(`(() => {
     ...
     return [...gaps].sort().join(',') === 'ИСК-330,ИСК-331,ИСК-344'
         && all.length === 51 && all.filter(n=>!gaps.has(n)).length === 48;
   })()`));
...
ok('видов мер 60 (было 51 — Task 2/4 расщепили «Определение суда» на 5 + добавили «Постановление апелляционной инстанции»; было 59 до наряда п.7 — п.7а добавил «Повторное предъявление ИЛ»)',
   g.ev('MEASURE_KINDS.length') === 60);
