...
ok('предстоящие сроки показаны, а не только просроченные',
   sRows().length === 91 && sRows().filter(r=>/просрочен/.test(r.textContent)).length === 62);
...
ok('отрисованные строки идут по возрастанию остатка', (()=>{
  ...
  return a.length === 91 && a.every((v,i) => i === 0 || a[i-1] <= v) && a[0] === -155 && a[a.length-1] === 7; })());
...
ok('сумма документа не пересчитана, расхождение подписано «сейчас …»',   // 116 из 190 строк (было 112 из 180 — `ADR-0079` добавил дела 346/347, каждое несёт обе свои претензии; было 110 из 178 — наряд п.7в, кейс 701)
   rRows(C,'claims').filter(r=>/сейчас /.test(r.textContent)).length === 116
...
ok('рамка считает вручение, сторно и расхождение',   // счётчики сняты с #claimsFrame
   /без подтверждения вручения <b>0<\/b> \(п\. 20\.1\) · сторнировано <b>2<\/b> · сумма документа разошлась с требованием у <b>116<\/b>/
...
ok('умолчание — ждут решения',               Q.ev(`regState.committee.seg`) === 'pending'
   && rRows(Q,'committee').length === 7 && Q.ev(`REGS.committee.all().length`) === 76);   // 7 нерешённых из 76 (было 7 из 74 — `ADR-0079` добавил дела 346/347 кассационной ветви, оба доходят до решения суда и приносят по решённому вопросу)
...
ok('отказов органа в затравке нет — все решённые вопросы положительные',
   ...
   && Q.ev(`REGS.committee.all().filter(x=>x.st.k === 'positive').length === 69`));   // было 67 — `ADR-0079`, дела 346/347 кассационной ветви
...
/* Дыр в затравке было три (ИСК-330 · ИСК-331 · ИСК-344) — наряд МП-11 (ADR-0047) закрыл
   ...
   Осталось две; все ОСТАЛЬНЫЕ 51 иск (было 49 — `ADR-0079` добавил 346/347 кассационной
   ветви, оба с решением суда в цепочке; было 47 — наряд п.7в добавил 701; было 48 до
   наряда МП-11) свой ответ/прогресс уже получили. */
ok('иски с уже зарегистрированным живым ответом суда (basedOn) дыр не показывают',
   g.ev(`(() => {
     ...
     return [...gaps].sort().join(',') === 'ИСК-330,ИСК-331'
         && all.length === 53 && all.filter(n=>!gaps.has(n)).length === 51;
   })()`));
...
/* Сверка «требовали → присудили» ходит по basedOn ОТДЕЛЬНОЙ свёрткой (claimChainsFor) —
   ...
   Затравкой это не поднять: `m.claims` (требуемое по статьям) в ней не несёт НИ ОДНА мера,
   и таблица сверки пуста на всех 101 деле — дыра названа ниже отдельной проверкой, а
   согласие двух свёрток снимается на подставленном claims с откатом. */
ok('сверка «требовали → присудили» ходит той же свёрткой, что и слой',
   g.ev(`(() => { const p = PROCESSES.find(x=>x.id==='346'); const r = REQ_INDEX['346/346/з'];
     const isk = p.measures.find(m=>m.num==='ИСК-346');
     isk.claims = { [r.id]: { principal: 300000 } };
     const ch = claimChainsFor(r).filter(x=>x.award);
     delete isk.claims;
     return ch.length === 1 && ch[0].award.num === 'РС-346'; })()`));
ok('дыра названа: требуемое по статьям (m.claims) затравка не несёт нигде',
   g.ev(`PROCESSES.flatMap(p=>p.measures||[]).filter(m=>m.claims).length === 0`));

// 4. Границы затравки — названы, чтобы не сойти за полноту
ok('присуждение на судебном акте несут только два дела кассационной ветви',
   g.ev(`(() => { const withAward = PROCESSES.flatMap(p=>(p.measures||[]).filter(m=>m.kind==='Решение суда' && m.awards).map(m=>m.num));
     return withAward.sort().join(',') === 'РС-346,РС-347'; })()`));
