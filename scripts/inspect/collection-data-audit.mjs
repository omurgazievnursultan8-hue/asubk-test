...
const out = await p.evaluate(() => {
  ...
  for(const pr of PROCESSES) for(const r of pr.requirements){
    ...
    /* C6: подразделение vs полномочия. Task 9 fix round 2 — раньше код ТОЛЬКО СОБИРАЛ
       строки respCheck и ничего не утверждал (Minor 5 ревью: «фикс, который держится
       инвариантом, а не кодом, возвращается»).

       Буквальное `resp ∈ allowed` данным недоступно: allowed — список ПОДРАЗДЕЛЕНИЙ
       (['ДАК','ОД','РП']), а m.responsible — ФИО, и карты «ФИО → подразделение» в
       данных нет. Единственная похожая — ACCEPTOR_BY_DEPT (куратор подразделения для
       колонки приёма передачи), но затравка переиспользует те же имена как кураторов
       дел ДРУГИХ подразделений: сверка по ней даёт 46 «нарушений» на 12 делах прошлых
       волн (303, 307, 316, 322, 338, 120, 345, 355, 359, 376, 377, 392 — измерено),
       то есть она не авторитет о принадлежности лица. Модельный пробел записан в
       отчёт Task 9 для Task 12.

       Проверяемое утверждение той же природы: подписант — функция ПОДРАЗДЕЛЕНИЯ, а не
       меры, поэтому В ПРЕДЕЛАХ ОДНОГО ДЕЛА все меры с ОДИНАКОВЫМ набором допустимых
       подразделений обязаны быть подписаны ОДНИМ лицом. Ровно это и ловит класс
       дефекта Minor 1 того же ревью (дело 412: «Первичная претензия» по ОД-требованию
       подписана куратором ОПК, а по ДПО-требованию — куратором ОД, одна и та же пара
       вид×допустимые подразделения, два разных подписанта). */
    for(const m of ms){
  ...
  /* C6 (утверждение) — по ДЕЛУ, а не по требованию: мера может целить в несколько
     требований, и внутри цикла выше один и тот же разнобой репортился бы дважды. */
  for(const pr of PROCESSES){
    const signerOf = {};
    for(const m of pr.measures||[]){
      const allowed = subdivOf(m.kind);
      ...
      if(prev && prev.resp !== m.responsible)
        add('C6-подписант', `дело ${pr.id} · мера ${m.num||'(без №)'} «${m.kind}»`,
          `подписант «${m.responsible}», а мера ${prev.num} «${prev.kind}» того же набора подразделений [${key}] подписана «${prev.resp}» — подписант выводится из ПОДРАЗДЕЛЕНИЯ, а не из вида меры: в одном деле у одного подразделения он один`);
  ...
  for(const pr of PROCESSES) for(const q of pr.committeeQuestions||[]){
    ...
    if(!known && !q.free) add('F1-вопрос', w, 'предмет вне справочника CQ_SUBJECTS (свободный предмет помечается free:true — так его ставит форма)');
