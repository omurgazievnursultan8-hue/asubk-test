/* Аудит ДОСТОВЕРНОСТИ ЗАТРАВКИ макета «Взыскание» (mockups/collection/collection.html).
   ...
   Текущий прогон (волна ЗС целиком, контуры К0…К7 + терминалы): 0 находок, 91 дело,
   100 требований — см. ИТОГО в конце вывода, не эту шапку. */
import { chromium } from 'playwright-core';
/* Путь выводится из cwd — тем же способом, что и у соседей по каталогу
   (collection-seed-coverage.mjs, collection-mock-shot.mjs). Абсолютный путь в
   рабочее дерево ветки (.claude/worktrees/vzyskanie/…) переживал бы слияние ветки
   и снос дерева: линейка либо не запустилась бы вовсе, либо молча проверяла бы
   устаревшую копию мокапа. Запускать из корня репозитория. */
...
/* Гард сборки (тем же приёмом, что уже применяет collection-seed-coverage.mjs:75):
   именованный throw билдера затравки роняет сборку PROCESSES, и вся дальнейшая
   evaluate() падала на «PROCESSES is not defined» — прогон краснел, но НЕ НАЗЫВАЛ
   причину, хотя она уже лежала в errs. Печатаем её и выходим красным. */
if(errs.length){
  console.log('pageerrors:', JSON.stringify(errs, null, 1));
  console.log('ИТОГО: затравка не собралась — ошибка страницы выше; аудит не выполнялся');
  await b.close();
  process.exit(1);
}
...
const out = await p.evaluate(() => {
  ...
  for(const pr of PROCESSES){
    ...
    for(const m of pr.measures||[]){
      ...
      /* A4-цель УДАЛЕНА (финальный обзор ветки, 02.08.2026). Правило разбирало
         `purpose` мерой регулярки /установление фазы «(.+?)»/ и сверяло названную
         там фазу с measureSetsPhase(m). Ни одна из 502 мер текущей затравки такого
         purpose не несёт, и подстроки «установление фазы» нет во всём файле мокапа
         вовсе: purpose — деловая цель документа («погашение просроченной
         задолженности», «взыскание задолженности в судебном порядке», SPINE.purpose
         и литералы веток билдера), фазу она не объявляет НИГДЕ и объявить не может —
         фаза выводится парой вид×исход (measureSetsPhase), второго её носителя в
         модели не осталось. Проверять было нечего: правило печаталось в «0 находок»,
         обещая покрытие, которого у него нет. Восстанавливать имеет смысл только
         вместе с полем, которое снова объявит фазу текстом. */
  ...
  /* C6 — подписант. Task 9 fix round 2 (утверждение вместо сбора строк) + round 3
     ...
     подписанта (respFor). А она разрешает подписанта ДВУМЯ РАЗНЫМИ путями, и ключ
     обязан повторять именно их (round 4, ревью: ключ «набор × подразделение кредита»
     из round 3 был грубее модели и РАЗБИВАЛ группы, которые модель требует держать
     вместе — подписанта меры ВНЕ подразделения кредита он делал непроверяемым):

       · мера ЗАКОННА для подразделения кредита (allowed ∋ cr.subdiv) →
         `deptCurators[cr.subdiv] || c.curator` — зависит ТОЛЬКО от подразделения
         кредита, поэтому все такие меры дела с одним cr.subdiv, КАКОГО БЫ ВИДА они
         ни были, подписаны одним лицом. Ключ — `@<подразделение кредита>`.
       · мера ВНЕ его → первый названный в deptCurators из ДОПУСТИМЫХ подразделений;
         cr.subdiv на выбор не влияет вовсе, значит все меры с одним и тем же набором
         допустимых подразделений — по любому кредиту дела — подписаны одним лицом.
         Ключ — сам набор.

     Дело с кредитами в РАЗНЫХ подразделениях, оба из которых допустимы для одного
     вида (претензия по ОД-договору и по ДАК-договору), попадает в ПЕРВУЮ ветку с
     разными ключами — два законных подписанта, не находка. Подразделение кредита
     читается через цели меры (m.targets → требование → _credit.subdiv), а не через
     r.subdivision: последнее двигает ПРИНЯТАЯ ПЕРЕДАЧА (ADR-0023), тогда как подписант
     определялся подразделением договора на момент регистрации.
  ...
  for(const pr of PROCESSES){
    ...
    for(const m of pr.measures||[]){
      ...
      const creditSubdivs = [...new Set((m.targets||[]).map(id => REQ_INDEX[id])
        .filter(Boolean).map(r => (r._credit||{}).subdiv))].sort();
      /* Мера целит в требования РОВНО одного кредита (цели кредит-скоупны — М-5/МТ-1);
         разнородный случай теоретически возможен только у рукописной extra-меры —
         тогда «в подразделении» она считается, лишь если ВСЕ её кредиты в нём.

         Финальный обзор ветки (02.08.2026): развилка СПРАШИВАЕТСЯ У МОДЕЛИ —
         measureOutOfSubdiv(kind, subdiv) — той же функцией, которой её спрашивает сам
         respFor билдера. Прежняя строка переписывала условие своими словами
         (`allowed.includes(s)`), и на кредите БЕЗ subdiv две записи расходились
         молча: respFor уходил в ветку «своё подразделение» (подписант — куратор
         дела), C6 — в ветку «чужое» (ключ по набору допустимых), поэтому подмену
         подписанта не ловил никто, а ИТОГО печатало 0 находок. Ветка отсутствия
         subdiv теперь ещё и недостижима — билдер бросает по имени, — но линейка
         обязана держать ТУ ЖЕ развилку, а не вторую её копию. */
      const inSubdiv = creditSubdivs.length > 0 && creditSubdivs.every(s => !measureOutOfSubdiv(m.kind, s));
      const key = inSubdiv ? `@ ${creditSubdivs.map(s => s || '—').join('+')}` : `[${allowed.join(' | ')}]`;
      const how  = inSubdiv ? 'подразделение кредита (мера законна для него)'
                            : 'набор допустимых подразделений (мера вне подразделения кредита)';
      const prev = signerOf[key];
      if(prev && prev.resp !== m.responsible)
        add('C6-подписант', `дело ${pr.id} · мера ${m.num||'(без №)'} «${m.kind}»`,
          `подписант «${m.responsible}», а мера ${prev.num} «${prev.kind}» с тем же ключом «${key}» подписана «${prev.resp}» — подписант разрешается по «${how}», а не по виду меры: у одного ключа он один`);
  ...
  for(const pr of PROCESSES) for(const h of pr.hearings||[]){
    ...
    // D3 (зеркало D2): исход вносится ПОСЛЕ заседания (см. коммент у hDone) — если
    // заседание ещё впереди (hLeft>0), у него в принципе не может быть исхода, ни
    // процессуального, ни тем более по существу спора (это дело мéры, не заседания).
    if(hApplicable(h) && hDone(h) && (hLeft(h) ?? -1) > 0)
      add('D3-заседание', w, 'заседание предстоящее, но исход уже внесён — событие ещё не наступило');
  }
  ...
  /* G — история дела vs журнал мер.

     Финальный обзор ветки (02.08.2026): правило переписано на ту формулировку, которую
     билдер РЕАЛЬНО печатает. Прежняя версия искала в тексте истории «фаза «X»» и
     сверяла названную фазу с фазами требований дела. После пересева (волна ЗС) билдер
     печатает 529 строк истории, и НИ ОДНА этой подстроки не содержит — все строки о
     мерах имеют вид «Мера «вид» НОМЕР зарегистрирована…». Правило оставалось
     работоспособным (возврат ОДНОЙ строки билдера к доволновой формулировке давал 422
     находки), но недостижимым: оно удостоверяло «0 находок», не проверив ничего.
     Формулировка «→ фаза «X»» осталась только у истории, которую пишет ЖИВАЯ форма
     (collection.html:7677/7743) — статическую затравку эта линейка и видит, живой
     записи здесь нет; да и разойтись там нечему: форма подставляет в текст phaseOf()/
     свёртку журнала В МОМЕНТ ЗАПИСИ, то есть тот же вычислитель, с которым правило
     сверяло бы. Оставлять ветку «на всякий случай» — ровно тот декоративный чек,
     из-за которого правило и переписывается.

     Проверяемое высказывание — той же природы, что прежнее («история дела есть
     пересказ его журнала, а не отдельный рассказ»), но на живой формулировке:
     строка истории, называющая меру, называет меру ЭТОГО ЖЕ дела — с тем номером,
     тем видом, тем исходом и той датой, что стоят в журнале; и обратно — мера,
     которой в истории нет, из пересказа выпала. */
  for(const pr of PROCESSES){
    const W = `дело ${pr.id}`;
    const byNum = {}; for(const m of pr.measures||[]) if(m.num) byNum[m.num] = m;
    const told = new Set();
    for(const h of pr.history||[]){
      const hm = /^Мера «([^»]+)»\s+(\S+)/.exec(h.what||'');
      if(!hm) continue;
      const kind = hm[1], num = hm[2];
      const src = byNum[num];
      if(!src){ add('G1-история', `${W} · история ${h.when}`, `названа мера ${num} («${kind}»), а меры с таким номером в журнале дела нет`); continue; }
      told.add(num);
      if(src.kind !== kind)
        add('G1-история', `${W} · история ${h.when}`, `мера ${num} названа видом «${kind}», а в журнале дела она «${src.kind}»`);
      const ho = /исход «([^»]+)»/.exec(h.what||'');
      if(ho && ho[1] !== src.outcome)
        add('G1-история', `${W} · история ${h.when}`, `мера ${num}: в истории исход «${ho[1]}», в журнале ${src.outcome ? `«${src.outcome}»` : 'исхода нет'}`);
      const hd = String(h.when).split(' ')[0];
      if(hd !== evd(src))
        add('G1-история', `${W} · история ${h.when}`, `мера ${num} датирована в журнале ${evd(src)} — история рассказывает о ней другим днём`);
    }
    for(const m of pr.measures||[]){
      if(!m.num || told.has(m.num)) continue;
      add('G1-история', `${W} · мера ${m.num} «${m.kind}» (${evd(m)})`, 'в истории дела не отражена — журнал и его пересказ разошлись');
    }
  }
