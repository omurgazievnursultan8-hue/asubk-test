...
const out = await p.evaluate(() => {
  ...
  for(const pr of PROCESSES) for(const r of pr.requirements){
    ...
    // basisKinds != null — вид ТРЕБУЕТ основания — а basedOn не задан вовсе. A7 уже
    // проверяет basedOn, КОГДА ОНО ЗАДАНО (вид, дата, пересечение целей); этот код —
    // симметричная половина: ОТСУТСТВИЕ обязательного basedOn. «Частная жалоба» без
