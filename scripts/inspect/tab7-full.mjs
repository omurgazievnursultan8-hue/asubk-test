...
// tab 7 = «Документы»
await page.evaluate(()=>{const t=[...document.querySelectorAll('vaadin-tab')].find(t=>/Документ/i.test(t.innerText));t&&t.click();});await page.waitForTimeout(1500);
...
const dump=()=>page.evaluate(()=>{
  ...
  // any grids/tables (доку lists often render as vaadin-grid)
  ...
  return {fields,headers:[...new Map(headers.map(h=>[h.t+h.y,h])).values()],grids,btns};
});
log('TABS:',JSON.stringify(await page.evaluate(()=>[...document.querySelectorAll('vaadin-tab')].map(t=>t.innerText.trim()))));
log('===TAB7===');log(JSON.stringify(await dump(),null,1));
await page.screenshot({path:'.auth/t7.png',fullPage:true});
await ctx.close();
