...
ok('у живого платежа карточка говорит «зафиксированной нет и не должно быть»',
   /Зафиксированной нет и не должно быть/.test(g.textOf(g.pay('P-2041','alloc','curator'))));
...
ok('разные даты среза разреза и очереди названы прямо, а не спрятаны',
   (() => { g.ev(`setRcStage('R-305')`);
     ...
     return /Очередь опубликована кредитом на/.test(t) && /даты среза разные/.test(t); })());
...
ok('режим ставится ДО разбивки — разбор говорит это прямо',
   /ставится ДО разбивки/i.test(g.textOf(g.rec('R-311','parse','curator'))));
...
ok('карточка платежа показывает ссылку на списание и отличает его от погашения',
   (() => { const t = g.textOf(g.pay('P-2054','alloc','curator'));
     return /Списание уменьшило НАЧИСЛЕННОЕ, погашением не является/.test(t)
       && /в матрицу разбивки не попадает/.test(t); })());
