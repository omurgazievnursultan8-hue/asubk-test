...
// программная смена роли (setRole) обязана синхронизировать value #roleSel —
// иначе шапка на скриншотах врёт про роль (P3-R41)
const t6f = await page.evaluate(() => {
  setRole('dept');
  const dept = document.getElementById('roleSel').value;
  setRole('spec');
  const spec = document.getElementById('roleSel').value;
  return { dept, spec };
});
