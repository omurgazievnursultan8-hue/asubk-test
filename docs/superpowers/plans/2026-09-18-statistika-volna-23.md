...
4. **Цель, снятая с меры.** Пары, чьей цели в записи больше нет, остаются с последней строкой
   навсегда — выбытия цели в модели нет (та же граница, что у выбытия записи в `rowsAsOf`).
5. **Поток событиями в открытом месяце.** Событие переписывается на месте, строка состояния на
   прошедшую дату — нет: поток за интервал, кончившийся между днём платежа и его сторно в том
   же открытом месяце, после сторно платежа не видит, а прирост «погашено всего» между
   строками кредита, написанными до сторно, — видит. Тождество `#272` держится для интервала,
   чей конец не раньше последней правки его событий. Граница названа в комментарии двери.
