...
1. Снятие сторно целой меры о нескольких целях ложится на D, а не на месте: сторнированная
   ...
   набора. Ответ на прошедшие даты это не меняет, но в открытом месяце строк больше, чем при
   перезаписи на месте.
2. Пересчёт ночи раньше, который на свою дату изменения не видит (история датирована
   позже), работу из очереди снимает как «без изменений». Так было и до правки: работа
   адресует запись, а не дату. Изменение подхватит ближайшая ночь, если его назовёт очередь
   или опрос, — или сама, если это смена набора.
