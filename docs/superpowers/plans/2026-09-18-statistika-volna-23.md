...
### правка ревью этапа 3 — охват меры по всем целям, путь признака во всех дверях, датировка пути, комментарии З-16c, retro по слитой задаче

**Коммит.** «Статистика: волна 23 — правка ревью этапа 3: охват меры по всем целям, путь
признака во всех дверях, датировка пути, комментарии З-16c, retro по слитой задаче».
Смоук `283/283 PASS`, `exit=0`; время этого прогона не мерилось (шёл вместе с мутациями), отдельный прогон после правок 1–3 — 113 с. Новых сторожей нет; переписаны на месте `#287`,
`#291`, `#299`.

**1. З-19 — охват меры видит все цели.** `scopeTest` для `via target` проверял только цель
представителя (`r.part`). Теперь мера видна, если видна ЛЮБАЯ её пара на дату вопроса
(`ADR-0243` §3, схема §11.1). Пары берутся из `eventAt` на эту дату. Снятая цель (пара
«сторнирована» при живом представителе) в охват не идёт. При сторно всей меры идут все пары:
снятую цель от сторно меры отличает представитель, как в `fullNight`. `#291` переписан на
месте (случай Осмонова): срез мер аналитика — 2 (МВ-2026/19 + МВ-2026/31), реестр на ту же
дату — те же 2, представители — ТВ-2025/11-1 и ТВ-2026/03-1. Мера МВ-2026/31 видна по второй
цели, а не по представителю.

**2. З-21 — путь не теряет `now`.** `joinVia` копировал строку без `now`. Теперь копия несёт
`now` строки, и если у строки соседа признак «текущее» (`nb.now` содержит `T.id`), в `now`
присоединённой строки ложится имя признака пути (`ADR-0242` §4). Датировка пути идёт за
строкой соседа. Конъюнкт в `#287`: паспорт среза по «Тип лица заёмщика» — «текущее» с
`d-cbptype` в `now`; сними «текущее» у строк заёмщика — ответ «на дату».

**3. З-18c — путь соединяется во всех дверях.** Раньше путь соединялся только в
`ST.statSlice`. Теперь одна функция `askRows` (охват, затем путь) обслуживает `ST.statSlice`,
`ST.statSeries` (проверку домена и каждую точку), `rowsFor` (`ST.statRows` и
`ST.exportJob`) и `ST.flowBetween` (строки конца и базы). `ST.registryList` читает признак
пути через `viaOfItem` — ключ записи, затем запись соседа той же даты. Отказа нет: одно
значение даёт один ответ в каждой двери. Конъюнкт в `#287`: фильтр «юридическое лицо» даёт
одно множество из 4 в срезе, `statRows`, `exportJob` и `registryList`; поток «погашено» по
нему ненулевой, а сумма потоков по трём значениям равна потоку без фильтра. `ST.statSeries`
в списке ревью не был — он соединён по правилу «каждая дверь».

**4. З-16c — комментарии.** Блоки комментариев из шагов 4–8 брифа `task-4c` вставлены
дословно: над условием на месте в `deltaNight`, шапка «СОБЫТИЕ ДЕЛЬТОЙ», `corrOf`, пункт
шапки `fullNight`, над `here`, над второй веткой, фраза СМЕНЫ НАБОРА, шапка `offNightOf`,
дописка к `wholeOff`, путь состояний в `doRun` (СС-184), `dequeueSeen` п. 1 и п. 2, дверь
`ST.eventFlow` («ТОЖДЕСТВО ДЕРЖИТСЯ НАВСЕГДА»). В том же духе переписаны остальные места со
старой формулой «на месте в открытом месяце, в закрытом — поправкой»: `effectiveOf`,
`laterNightOf`, молчание в `deltaNight`, шапка МЕРА × ЦЕЛЬ, пункт СМЕНЫ НАБОРА в шапке
`fullNight`, `prev`, ветка `here`, путь событий в `doRun`. Устаревшие слова о дозаполнении
как задаче (`deltaNight`, путь состояний в `doRun`) заменены: задачи дозаполнения нет
(З-22b). Бриф `task-5` (З-17) и бриф `task-6a` (З-18a) сверены сценарием: каждый фрагмент
комментария из их блоков `js` есть в коде — недостающих нет. Код в этом пункте не менялся.

**5. З-22c/З-22b — retro по слитой задаче.** `retroPass` брал только задачи вида `retro`. Ответ
соседа, слитый в открытую ручную задачу, давал ей раннюю `from`, но вид оставался `manual`, и
пересчёт её пропускал. Ночь закрывала задачу «обойдён прогоном», ни одна ранняя дата не
пересчитывалась (`ADR-0245` §4). Решение координатора: задача берётся по датам, не по виду;
вид — ярлык и не повышается. Конъюнкт в `#299`: ручная задача и ответ соседа на одной записи
дают одну задачу `manual` с `from` 11.08, 11 retro-дат, строки 11.08 и 21.08 — «Касымов Т.»,
записи журнала с причиной `retro` есть. Комментарий `enq` исправлен.

**Отклонение от буквы решения.** По букве («каждая открытая задача с `from < D`») смоук дал
`278/283`: упали `#206`, `#267`, `#270`, `#276`, `#278`. У всех одна причина. Ручная задача
одна имеет `from` = `at` (день постановки). Если она пережила ночь постановки (поставлена
после её прогона, или прогон отбит дверью), её стал пересчитывать retro с дня постановки. Это
меняет модель ручной задачи, которую держат правка ревью 3 З-16b и СС-184: ручную задачу
отвечает ночь не раньше `at`. Взято наименьшее правило только по датам:
`q.from < q.at && q.from < D`. Задача пересчитывается, если у неё есть даты раньше
отвечающей ночи. Слитая задача и задача `retro` их имеют, одиночная ручная — нет. Пробник
ревью (`rv5c/p1.mjs`) даёт 11 retro-дат и с ручной задачей, и без неё. Тот же случай с датой
действия в закрытом июне даёт 51 дату с 02.07 в обоих вариантах. Буквальное правило —
вопрос координатору, в журнал решений не заносилось.

**Мутации** (копия дерева, по одной на правку):
- правка 1: `scopeTest` снова смотрит только `r.part` — `282/283`, упал ровно `#291`;
- правка 2: `joinVia` без переноса «текущего» — `282/283`, упал ровно `#287`;
- правка 3: `rowsFor` без соединения пути (`askRows(…, {})`) — `282/283`, упал ровно `#287`;
- правка 5: `retroPass` снова берёт только `kind === 'retro'` — `282/283`, упал ровно `#299`;
- правка 4 — только комментарии; сверка кода без комментариев с `9913ff1` показывает, что
  изменились только правки 1, 2, 3 и 5.
