...
- Produces: `_conclOpen` (объект), `_conclOpenAll` (bool), `_conclLogOpen` (объект), `conclToggle(key)`, `conclToggleAll()`, `conclLogToggle(key)`, `conclScrollTo(kind)`, `_conclGateBanner(app)`, `_conclChips(app)`, `_conclCard(app, key)`, `renderConclusions(app)` (полный)
...
function _conclBody(app, key){
  ...
  const conds = it.conds.length
    ? `<div class="concl-sub">Условия (${it.conds.length})</div><ol class="concl-cond-ro">${it.conds.map(x => `<li>${_esc(x.text)}</li>`).join('')}</ol>` : '';
  const files = it.files.length
    ? `<div class="concl-sub">Вложения (${it.files.length})</div><div class="concl-files">${it.files.map(f =>
        `<span class="concl-file">${_esc(f.name)} <i>${_esc(f.size)}</i> <a href="#" onclick="showToast('Демо: файл не хранится','info');return false;">Скачать</a></span>`).join('')}</div>` : '';
  return `<div class="concl-text">${_esc(it.text) || '—'}</div>${conds}${files}${_conclLogBlock(key, it)}`;
}
...
function _conclEditor(app, key){
  ...
  if (it.status === 'submitted'){
    ...
    const files = it.files.length
      ? `<div class="concl-sub">Вложения (${it.files.length})</div><div class="concl-files">${it.files.map(f =>
          `<span class="concl-file">${_esc(f.name)} <i>${_esc(f.size)}</i></span>`).join('')}</div>` : '';
    return `<div class="concl-locked">${_docIcoLock()}<span>Заключение внесено ${_esc(it.date)} · ${_esc(it.author)}</span>
      ...
      <div class="concl-text">${_esc(it.text)}</div>${conds}${files}`;
  }
