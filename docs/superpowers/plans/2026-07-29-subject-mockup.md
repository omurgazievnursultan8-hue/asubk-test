...
const IP_REG = [
  ...
  /* Своя регистрация ИП у S-DUP (Task 9): половинчатая миграция принесла и её. Без этой
     строки перенос IP_REG при слиянии не проверяется ничем — массив числится в OWN_ARRAYS,
     а двигать ему нечего, и удаление его из списка проходит незамеченным. Период закрыт
     смертью лица (20.01.2025), поэтому тип лица ни у кого не меняется задним числом.
     Начало — 2021 год, а не 2018: тест 2.5 спрашивает разрез ИП на 01.06.2020 и опирается
     на то, что в 2020-м ИП в наборе ровно один. Дата начала здесь ничего не значит по делу,
     а утверждение теста о наборе — значит, поэтому двигается дата, а не тест. */
  { key:'S-DUP', no:'ИП-0770117', from:'10.02.2021', to:'20.01.2025', doc:'Патент 0770117' },
];
...
const SUBJECT_EVENTS = [
  ...
  /* Своё событие S-DUP: половинчатая миграция принесла и это — тот же факт, что уже стоит
     на 07701199970071, но своим документом. Слияние не дедуплицирует содержимое, оно
     переносит запись целиком на главный ключ (СБ-11). Без своих фактов у S-DUP тест 8.5
     «своё перенесено» доказывался бы на пустом месте: ни один массив не содержал бы
     ссылки на S-DUP и до переноса. То же — своя связь L-6 и свой документ ниже. */
...
ok('6.2 новая регистрация ИП после закрытой возвращает тип «ИП» на новые даты',
  (() => { g.ev("addIpReg({key:'04401199940041',no:'ИП-0044012',from:'01.02.2026',doc:'Патент 0044012'})");
    return g.ev("personKindAt('04401199940041','01.03.2026')")==='ИП' &&
           /* Дата берётся из промежутка между двумя регистрациями: старая закрыта 31.12.2025,
              новая начинается 01.02.2026. Внутри старого окна ответ «физ» недостижим — там
              лицо и было ИП, и требовать обратного значит требовать поломки. */
...
**Осадок тестов 6.2–6.4 убирать нельзя, и это не оплошность.** Регистрация `ИП-0044012` и
события «утрата статуса ИП» / «ликвидация» остаются в общих `IP_REG` и `SUBJECT_EVENTS`
до конца прогона, потому что на них стоят следующие проверки: 6.5 и 6.7 читают
ликвидацию, 6.6 — закрытую регистрацию. Уборка сломала бы их. Отсюда правило для задач
8 и 9: **не опираться на число строк в `IP_REG` и `SUBJECT_EVENTS` для ключей
`04401199940041` и `02201199920021`**. Задача 9 и так работает на своём экземпляре jsdom
(`m`), задача 8 считает роли и документы, а не события.
...
/* .every() на пустом списке истинно: без первой строки тест прошёл бы и у tabsFor,
   вернувшего группе вообще ничего. Сначала доказываем, что вкладки у неё есть. */
...
/* Негативная проверка на весь #cardMount проходит и на пустой карточке — сначала
   доказываем, что карточка реально что-то отрисовала (имя субъекта), и лишь потом,
   что денег в ней нет. */
...
- [ ] Открыть `mockups/subject/subject.html` в браузере и пройти руками: реестр → фильтр по типу → карточка организации → вкладка «Связи» → карточка группы → «Состав» → карточка бывшего ИП со срезом `#/s/04401199940041?on=2020-06-01` (параметр живёт **внутри хеша**, а не в строке запроса — `?on=…#/s/…` срез не включит и выглядит как дефект) → разрез «похожие записи» → сравнение → слияние → маршрут по псевдониму
