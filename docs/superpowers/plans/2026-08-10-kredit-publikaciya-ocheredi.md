...
**Соответствие таблице §6 спеки.** Восемь строк таблицы закрываются так: `:675` — Step 2;
`:656` — Step 3; `:3547`/`:3586` (ссылки на Д-8 внутри `buildLedger`) — уже переписаны
комментариями в Task 2; `:6414` — Step 6; `:6405` — Step 6; `:6358` — Step 1;
`:189`/`:218`/`:365`/`:1537` — Step 5; «шапка, список источников» — Step 4: отдельного
перечня ADR в шапке нет, источники цитируются внутри решений `Р-*`, и `ADR-0060` входит
туда блоком Р-31.
