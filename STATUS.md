...
> Last updated: 2026-07-09 (loan-application mockup — залоговые документы: сворачиваемые под-панели по предметам) — _(update this date every time you edit)_
...
- 2026-07-09 — **Залоговые документы — сворачиваемые под-панели по предметам** (`loan-application.html`,
  ...
  готовности `N/M` + статус-чипом (готов ✓ / блок / в работе) в шапке. Авто-раскрытие: собранный
  предмет свёрнут, несобранный/блокированный раскрыт; клик перекрывает (`_docCollOpen`). Скан
  готовности — без раскрытия. Проверено в браузере на 5 предметах (вкл. авто-collapse готового
  + ручной toggle). Выбран **вариант A** (аккордеон по предметам) — матрица/по-типу отпали:
