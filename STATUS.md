...
> Last updated: 2026-08-04 (журнал догнан по кредиту за 27.07…04.08: волна приведения карточки к ADR-модели, подача карточки, волна ADR-0042 «план · прогноз · исполнение», реестр и фильтр, единый переключатель области КВ-17; смоук `credit-check.mjs` 98/98, jsdom-тесты `mockups/loan-credit/tests/` 38/38) — _(update this date every time you edit)_
...
| Кредит: доработка макета после передачи | admin | 2026-07-27 | Волны 27.07…04.08 (КВ-1…КВ-17, КР-1…КР-57) — макет ушёл вперёд пакета от 26.07. Учёт волн — `mockups/loan-credit/ASUBK-status-razrabotki.md`. Требует переиздания: `docs/tasks/p15-kredit-tasks.html` (частично догнан 03.08) и `requirements/tz/05-kredit.html` |
