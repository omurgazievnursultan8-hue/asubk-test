...
> Last updated: 2026-08-04 (взыскание: **шаг 2 наряда сверки закрыт — модуль считает
> просрочку с 1-го дня, а не с 6-го** (п. 16.1); попутно `ADR-0062` снизил предел окна
> ожидания с 14 к.д. до 7 (дефект Н-1). Смоук 660/32 — без новых провалов. Ранее в тот же
> день: шаг 1 (перенумерация 731 ссылки, ❌ 0), получена сама редакция №41, сделана сверка
> `docs/normativ/2026-08-04-sverka-red-N41.md`, письмо ГФХ переиздано редакцией 2,
> пакет передачи P14 переиздан редакцией 2. Содержательных расхождений осталось 11,
> шаги 3–6 — впереди) — _(update this date every time you edit)_
...
| Приведение модуля взыскания к редакции Порядка №41 | Волна правок по `docs/normativ/2026-08-04-sverka-red-N41.md` | 2026-08-04 | Шаги 1 (перенумерация) и 2 (1-й день просрочки) ✅ закрыты 04.08.2026 — пакет передачи P14 разморожен. Осталось: пороги и паузы → маршруты → новые ветви и предусловия → справочник видов → письмо ГФХ |
...
| Порог просрочки в модуле заёмщика | Отдельная волна по модулю заёмщика | 2026-08-04 | `catByDays` живёт вторым экземпляром в `mockups/borrower/*` (+ ТЗ 04 и смоук `borrower-check.mjs` с границами 5/6/181) и по-прежнему начинает средний риск с 6-го дня. Норма (п. 16.1) та же, что во взыскании; правка — не продолжение шага 2, а своя волна |
...
- 🟠 **Модуль взыскания приведён к редакции Порядка №41 наполовину.** Действующая — №41 от
  06.07.2026 (`docs/normativ/poryadok-2026-07-06-N41.pdf`). Нумерация приведена 04.08.2026
  (731 ссылка в 36 артефактах, проверка `python3 scripts/check_points.py` зелёная), тогда же
  закрыто самое дорогое расхождение — просрочка считается с **1-го** дня, и вместе с ней
  переехали автооткрытие дела, окно ожидания и база половины сроков. **Осталось 11 мест**:
  гарантийное письмо ставит паузу только решением комитета и только на безакцепт, узкая
  пауза реструктуризации, отложенный 181-й день, другие маршруты после возврата ИЛ, смерть
  заёмщика и предусловия банкротства, недостающие виды мер. Разбор —
