...
> Last updated: 2026-08-04 (взыскание: волна модельных пробелов «МП» + наряд §30 закрыты в коде —
> реестровые волны ТР…ЗС и 5 кругов разбора МП дали 18 решений и ADR `0043…0054`;
> `collection-check.mjs` 660/32, `collection-data-audit.mjs` 0 находок, `collection-seed-coverage.mjs`
> починен — 43/43, 0 находок. Пакет передачи разработчикам `docs/tasks/p14-collection-tasks.html`
> (22.07) устарел относительно модели — реисью не делался) — _(update this date every time you edit)_
...
| Пакеты передачи по модулям (мокап + ТЗ + задачи) | admin | 2026-07-19 | Актуальны: залог (P13), кредит (P15, 2026-07-26), заёмщик (P4-R6…R25, 2026-07-27). **Взыскание (P14) устарел** — пакет `docs/tasks/p14-collection-tasks.html` от 22.07 предшествует волне «МП»/наряду §30, реисью не делался |
| Взыскание — волна «МП» (модельные пробелы) + наряд §30 | admin | 2026-07-27 | Реестровые волны ТР…МТ, модельные ПМ/РМ/ПЕ/РР/ДС/ЗС, 5 кругов разбора МП (18 решений, ADR `0043…0054`), наряд §30 п.1–п.7 + МП-11 + «выручка/очередь» + МП-2 — все реализованы в код. `collection-check.mjs` 660/32 (32 — старый долг карточки дела/`ПЕ`, не новый), `collection-data-audit.mjs` 0 находок, `collection-seed-coverage.mjs` 43/43 0 находок. Журнал волн — `mockups/collection/ASUBK-status-razrabotki.md`. Следующий шаг — реисью пакета передачи P14 |
...
- **Взыскание — пакет передачи разработчикам устарел.** `docs/tasks/p14-collection-tasks.html`
  (22.07.2026) — «legacy»-формат без JSON-острова (`scripts/check_tasks.py` его не проверяет),
  модуль с тех пор пережил три волны переработки по существу; по правилу CLAUDE.md
  «модуль реворкнутый по существу переиздаётся целиком под тем же именем» — кандидат на реисью,
  не сделан.
