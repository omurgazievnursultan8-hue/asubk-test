...
> Last updated: 2026-08-04 (взыскание: **шаг 1 наряда сверки закрыт — все 35 артефактов
> модуля переведены на нумерацию действующей редакции Порядка №41**, `check_points.py`
> зелёный: 687 ссылок, ❌ 0. Содержательные расхождения — 12 мест, шаги 2–6 — впереди.
> Ранее в тот же день: получена сама редакция №41, сделана сверка
> `docs/normativ/2026-08-04-sverka-red-N41.md`, письмо ГФХ переиздано редакцией 2,
> пакет передачи P14 переиздан редакцией 2) — _(update this date every time you edit)_
...
| Приведение модуля взыскания к редакции Порядка №41 | Волна правок по `docs/normativ/2026-08-04-sverka-red-N41.md` | 2026-08-04 | Шаг 1 (перенумерация) ✅ закрыт 04.08.2026. Осталось: 1-й день просрочки вместо 6-го → пороги и паузы → маршруты → новые ветви и предусловия → справочник видов → письмо ГФХ. Пакет передачи P14 (редакция 2) отдавать в разработку **после** шага 2: базы отсчёта половины сроков меняются |
| Нумерация Порядка в четырёх других модулях | Таблицы соответствия для их пунктов нет | 2026-08-04 | `CONTEXT.md`, заёмщик (ТЗ 04 + мокап), платежи, реструктуризация цитируют тот же Порядок по прежней нумерации — перечень в §6.1 сверки. Пункты 89, 90, 93, 97 в таблице §3 отсутствуют: аналоги в №41 не установлены |
...
- 🔴 **Модуль взыскания построен на устаревшей редакции Порядка.** Действующая — №41 от
  06.07.2026 (`docs/normativ/poryadok-2026-07-06-N41.pdf`). Нумерация приведена 04.08.2026
  (687 ссылок в 35 артефактах, проверка `python3 scripts/check_points.py` зелёная), но
  **содержание — нет**: разошлись 12 мест — самое дорогое: средний риск
  наступает с **1-го** дня просрочки, а не с 6-го, а от 6-го дня в модуле считается
  автооткрытие дела, окно ожидания и база половины сроков. Разбор —
