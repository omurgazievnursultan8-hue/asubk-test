...
### Live re-inspection 2026-06-29

> Проверено 2026-06-29, `scripts/inspect/app-capture.mjs`.

Новые наблюдения, зафиксированные после документации 2026-06-18/19:

- **Статистические плитки (stat cards)** над гридом заявок — добавлены 5 цветовых сводных тайлов:
  **Всего заявок** (синий) · **На рассмотрении** (янтарный) · **Одобрено** (зелёный) ·
  **Отклонено** (красный) · **Требуется доп. информация** (фиолетовый). Показывают
  счётчик соответствующих заявок (интерактивность клика по плитке на стенде не подтверждена).
- **Новый статус «Новый»** — добавлен в набор значений «Статус заявки» (в дополнение к
  «На рассмотрении» / «Одобрено» / «Отклонено» / «Требуется доп. информация»).
- **Степпер статуса** на странице детали теперь отображает зелёную галочку + дату
  под завершёнными шагами (Подана → На рассмотрении → Одобрена → Регистрация кредита).
- **«Условия кредита, одобренные комиссией»** — рядом с полями одобренной суммы/срока
  появилась подсказка диапазона: **«Допустимо: от … до …»** (границы из кредитной
  программы). Уточнение: заголовок раздела — «одобренные комиссией» (не «кредитной
  комиссией»); во второй колонке «Общей информации» добавлено поле **«Кредитная программа»**.
- **Новый мокап** клонирует весь этот поток as-is:
  `mockups/loan-application/loan-application.html` — список заявок + мастер «Новая
  заявка» + страница детали (все 9 вкладок) + диалоги «Выберите комиссию»; воспроизводит
  живые дефекты (серые поля, eager-валидация, нет «Просмотр»), без улучшений.
