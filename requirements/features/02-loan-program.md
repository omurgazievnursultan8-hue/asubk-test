...
|---|---|---|---|---|
| P2-R1 | 🟡 Low | **Field visual semantics** (agreed 2026-06-17). Three field states: required-editable → white bg + border + **red `*`** at the label; optional-editable → white bg + border, no `*`; read-only/computed → grey, no border. Show the `*` **immediately on form open**, not only after a failed submit. Implement as **one shared design-system field style with Phase 1 R6** and apply across all 9 tabs. **Large-form error UX (merged from former P2-R6):** the validation toast is generic and names neither tab nor field — badge/count the tabs containing errors (red indicator + number) on the 9-tab wizard, and show an inline «Обязательное поле» under each invalid field, not just a red border. (Out of scope by decision: auto-switch+focus to the first error tab, aggregate list of missing fields.) | A ~40-field wizard where required/optional/read-only look identical is hard to fill correctly. | ✅ confirmed (P2-01, P2-06) — recurs P1-06 |
