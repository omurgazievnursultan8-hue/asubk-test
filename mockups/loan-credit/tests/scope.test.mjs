...
test('T4-1: «Прогноз» при «по кредиту» — слитая таблица с колонкой «Транш»', () => {
  ...
  has(h, 'Позиции впереди (по кредиту)', 'заголовок должен назвать и область, и то, что таблица — только будущее');
...
test('T4-2: счёт позиций и итог считают по всем траншам области', () => {
  ...
  has(h2, 'Позиции впереди (по кредиту)', 'заголовок секции от окна не зависит');
...
test('T4-3: на конкретном транше «Прогноз» колонки «Транш» не показывает', () => {
  ...
  has(h, 'Позиции впереди (транш №2)', 'заголовок должен назвать транш');
});
