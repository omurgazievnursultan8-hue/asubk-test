...
test('T4-1: «Прогноз» при «по кредиту» — слитая таблица с колонкой «Транш»', () => {
  ...
  has(h, 'Ожидаемые взносы (по кредиту)', 'единственный заголовок вкладки называет область и содержимое — ожидаемое, то есть будущее');
...
test('T4-2: счёт позиций и итог считают по всем траншам области', () => {
  ...
  has(h2, 'Ожидаемые взносы (по кредиту)', 'заголовок секции от окна не зависит');
...
test('T4-3: на конкретном транше «Прогноз» колонки «Транш» не показывает', () => {
  ...
  has(h, 'Ожидаемые взносы (транш №2)', 'заголовок должен назвать транш');
});
