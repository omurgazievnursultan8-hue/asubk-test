...
test('F2-3: «Внести освоение» при «по кредиту» — тоже погашено (действие адресное)', () => {
  const { CR } = load();
  const c = multiCredit(CR);
  CR.openDetail('K-C40');
  CR.setCardScope('credit');
  const h = CR.renderTab('Транши', c);
  has(h, 'Освоение — действие по траншу; выберите транш в шапке',
      'гейт нельзя мерить траншем №1, когда область — кредит целиком');
  hasNot(h, 'CR.openDisbModal()', 'погашенная кнопка модалку открывать не должна');
  CR.setCardScope(1);
  has(CR.renderTab('Транши', c), 'CR.openDisbModal()', 'на транше кнопка обязана ожить');
});
