...
/* F2-1б (КВ-36) — тело переехало на «Расчёты» целиком, а не «показано в двух местах».
   Проба сторожит переезд с обоих концов и на обеих областях: на «Составе» слова нет ни
   при какой области, на «Расчётах» таблица тела есть и сама сужается до выбранного транша. */
test('F2-1б: остаток тела — только на «Расчётах», и область его сужает', () => {
  const { CR } = load();
  const c = multiCredit(CR);
  CR.openDetail('K-C40');
  CR.setCardScope('credit');
  ...
  has(r, 'Тело по траншам', 'разбивка тела обязана стоять всегда, а не только при переносах');
  has(r, 'Остаток тела', 'колонка ИР-3 живёт здесь');
  ...
  has(r2, 'Тело транша №2', 'на транше таблица тела называет транш');
  has(r2, 'на этот свод не действует', 'свод обязан сказать, что область его не сужает');
});
