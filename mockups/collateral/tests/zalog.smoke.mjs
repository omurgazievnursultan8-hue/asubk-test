...
// §18 п.47 (Reg-47): регрессия Р-1…Р-9 — базовые демо-объекты на месте и расчёт по всем
...
test('W12-1 (КД-12): «покрыт*» не осталось ни в одной строке модуля вне комментариев', () => {
...
test('W12-2 (КД-12): слово живо только там, где объясняет собственный запрет', () => {
  const src = readFileSync(new URL('../zalog.html', import.meta.url), 'utf8');
...
test('W12-3 (КД-12): сидовая история не несёт термин ни на предметах, ни на договорах', () => {
  const { zt } = load();
...
test('W12-4 (КД-12): карточка предмета чиста — там слово держалось дольше всего', () => {
  const { win, zt } = load();
  win.eval("role='Куратор отдела залогового обеспечения'");
  zt.ITEMS.forEach(it => {
...
test('W12-5 (КД-12): справочник регламентных параметров переписан на «обеспеченность»', () => {
  const { win } = load();
